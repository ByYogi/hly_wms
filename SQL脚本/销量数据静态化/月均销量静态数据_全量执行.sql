...
INCLUDE(RootArea);
--drop table #childArea

PRINT('------------ 清除所有静态数据 ------------')
TRUNCATE TABLE Tbl_Cargo_MonthSaleStatic

PRINT('------------ 获取产品每月销量 ------------')
;WITH 
-- 获取从第一个销售单的创建时间到今天所有年月时间
DateRange AS (
    SELECT TOP 1 DATEADD(MONTH, DATEDIFF(MONTH, 0, MIN(OP_DATE)), 0) AS MinMonth,
           DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()), 0) AS MaxMonth
    FROM Tbl_Cargo_OrderGoods
),
MonthDim AS (
...
SELECT
    ...
    CEILING(WeightedMonthSale) WeightedMonthSale, --近3月加权月均销量
