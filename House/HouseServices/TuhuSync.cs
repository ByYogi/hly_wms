    ...
    internal class TuhuSync
    {
        ...
        public void TuhuFullSync()
        {
            ...
            try
            {
                ...
                {
                    log.Memo = $"未映射商品数：{unmappedProducts.Count}，未映射编码：{string.Join(",", unmappedProducts)}";
                ...
                skuDelAllRes = TuhuHandle.PostJson(
                    TuhuConfig.Test.Url + TuhuConfig.UrlSkuDeleteAll,
                    new TuhuPushDto(TuhuConfig.Platform, TuhuConfig.Test.AuthCode),
                    comheaders
                );

                // 开始推送商品数据
                // 根据 TuhuCode 分组，汇总库存，拼接 ProductCodes
                var groupedData = prductData
                ...
                log.Memo = $"途虎商品全量同步成功。同步数量：{groupedData.Count}（分组后）。{unmappedInfo}{tuhuRes}";
                logbus.InsertLog(log);
