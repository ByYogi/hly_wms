...
namespace HouseServices
{
    internal class TuhuSync
    {
        ...
        public void TuhuFullSync()
        {
            ...
            //获取途虎Url连接信息
            string tuhuUrl = TuhuConfig.ProdUrl;
            ...
            try
            {
                ...
                foreach (var batch in batches)
                {
                    ...
                    var tuhuPushSkuBody = new TuhuPushDto()
                    {
                        ...
                        authCode = TuhuConfig.ProdAuthCode,
                    ...
                    var skuRes = TuhuHandle.PostJson(
                        TuhuConfig.ProdUrl + TuhuConfig.UrlPushSku,
                        tuhuPushSkuBody,
                        comheaders
                    );
                    ...
                    var stockRes = TuhuHandle.PostJson(
                        TuhuConfig.ProdUrl + TuhuConfig.UrlPushStock,
                        tuhuPushStockBody,
                        comheaders
                    );
                    ...
                    var priceRes = TuhuHandle.PostJson(
                        TuhuConfig.ProdUrl + TuhuConfig.UrlPushPrice,
                        tuhuPushPriceBody,
                        comheaders
                    );
        ...
        public void TuhuIncrementalSync()
        {
            ...
            try
            {
                ...
                var willAsyncBrandIDs = dict.Keys.Select(x => Convert.ToInt32(x.Split('_')[1])).Distinct().Where(x => TuhuConfig.Brands.Keys.ToArray().Contains(x)).ToArray();
            ...
            catch (Exception ex)
            {
                string tuhuRes = $"\r\n同步商品库存响应集：{string.Join(",", pushStockResList)}";
                log.Status = "1";
                log.Memo = $"途虎商品库存增量同步失败。{tuhuRes}" + ex.FormatErr();
    ...
    internal static class TuhuHandle
    {
        ...
        public static Dictionary<string, string> BuildHeaders()
        {
            ...
            string sign = Sha256(
                $"appId={TuhuConfig.ProdAppId}&nonceStr={nonceStr}&signType=sha256&timestamp={timestampMs}{TuhuConfig.ProdAppSecret}"
            );

            return new Dictionary<string, string>
            {
                ["appid"] = TuhuConfig.ProdAppId,
                ["noncestr"] = nonceStr,
                ["signType"] = "sha256",
                ["sign"] = sign,
                ["authType"] = "openapi",
    ...
    static class TuhuConfig
    {
        ...
        public static string TestAuthCode => Get("Tuhu.test.authCode");
        ...
        public static string ProdAuthCode => Get("Tuhu.prod.authCode");
