...
namespace House.Business.Cargo
{
    ...
    public class CargoPriceBus
    {
        ...
        /// </summary>
        /// <param name="entity"></param>
        public void UpdateProductNextDayPrice(List<CargoProductEntity> entity, LogEntity log)
        {
            LogWrite<CargoProductEntity> lw = new LogWrite<CargoProductEntity>();
            try
            {
                foreach (var it in entity)
                {
                    man.UpdateProductNextDayPrice(it);
                    log.Memo = "修改产品次日达单价：产品ID：" + it.ProductID.ToString() + "，产品名称：" + it.ProductName.Trim() + "，型号：" + it.Model.Trim() + "，花纹：" + it.Figure + "，修改后单价：" + it.NextDayPrice.ToString();
                    lw.WriteLog(log);
                }
            }
            catch (ApplicationException ex)
            {
                log.Status = "1";
                log.Memo = "修改产品次日达单价失败，失败信息：" + ex.Message;
                lw.WriteLog(log);
            }
        }
        /// <summary>
        ...
        public void UpdateWebProductSalePrice(List<CargoProductEntity> entity, LogEntity log)
        {
            ...
            try
            {
                foreach (var it in entity)
                {
                    ...
                    log.Memo = "修改产品批发销售价格：产品ID：" + it.ProductID.ToString() + "，产品名称：" + it.ProductName.Trim() + "，规格：" + it.Specs + "，花纹：" + it.Figure + "，修改前进仓价：" + it.UnitPrice.ToString() + "，修改后进仓价：" + it.InHousePrice.ToString() + "，修改后销售价：" + it.SalePrice.ToString() + ",供应商编码:" + it.SuppClientNum.ToString() + ",规格类型:" + it.SpecsType;
                    lw.WriteLog(log);
                }
            }
            catch (ApplicationException ex)
        ...
        {
            return man.QueryAllTypeIDToProductID(entity);
        }
        ...
        public void UpdateProductSalePrice(List<CargoProductEntity> entity, LogEntity log)
        {
            ...
            try
            {
                foreach (var it in entity)
                {
                    ...
                    log.Memo = "修改产品销售价：产品编码：" + it.ProductCode + "，批次年：" + it.BatchYear + "规格类型：" + it.SpecsType + "，修改前销售价：" + it.CostPrice.ToString() + "，修改后销售价：" + it.SalePrice.ToString() + "，修改后进仓价：" + it.InHousePrice.ToString() + ",客户编码：" + it.SuppClientNum;
                    lw.WriteLog(log);
                }
            }
            catch (ApplicationException ex)
        ...
        {
            return man.QueryPriceModifyData(pIndex, pNum, entity);
        }
        ...
        /// </summary>
        /// <param name="entity"></param>
        public void AddPriceModify(List<CargoPriceModifyEntity> entity, LogEntity log)
        {
            LogWrite<CargoPriceModifyEntity> lw = new LogWrite<CargoPriceModifyEntity>();
            //使用事务
            using (TransactionScope scope = new TransactionScope(TransactionScopeOption.Required))
            {
                try
                ...
                {
                    log.Status = "1";
                    log.Memo = "新增价格修改数据失败，失败信息：" + ex.Message;
                    lw.WriteLog(log);
                    throw;
                }
            }
