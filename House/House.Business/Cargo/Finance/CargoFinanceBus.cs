...
namespace House.Business.Cargo
{
    ...
    public class CargoFinanceBus
    {
        ...
        /// <param name="entity"></param>
        /// <param name="log"></param>
        public void plSecondCheckReserveOrder(List<CargoReserveOrderEntity> entity, LogEntity log)
        {
            ...
            using (TransactionScope scope = new TransactionScope(TransactionScopeOption.Required))
            {
                try
                {
                    man.plSecondCheckReserveOrder(entity);
                    log.Memo = "订单号：";
                    foreach (var awb in entity)
                    {
                        log.Memo += awb.OrderNo.Trim() + ",";
        ...
        /// </summary>
        /// <param name="awb"></param>
        public void SaveCash_Advance(CargoCashierEntity awb, LogEntity log)
        {
            LogWrite<CargoCashierEntity> lw = new LogWrite<CargoCashierEntity>();
            //使用事务
            using (TransactionScope scope = new TransactionScope(TransactionScopeOption.Required))
        ...
        {
            return man.QueryCenterOrder(entity);
        }
        /// <summary>
        /// 更新CargoOrder已退款状态等
        /// </summary>
        ...
        {
            LogWrite<CargoExpenseEntity> lw = new LogWrite<CargoExpenseEntity>();
            CargoClientManager clientManager = new CargoClientManager();
            //使用事务
            using (TransactionScope scope = new TransactionScope(TransactionScopeOption.Required))
            {
                try
                    ...
                    if (ty.Equals(1))//通过
                    {
                        foreach (var it in entity)
                        {
                            ...
                            if (it.Status.Equals("3"))
                            {
                                ...
                                bool IsPreCollect = false;
                                CargoExpenseEntity expenseEntity = man.GetExpenseById(it.ExID);
                                ...
                                if (IsPreCollect)
                                {
                                    CargoClientPreRecordEntity ccpr = new CargoClientPreRecordEntity();
                                    ccpr.ClientID = expenseEntity.ClientID;
                                    ccpr.ExID = expenseEntity.ExID.ToString();
                                    ccpr.Money = expenseEntity.ExCharge;
                                    ccpr.RecordType = "0";
                                    ccpr.OperaType = "0";
        ...
        /// <param name="entity"></param>
        /// <param name="log"></param>
        public void UpdateRefundCheckStatus(WXOrderEntity entity, LogEntity log, ref int isAuto)
        {
            ...
            using (TransactionScope scope = new TransactionScope(TransactionScopeOption.Required))
            {
                try
                {
                    ...
                    var orderAwbStatus = Convert.ToInt32(entity.AwbStatus);
                    var openOrderAwbStatus = 0;
                    ...
                    if ((entity.ThrowGood == "23" && !AwbStatus) || (entity.ThrowGood == "22" && wXOrder.OrderStatus == "0") || (entity.ThrowGood == "26" && wXOrder.OrderStatus == "0"))//订单未出库 
                    {
                        ...
                        //2,次日达单 同时删除中心仓订单
                        if (entity.ThrowGood == "23" && !string.IsNullOrEmpty(entity.OpenOrderAwbStatus) && Convert.ToInt32(entity.OpenOrderAwbStatus) <= 2)
                        {
                            var Order = busOrder.QueryOrderInfo(new CargoOrderEntity { OrderNo = entity.OpenOrderNo });
                            busOrder.DeleteOrderInfo(new List<CargoOrderEntity>() { new CargoOrderEntity { OrderNo = Order.OrderNo, OrderID = Order.OrderID, DeleteID = "2029", DeleteName = "机器人" } }, log);
                        }
                        isAuto = 2;
                    ...
                    else if (entity.ThrowGood == "23" && AwbStatus && !string.IsNullOrEmpty(wXOrder.FacOrderNo) && !string.IsNullOrEmpty(entity.InCargoStatus) && entity.InCargoStatus.Equals("1"))
                    {
                        isAuto = 1;
                        //2、已支付已生成退货单已入库-审核通过
                        wXOrder.RefundCheckStatus = "1";
        ...
        /// <param name="entity"></param>
        /// <param name="log"></param>
        public void UpdateRefundCheckStatus_Reserve(WXOrderEntity entity, LogEntity log)
        {
            LogWrite<WXOrderEntity> lw = new LogWrite<WXOrderEntity>();
            //使用事务
            using (TransactionScope scope = new TransactionScope(TransactionScopeOption.Required))
            {
                try
                {
                    ...
                    var orderDataList = busOrder.QueryReserveOrderAndOrder(new CargoReserveOrderEntity { OrderNo = entity.CargoOrderNo });
        ...
        {
            return man.GetAreaData(HouseID);
        }
    }
}
