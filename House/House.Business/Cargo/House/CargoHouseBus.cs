...
namespace House.Business.Cargo
{
    ...
    public class CargoHouseBus
    {
        ...
        public void SaveMoveContainer(CargoContainerShowEntity oldContainer, CargoContainerShowEntity newContainer, LogEntity log)
        {
            ...
            using (TransactionScope scope = new TransactionScope(TransactionScopeOption.Required))
            {
                try
                {
                    ...
                    {
                        //说明已经存在货位与产品关联表数据就修改
                        man.UpdateCargoContainerGoodsPiece(new CargoContainerGoodsEntity { ID = good.ID, Piece = oldContainer.Piece, IsAdd = true });
                    }
                    else
                    {
                        //新增货位与产品关联表数据
                        man.AddCargoContainerGoods(new CargoContainerGoodsEntity { ContainerID = newContainer.ContainerID, TypeID = oldContainer.TypeID, ProductID = oldContainer.ProductID, Piece = oldContainer.Piece, Weight = 0, InHouseTime = DateTime.Now, IsPrintInCargo = false, InCargoID = newContainer.InCargoID });
                    }
        ...
        /// </summary>
        /// <param name="entity"></param>
        public void SaveStockTake(CargoStockTakeEntity entity, LogEntity log)
        {
            ...
            using (TransactionScope scope = new TransactionScope(TransactionScopeOption.Required))
            {
                try
                {
                    StockID = man.SaveStockTake(entity);
                    log.Memo = "新增盘点单，盘点单号:" + StockID.ToString() + ",区域大仓:" + entity.HouseID.ToString() + ",前置仓:" + entity.HeadHouseName + ",盘点区域:" + entity.AreaName + ",创建人:" + entity.CreateName;
                    lw.WriteLog(log);
                    scope.Complete();
                }
                catch (ApplicationException ex)
