    ...
    public class CargoInterfaceBus
    {
                ...
                try
                {
                    ...
                    switch (entity.ParentID)
                    {
                        case 1:
                            ...
                            // 2025-09-15添加供应商
                            productEntity = productMan.QueryProductInfo(new CargoProductEntity { HouseID = entity.HouseID, TypeID = entity.TypeID, Specs = entity.Specs, Figure = entity.Figure, Batch = entity.Batch, ContainerCode = entity.ContainerCode, GoodsCode = entity.GoodsCode, BelongDepart = entity.BelongDepart, OperaType = "1", Source = entity.Source, SpecsType = entity.SpecsType, SuppClientNum = entity.SuppClientNum, IsSpecsType = "5", TyreModel = entity.TyreModel });
                            break;
                        case 113:
                            //车模
                            productEntity = productMan.QueryProductInfo(new CargoProductEntity { HouseID = entity.HouseID, TypeID = entity.TypeID, Batch = entity.Batch, ContainerCode = entity.ContainerCode, GoodsCode = entity.GoodsCode, IsSpecsType = "5", SuppClientNum = entity.SuppClientNum, TyreModel = entity.TyreModel });
                            break;
                        case 354:
                            ...
                            break;
                        default:
                            productEntity = productMan.QueryProductInfo(new CargoProductEntity { HouseID = entity.HouseID, TypeID = entity.TypeID, Specs = entity.Specs, ContainerCode = entity.ContainerCode, GoodsCode = entity.GoodsCode, IsSpecsType = "5", SuppClientNum = entity.SuppClientNum, TyreModel = entity.TyreModel });
                            break;
                    }
                    //判断产品表里是否存在，如果 没有新增
                    if (productEntity == null || productEntity.ProductID.Equals(0) || productEntity.Numbers <= 0)
        ...
        public void ScanMoveOrderInCargo(CargoProductEntity entity, LogEntity log)
        {
            ...
            using (TransactionScope scope = new TransactionScope(TransactionScopeOption.Required))
            {
                ...
                {
                    bool isAddNew = false, curDay = false;
                    CargoProductEntity productEntity = productMan.QueryProductInfo(new CargoProductEntity { HouseID = entity.HouseID, TypeID = entity.TypeID, Specs = entity.Specs, Figure = entity.Figure, Batch = entity.Batch, ContainerCode = entity.ContainerCode, GoodsCode = entity.GoodsCode, SuppClientNum = entity.SuppClientNum, IsSpecsType = "5", TyreModel = entity.TyreModel });
        ...
        #region 数据接口生成次日达库存
        public void SaveNextDayProductData(List<CargoProductEntity> list, bool isFull = true)
        {
            //日志信息
            LogWrite<CargoProductEntity> lw = new LogWrite<CargoProductEntity>();
            ...
            var mainHouses = list.Select(x => x.ShareHouseID).Distinct().ToArray();
            var shareHouses = list.Select(x => x.HouseID).Distinct().ToArray();
            ...
            try
            {
                ...
                logInfo.Memo = $"次日达前置仓同步成功;" +
                    $"\r\n更新数量：{toUpdateContainerGoods.Count}\r\n新增数量：{toAddProductDict.Count}" +
                    $"\r\n前置仓IDs: {JsonConvert.SerializeObject(mainHouses)}\r\n共享仓IDs: {JsonConvert.SerializeObject(shareHouses)}";
                lw.WriteLog(logInfo);
            }
            catch (Exception ex)
            {
                logInfo.Status = "1";
                logInfo.Memo = $"次日达前置仓同步失败;" +
                    $"\r\n更新数量：{toUpdateContainerGoods.Count}\r\n新增数量：{toAddProductDict.Count}" +
                    $"\r\n前置仓IDs: {JsonConvert.SerializeObject(mainHouses)}\r\n共享仓IDs: {JsonConvert.SerializeObject(shareHouses)}";
                logInfo.Memo += "\r\n" + ex.FormatErr();
                lw.WriteLog(logInfo);
            ...
            using (TransactionScope scope = new TransactionScope(TransactionScopeOption.Required))
            {
                try
                {
                    ...
                    var orderData = bus.QueryOrderDataInfo(new CargoOrderEntity { OpenOrderNo = notice.data.orderId })
                        .FirstOrDefault(a => Convert.ToInt32(a.AwbStatus) > 1);
                    if (orderData == null)
                    {
                        //1.更新开思表状态
        ...
        public int DelTmallOrder(operateNoticeSpiPo entity, LogEntity log)
        {
            CargoOrderBus bus = new CargoOrderBus();
            LogBus logbus = new LogBus();
            var isExistsData = man.QueryStockTMallData(new CargoTMallEntity { outboundNoticeNo = entity.noticeNo });
            if (isExistsData.Count > 0 && isExistsData.FirstOrDefault().AwbStatus < 2)
            {
                log.Status = "0";
                log.Memo = $@"天猫取消订单 [{entity.noticeNo}]天猫数据：" + JsonConvert.SerializeObject(isExistsData);
                ...
                //2.删除系统订单
                var orderList = QueryTMallOrderList(isExistsData.FirstOrDefault().originBillNo);
                log.Status = "0";
                log.Memo = $@"天猫取消订单 [{entity.noticeNo}]系统数据：" + JsonConvert.SerializeObject(orderList);
                logbus.InsertLog(log);
            ...
            else if (isExistsData.Count > 0 && isExistsData.FirstOrDefault().AwbStatus >= 2)
            {
                return 0;
            }
            return -1;
        }
        public Hashtable QueryTMallOrder(int pIndex, int pNum, CargoTMallEntity entity)
        {
            return man.QueryTMallOrder(pIndex, pNum, entity);
        }
        public List<OutboundNoticeDetailSpiPo> QueryTMallOrderGoods(CargoTMallEntity entity)
        {
            return man.QueryTMallOrderGoods(entity);
        }

        public List<CargoOrderEntity> QueryTMallOrderList(string notice)
        {
            return man.QueryTMallOrderList(notice);
        }
        #endregion
    }
}
