<%@ Page Language="C#" Title="即日达订单" AutoEventWireup="true" MasterPageFile="~/Site.Master" CodeBehind="ToDayOrders.aspx.cs" Inherits="Supplier.Order.ToDayOrders" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            var columns = [];
            columns.push({
                title: '订单数量', field: 'Piece', width: '60px', align: 'right', styler: function (val, row, index) { return "color:#12bb1f;font-weight:bold;" }, formatter: function (value,row) {
                    if (row.OrderModel == "1") {
                        return "-" + value;
                    } else { return value; }
                }
            ...
            columns.push({
                title: '总金额', field: 'TotalCharge', width: '90px',align:'right' ,formatter: function (value,row) {
                    if (row.OrderModel == "1") {
                        return "-" + value;
                    } else { return value; }
                }
            ...
            //});
            columns.push({
                title: '送货方式', field: 'DeliveryType', width: '70px',
            ...
            columns.push({
                title: '出库仓库', field: 'HouseName', width: '90px', formatter: function (value) {
                    return "<span title='" + value + "'>" + value + "</span>";
                }
            });
            columns.push({
            ...
            $('#dg').datagrid({
                ...
                onClickRow: function (index, row) {//当用户点击一行时触发
                    ...
                    gridOpts.url = 'orderApi.aspx?method=QueryToDayOrderDetails';
                    $('#orderGoodsDg').datagrid('load', { OrderNo: row.OrderNo });
            ...
            $('#orderGoodsDg').datagrid({
                ...
                columns: [[
                    ...
                    { title: '数量', field: 'Piece', width: '100px' },
                    {
                    ...
                    {
                        title: '实际销售价', field: 'ActSalePrice', width: '100px', formatter: function (value) {
                            return "<span title='" + value + "'>" + value + "</span>";
                        }
                    },
        ...
        function dosearch() {
            ...
            gridOpts.url = 'orderApi.aspx?method=QueryToDayOrdersInfo';
            $('#dg').datagrid('load', {
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <script type="text/javascript">
        ...
        function exportData() {
            ...
            $.ajax({
                url: "orderApi.aspx?method=ExportToDayOrders&OrderNo=" + $("#AOrderNo").val() + "&AcceptPeople=" + $("#AcceptPeople").val() +
        ...
        function exportDataGoods() {
            ...
            $.messager.progress({ msg: '请稍后,正在导出中...' });
            $.ajax({
                url: "orderApi.aspx?method=ExportToDayOrderGoods&OrderNo=" + $("#OrderNo").val(),
                success: function (text) {
                    $.messager.progress("close");
