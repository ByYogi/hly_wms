<%@ Page Title="进仓单明细" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="PurchaseOrderDetails.aspx.cs" Inherits="Supplier.Report.PurchaseOrderDetails" %>
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            columns.push({
                title: '进仓单号', field: 'FacOrderNo', width: '8%', formatter: function (value) {
            ...
            columns.push({
                title: '载速', field: 'LoadIndex', width: '8%', formatter: function (value,row) {
                    return "<span title='" + value + row.SpeedLevel + "'>" + value + row.SpeedLevel + "</span>";
                }
            });
            columns.push({
                title: '批次', field: 'Batch', width: '8%', formatter: function (value) {
                    return "<span title='" + value + "'>" + value + "</span>";
                }
            });
            columns.push({
                title: '进仓数量', field: 'Piece', width: '6%', formatter: function (value) {
                    return "<span title='" + value + "'>" + value + "</span>";
                }
            });
            columns.push({
                title: '入库数量', field: 'InPiece', width: '6%', formatter: function (value) {
                    return "<span title='" + value + "'>" + value + "</span>";
                }
            });
            columns.push({
                title: '成本单价', field: 'InHousePrice', width: '8%', formatter: function (value) {
                    return "<span title='" + value + "'>" + value + "</span>";
                }
            });
            columns.push({
                title: '成本金额', field: 'CostPrices', width: '8%', formatter: function (value, row) {
                    return "<span title='" + (row.InHousePrice * row.Piece).toFixed(2) + "'>" + (row.InHousePrice * row.Piece).toFixed(2) + "</span>";
                }
            });
            columns.push({
                title: '销售单价', field: 'SalePrice', width: '8%', formatter: function (value) {
                    return "<span title='" + value + "'>" + value + "</span>";
                }
            });
        ...
        function dosearch() {
            ...
            gridOpts.url = 'reportApi.aspx?method=QueryPurchaseOrderDetails';
            $('#dg').datagrid('load', {
                FacOrderNo: $('#AFacOrderNo').val(),
    ...
    <div id="saPanel" name="SelectDiv1" class="easyui-panel" title="" data-options="iconCls:'icon-search'" style="width: 100%">
        <table>
            <tr>
                ...
                <td>
                    <input id="AFacOrderNo" class="easyui-textbox" data-options="prompt:'请输入进仓单号'" style="width: 100px" />
                </td>
   ...
   <a href="#" class="easyui-linkbutton" iconcls="icon-add" plain="false" onclick="exportData()">&nbsp;导出&nbsp;</a>&nbsp;&nbsp;
    ...
    <script type="text/javascript">
        ...
        function exportData() {
            ...
            $.ajax({
                url: "reportApi.aspx?method=QueryPurchaseOrderDetailsExport&FacOrderNo=" + $('#AFacOrderNo').val() + "&TrafficType=5" + "&SID=" + $("#ASID").combobox('getValue') + "&Specs=" + $("#ASpecs").val() + "&Figure=" + $("#AFigure").val() + "&Batch=" + $("#ABatch").val() + "&HouseID=" + $("#AHouseID").combobox('getValue') + "&StartDate=" + $('#StartDate').datebox('getValue') + "&EndDate=" + $('#EndDate').datebox('getValue'),
