<%@ Page Language="C#" Title="访问备货明细" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="AccessingStockDetails.aspx.cs" Inherits="Supplier.Report.AccessingStockDetails" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            $('#dg').datagrid({
                ...
                columns: [[
                    ...
                    { title: '访问数量', field: 'AccessCount', width: '80px', },
                    { title: '销售数量', field: 'SaleOrderNum', width: '100px', },
                    { title: '库存数量', field: 'Piece', width: '80px', },
                    { title: '最近入库时间', field: 'InHouseTimeStr', width: '130px', },// formatter: DateTimeFormatter },
                    { title: '库存状况', field: 'StockStatus', width: '90px', },
        ...
        function dosearch() {
            ...
            gridOpts.url = 'reportApi.aspx?method=QueryPageAccessingStockDetails';
            $('#dg').datagrid('load', {
                ProductCode: $('#AProductCode').val(),//产品编码
                SID: $("#ASID").combobox('getValue'),//二级产品 品牌
                HouseID: $("#AHouseID").combobox('getValue'),//仓库ID
                Specs: $('#ASpecs').val(),//规格
                StockStatus: $('#AStockStatus').combobox('getValue'),//库存状态
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <div id="saPanel" name="SelectDiv1" class="easyui-panel" title="" data-options="iconCls:'icon-search'" style="width: 100%">
        <table>
            <tr>
                ...
                <td>
                    <select class="easyui-combobox" id="AStockStatus" style="width: 100px;" panelheight="auto" editable="false">
                        <option value="-1">全部</option>
                        <option value="0">低库存</option>
                        <option value="1">高库存</option>
                    </select>
                </td>
    ...
    <script type="text/javascript">
        ...
        function exportData() {
            ...
            $.ajax({
                url: "reportApi.aspx?method=QueryPageAccessingStockDetailsExport&SID=" + $("#ASID").combobox('getValue') + "&ProductCode=" + $("#AProductCode").val() +
                    "&HouseID=" + $("#AHouseID").combobox('getValue') + "&Specs=" + $('#ASpecs').val() +
                    "&StockStatus=" + $('#AStockStatus').combobox('getValue') +
