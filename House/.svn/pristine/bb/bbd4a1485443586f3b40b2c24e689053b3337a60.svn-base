<%@ Page Title="新增退货单" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="addReturnOrder.aspx.cs" Inherits="Cargo.Order.addReturnOrder" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            $('#dg').datagrid({
                ...
                columns: [[
                    ...
                    {
                        title: '业务员价', field: 'ActSalePrice', width: '60px', align: 'right', formatter: function (value) {
                            return "<span title='" + value + "'>" + value + "</span>";
                        }
                    },
                    {
                    ...
                    {
                        title: '级数', field: 'SpeedLevel', width: '80px', formatter: function (value) {
                            return "<span title='" + value + "'>" + value + "</span>";
                        }
                    },
                    {
                ...
                groupFormatter: function (value, rows) {
                    return value + ' - 总件数：' + rows[0].TotalPiece + ' - 总收入：' + rows[0].TotalCharge + ' 元';
                },
            ...
            $('#dgReturn').datagrid({
                ...
                columns: [[
                    ...
                    {
                        title: '退货数量', field: 'Piece', width: '60px', align: 'right', styler: function (val, row, index) { return "color:#12bb1f;font-weight:bold;" }, formatter: function (value) {
                            return "<span title='" + value + "'>" + value + "</span>";
                        }
                    },
                    {
                        title: '业务员价', field: 'ActSalePrice', width: '60px', formatter: function (value) {
                            return "<span title='" + value + "'>" + value + "</span>";
                        }
                    },
                    {
            ...
            $('#CreateDate').datetimebox('setValue', AllDateTime(datenow));
            ...
            if (RoleCName.indexOf("安泰路斯") >= 0) {
                ...
                $('#AHAID').combobox('reload', url);
                $('#AHAID').combobox('setText', '<%=UserInfor.HeadHouseName%>');
                $('#AHAID').combobox('setValue', '<%=UserInfor.HeadHouseID%>');
                $("#AHAID").combobox("readonly", true);
                $('#AHAID').combobox('textbox').css('background-color', '#e8e8e8');
                $('#ASaleManID').combobox('setValue', '2940');
                $('#ASaleManID').combobox('setText', '林逸展');
                $("#ASaleManID").combobox("readonly", true);
                $('#ASaleManID').combobox('textbox').unbind('focus');
                $('#ASaleManID').combobox('textbox').css('background-color', '#e8e8e8');
                $("#BelongDepart").combobox("readonly", true);
        ...
        function dosearch() {
            ...
            gridOpts.url = 'orderApi.aspx?method=QueryOrderInfoForReturn';
            $('#dg').datagrid('load', {
                ...
                BelongDepart: $("#BelongDepart").combobox('getValue'),//归属部门
                OrderModel: "0"//订单类型
            });
        }
    </script>
    ...
    <div id="saPanel" name="SelectDiv1" class="easyui-panel" title="" data-options="iconCls:'icon-search'" style="width: 100%">
        <table>
            <tr>
                ...
                <td>
                    <input id="Dep" class="easyui-combobox" style="width: 100px;" data-options="valueField:'CityName',textField:'CityName',url:'../systempage/sysService.aspx?method=QueryAllCity',multiple:true" panelheight="auto" />
                </td>--%>
            ...
            <tr>
                ...
                <td>
                    <input id="AHAID" class="easyui-combobox" style="width: 100px;" data-options="valueField:'AreaID',textField:'Name',required:true" panelheight="auto" />
                </td>
                <%--<td style="text-align: right;">到达站:
                </td>
                <td>
                    <input id="Dest" class="easyui-combobox" style="width: 100px;" data-options="valueField:'CityName',textField:'CityName',url:'../systempage/sysService.aspx?method=QueryAllCity'" panelheight="auto" />
                </td>--%>
            </tr>
        </table>
    </div>
        ...
        <form id="fmDep" class="easyui-form" method="post">
            ...
            <table style="width: 100%">
                <tr>
                    ...
                    <td>
                        <input name="Piece" id="APiece" class="easyui-numberbox" data-options="min:0,precision:0"
                            style="width: 60px;" />
                    </td>
                    ...
                    <td>
                        <input name="SaleManName" id="SaleManName" class="easyui-textbox" style="width: 80px;" />
                    </td>
                    <td style="text-align: right;">物流公司:
</td>
<td>
    <input name="LogisID" id="ALogisID" class="easyui-combobox" style="width: 95px;" data-options="valueField:'ID',textField:'LogisticName',url:'../systempage/sysService.aspx?method=QueryAllLogistic'" />
</td>
                </tr>
                <tr>
    ...
    <script type="text/javascript">
        ...
        function saveReturn() {
            ...
            $.messager.confirm('<%= Cargo.Common.GetSystemNameAndVersion()%>', '确定保存？', function (r) {
                if (r) {
                    ...
                    $('#fmDep').form('submit', {
                        url: 'orderApi.aspx?method=saveReturn',
                        ...
                        success: function (msg) {
                            ...
                            if (result.Result) {
                              ...
                              <%--  var dd = result.Message.split('/');
                                $('#ONum').val(dd[0]);
                                $('#OutNum').val(dd[1]);
                                $.messager.confirm('<%= Cargo.Common.GetSystemNameAndVersion()%>', '保存成功，是否打印退货单？', function (r) {
                                    if (r) { prePrint(); }//打印
                                });--%>
                            } else {
        ...
        function dblClickOutCargo(Did) {
           ...
           <%-- if (row == null || row == "") {
            ...
            if (row.Piece == 0) {
                $.messager.alert('<%= Cargo.Common.GetSystemNameAndVersion()%>', '退货件数为0', 'warning');
                return;
            }
            if (row) {
                ...
                $('#ActSalePrice').numberbox('setValue', row.ActSalePrice);
        ...
        function outOK() {
            ...
            var SalePrice = $('#ActSalePrice').numberbox('getValue');//业务员价格 Number(row.SalePrice);//销售价
            ...
            if (index < 0) {
                ...
                $('#dgReturn').datagrid("getPanel").panel("setTitle", title);
                closedgShowData();
            } else { $.messager.alert('<%= Cargo.Common.GetSystemNameAndVersion()%>', '退货列表已存在该产品，请先删除再添加！', 'warning'); }
        }
        //关闭
