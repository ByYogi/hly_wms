...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    ...
    <style type="text/css">
        ...
        div#detailID1, div#detailID2, div#detailID3 {
            margin-top: 26px;
        }
    ...
    <script type="text/javascript">
        $(function () {
            //InitTab();
            //dgHouseAim2 占位 当页面的表格数量超过某个数量后，会导致页面错位，这里弄一个隐藏table
            $('#dgHouseAim2').datagrid({
                width: '0',
                height: '0',
                title: '', //标题内容
                loadMsg: '数据加载中请稍候...',
            ...
            $('#dgOrderDayStatistics').datagrid({
                ...
                columns: [[
                    ...
                    {
                        title: '超期库存', field: 'OverduePiece', width: '60px', align: 'right', sortable: true, formatter: function (val, item) {
                            if (item.TypeName == "footer") return '';
                            return val
                        }
                    },
                    {
                        title: '超期库存占有率', field: 'OverdueProCountRate', width: '120px', align: 'right', sortable: true, formatter: function (val, item) {
                            if (item.TypeName == "footer") return '';
                            return val + "%"
                        }
                    },
                ...
                onDblClickCell: function (index, field, data) {
                    ...
                    if (field == "SalesRate" || field == "ClickConversionRate" || field == "LargeSpecRatio" || field == "LocalSkuOutOfStockRate" || field == "GlobalSkuOosRate")
                        clearDataGridSort('dgDetail')
            ...
            $('#dgDetail').datagrid({
                ...
                columns: [[
                    ...
                    {
                        title: '销量', field: 'Piece', width: '80px', sortable: true, sorter: function (a, b) {
                            return (parseInt(a) > parseInt(b) ? 1 : -1);
                        }
                    },
                    {
                        title: '剩余库存', field: 'ProPiece', width: '80px', sortable: true, sorter: function (a, b) {
                            return (parseInt(a) > parseInt(b) ? 1 : -1);
                        }
                    },
                ]]
            ...
            $('#dgHouseOrderDayStatistics').datagrid({
                ...
                onDblClickCell: function (index, field, data) {
                    if (field == "SalesRate" || field == "ClickConversionRate" || field == "LargeSpecRatio" || field == "LocalSkuOutOfStockRate" || field == "GlobalSkuOosRate") {
                        clearDataGridSort('dgHouseDetail')
                    }
                    if (field == "SalesRate") {
            ...
            $('#dgTypeOrderDayStatistics').datagrid({
                width: '100%',
                height: '400px',
                title: '', //标题内容
                loadMsg: '数据加载中请稍候...',
                ...
                onDblClickCell: function (index, field, data) {
                    if (field == "SalesRate" || field == "ClickConversionRate" || field == "LargeSpecRatio" || field == "LocalSkuOutOfStockRate" || field == "GlobalSkuOosRate") {
                        clearDataGridSort('dgTypeDetail')
                    }
                    if (field == "SalesRate") {
                        var row = $("#dgTypeOrderDayStatistics").datagrid("getRows")[index];
                        GetSalesRateDetail(row, 'alltype')
                    } else if (field == "ClickConversionRate") {
                        var row = $("#dgTypeOrderDayStatistics").datagrid("getRows")[index];
                        GetClickConversionRateDetail(row, 'alltype')
                    } else if (field == "LargeSpecRatio") {
                        var row = $("#dgTypeOrderDayStatistics").datagrid("getRows")[index];
                        GetLargeSpecRatioDetail(row, 'alltype')
                    } else if (field == "LocalSkuOutOfStockRate") {
                        var row = $("#dgTypeOrderDayStatistics").datagrid("getRows")[index];
                        GetLocalSkuOutOfStockRate(row, 'alltype')
                    } else if (field == "GlobalSkuOosRate") {
                        var row = $("#dgTypeOrderDayStatistics").datagrid("getRows")[index];
                        GetGlobalSkuOosRate(row, 'alltype')
                    }

                },
            });
            $('#dgTypeDetail').datagrid({
                width: '100%',
                height: '400px',
                title: '', //标题内容
                loadMsg: '数据加载中请稍候...',
            ...
            $('#dgBrowseView').datagrid({
                ...
                remoteSort: false,
                columns: [[
                    { title: '产品编码', field: 'ProductCode', width: '100px', sortable: true },
                    { title: '品牌', field: 'TypeName', width: '80px', sortable: true },
                    { title: '规格', field: 'Specs', width: '80px', sortable: true },
                    { title: '花纹', field: 'Figure', width: '100px', sortable: true },
                    {
                        title: '载速', field: 'LoadIndex', width: '70px', sortable: true, formatter: function (value, row) {
                            if (row.CompanyName == "汇总：") return '';
                            return row.LoadIndex + row.SpeedLevel;
                        }
                    },
                    //{ title: '门店名称', field: 'CompanyName', width: '100px' },
                    { title: '点击数（浏览量）', field: 'clickCount', width: '80px', sortable: true },
                    { title: '下单数', field: 'UniqueClientCount', width: '80px', sortable: true },
                    { title: '销量', field: 'UniqueClientNum', width: '80px', sortable: true },
                    {
                        title: '点击转化率', field: 'ClickConversionRate', width: '80px', sortable: true, formatter: function (val, item) {
                            return val + "%"
                        },
                        styler: function (value, row, index) {
                            return 'background-color:#ffee00;color:red;';
                ...
                onClickRow: function (index, row) { },
                onDblClickCell: function (index, field, data) {
                    if (field == "ClickConversionRate") {
                        clearDataGridSort('dgTypeBrowseDetail')
                    }
                    var row = $("#dgBrowseView").datagrid("getRows")[index];
                    if (field == "ClickConversionRate") {
                        GetClickConversionRateDetailV2(row, 'Browsetype')
                    }
                },
            });

            $('#dgTypeBrowseDetail').datagrid({
                width: '100%',
                height: '400px',
                title: '', //标题内容
                loadMsg: '数据加载中请稍候...',
                ...
                columns: [[
                    ...
                    { title: '产品编码', field: 'ProductCode', width: '110px' },
                    { title: '规格', field: 'Specs', width: '80px' },
                    { title: '门店名称', field: 'CompanyName', width: '80px' },
                    { title: '联系电话', field: 'Cellphone', width: '80px' },
                    {
                        title: '点击数（浏览量）', field: 'clickCount', width: '80px', sortable: true, sorter: function (a, b) {
                            return (parseInt(a) > parseInt(b) ? 1 : -1);
                        }
                    },
                    {
                        title: '下单数', field: 'UniqueClientCount', width: '80px', sortable: true, sorter: function (a, b) {
                            return (parseInt(a) > parseInt(b) ? 1 : -1);
                        }
                    },
                    {
                        title: '下单销量', field: 'UniqueClientNum', width: '80px', sortable: true, sorter: function (a, b) {
                            return (parseInt(a) > parseInt(b) ? 1 : -1);
                        }
                    },
                ]]
            ...
            $('#dgUserStatistics').datagrid({
                ...
                onClickRow: function (index, row) { },
                onDblClickCell: function (index, field, data) {
                    if (field == "AllUserCount") {
                        clearDataGridSort('dgUserDetail')
                    }
                    var row = $("#dgUserStatistics").datagrid("getRows")[index];
                    if (field == "AllUserCount") {
                        GetAllUserCount(row)
                    }
                },
            ...
            $('#dgUserDetail').datagrid({
                ...
                remoteSort: false,
                columns: [[
                    { title: '用户名称', field: 'UserName', width: '70px' },
                    { title: '联系电话', field: 'Cellphone', width: '110px' },
                    { title: '门店名称', field: 'CompanyName', width: '80px' },
                    {
                        title: '下单数', field: 'UniqueClientCount', width: '80px', sortable: true, sorter: function (a, b) {
                            return (parseInt(a) > parseInt(b) ? 1 : -1);
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <div style="width: 100%; height: 100%;" id="ChartBox">
        <div style="width: 100%; display: flex; align-items: center;">
            <div style="width: 28%;" id="pn73" data-options="tools:'#tl1'" class="easyui-panel" title="月销售统计">

                <table id="dgHouseAim2" style="display: none !important;" class="easyui-datagrid"></table>
                <table id="dgHouseAim" class="easyui-datagrid"></table>
    ...
    <div style="width: 100%; display: flex;">
                ...
                <tr>
                    ...
                    <td style="position: relative;"><span href="#" class="easyui-linkbutton" iconcls="icon-application_put" plain="false" onclick="DayStatisticsExport()" id="btnExport">&nbsp;导&nbsp;出&nbsp;
                             ...
                             <form runat="server" id="Form2">
                                 ...
                                 <asp:Button ID="btnDerived5" runat="server" Style="display: none;" Text="导出" OnClick="btnDerived5_Click" />
                                 <asp:Button ID="btnDerived6" runat="server" Style="display: none;" Text="导出" OnClick="btnDerived6_Click" />
                             </form>
        ...
        <div style="width: 38%;" class="easyui-panel" title="品牌月销售-详情">
            <table>
                <tr>
                    <td style="position: relative;">
                        <span href="#" class="easyui-linkbutton" iconcls="icon-application_put" plain="false" onclick="exportEasyUIToCSV('dgDetail')" id="btnDetailExport">&nbsp;导&nbsp;出&nbsp;</span>
                    </td>
                </tr>
            </table>
    ...
    <div style="width: 100%; display: flex;">
        <div style="width: 62%;" id="pn78" class="easyui-panel" title="仓库月销售排名统计数">
        ...
        <div style="width: 38%;" id="pn78" class="easyui-panel" title="仓库月销售-详情">
            <table>
                <tr>
                    <td style="position: relative;">
                        <span href="#" class="easyui-linkbutton" iconcls="icon-application_put" plain="false" onclick="exportEasyUIToCSV('dgHouseDetail')" id="btnDetailExport">&nbsp;导&nbsp;出&nbsp;</span>
                    </td>
                </tr>
            </table>
    ...
    <div style="width: 100%; display: flex;">
        <div style="width: 62%;" id="pn77" class="easyui-panel" title="全网品牌销售列表">
            <table id="saPanelTabType">
                <tr>
                    <td style="text-align: right;">日期:
                    </td>
                    <td>
                        <input id="TypeV2StartDate" class="easyui-datebox" style="width: 100px" />~<input id="TypeV2EndDate" class="easyui-datebox" style="width: 100px" />
                    </td>

                    <td><span href="#" class="easyui-linkbutton" iconcls="icon-search" plain="false" onclick="DoSearchAllType()">&nbsp;查&nbsp;询&nbsp;</span>&nbsp;&nbsp;</td>
                    <td style="position: relative;"><span href="#" class="easyui-linkbutton" iconcls="icon-application_put" plain="false" onclick="AllTypeExport()" id="btnExport">&nbsp;导&nbsp;出&nbsp;
                    </span>&nbsp;&nbsp;
                        <i class="iTitle">双击特殊列查看详情</i>
                    </td>
                </tr>
            </table>
            <table id="dgTypeOrderDayStatistics" class="easyui-datagrid">
            </table>
        </div>
        <div style="width: 38%;" class="easyui-panel" title="全网品牌销售-详情">
            <table>
                <tr>
                    <td style="position: relative;">
                        <span href="#" class="easyui-linkbutton" iconcls="icon-application_put" plain="false" onclick="exportEasyUIToCSV('dgTypeDetail')" id="btnDetailExport">&nbsp;导&nbsp;出&nbsp;</span>
                    </td>
                </tr>
            </table>
            <table id="dgTypeDetail" class="easyui-datagrid"></table>
        </div>

    </div>

    <div style="width: 100%; display: flex;">
        <div style="width: 62%;" class="easyui-panel" title="品牌浏览数据">
            <table>
                <tr>
                    <td style="text-align: right;">日期:
                    </td>
                    <td>
                        <input id="TypeBrowseStartDate" class="easyui-datebox" style="width: 100px" />~<input id="TypeBrowseEndDate" class="easyui-datebox" style="width: 100px" />
                    </td>
                    <td style="text-align: right;">仓库:
                    </td>

                    <td>
                        <input id="TypeHouseID" name="TypeHouseID" class="easyui-combobox" style="width: 100px;" data-options="valueField:'HouseID',textField:'Name',editable:true" />
                    </td>
                    <td style="text-align: right;">品牌:
                    </td>
                    <td>
                        <input id="BrowseTypeID" name="BrowseTypeID" class="easyui-combobox" style="width: 100px;" data-options="valueField:'TypeID',textField:'TypeName',editable:true" />
                    </td>
                    <td><span href="#" class="easyui-linkbutton" iconcls="icon-search" plain="false" onclick="DoSearchTypeBrowse()">&nbsp;查&nbsp;询&nbsp;</span>&nbsp;&nbsp;</td>
                    <td style="position: relative;"><span href="#" class="easyui-linkbutton" iconcls="icon-application_put" plain="false" onclick="TypeBrowseExport()" id="TypeBrowseExport" />
                        &nbsp;导&nbsp;出&nbsp;</td>
                </tr>
            </table>
            <table id="dgBrowseView" class="easyui-datagrid">
            </table>

        </div>

        <div style="width: 38%;" class="easyui-panel" title="品牌浏览数据-详情">
            <table>
                <tr>
                    <td style="position: relative;">
                        <span href="#" class="easyui-linkbutton" iconcls="icon-application_put" plain="false" onclick="exportEasyUIToCSV('dgTypeBrowseDetail')" id="btnBrowseDetailExport">&nbsp;导&nbsp;出&nbsp;</span>
                    </td>
                </tr>
            </table>
            <table id="dgTypeBrowseDetail" class="easyui-datagrid"></table>
        </div>
    </div>

    <div style="width: 100%; display: flex;">
        <div style="width: 62%;" class="easyui-panel" title="用户统计数">
        ...
        <div style="width: 38%;" id="UserDetailID" class="easyui-panel" title="用户统计数-详情">
            <table>
                <tr>
                    <td style="position: relative;">
                        <span href="#" class="easyui-linkbutton" iconcls="icon-application_put" plain="false" onclick="exportEasyUIToCSV('dgUserDetail')" id="btnBrowseDetailExport2">&nbsp;导&nbsp;出&nbsp;</span>
                    </td>
                </tr>
            </table>
            <table id="dgUserDetail" class="easyui-datagrid"></table>
        </div>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $('#TypeV2StartDate').datebox('setValue', getNowFormatDate(datenow.getFullYear().toString() + "-" + (datenow.getMonth() + 1).toString() + "-01"));
            $('#TypeV2EndDate').datebox('setValue', getNowFormatDate(datenow));

            $('#TypeBrowseStartDate').datebox('setValue', getNowFormatDate(datenow.getFullYear().toString() + "-" + (datenow.getMonth() + 1).toString() + "-01"));
            $('#TypeBrowseEndDate').datebox('setValue', getNowFormatDate(datenow));
        ...
        function TypeBrowseExport() {
            var StartDate = $('#TypeBrowseStartDate').datebox('getValue')
...
<%--            if (houseid == "" || houseid == null) {
            ...
            $.ajax({
                url: "huiCaiApi.aspx?method=DayStatisticsExportV6",
                type: 'post', data: key,
                success: function (text) {
                    console.log(text)
                    if (text == "OK") {
                        var obj = document.getElementById("<%=btnDerived6.ClientID %>");
                        console.log(obj)
                        obj.click();
                    }
                    else { $.messager.alert('<%= Cargo.Common.GetSystemNameAndVersion()%>', text, 'warning'); }
        ...
        function AllTypeExport() {
            var key = {
                StartDate: $('#TypeV2StartDate').datebox('getValue'),
                EndDate: $('#TypeV2EndDate').datebox('getValue'),
                OrderType: $('#OrderType').combobox('getValue'),
                typeStr: "1",
            }
            ...
            $.ajax({
                ...
                success: function (text) {
                    console.log(text)
                    if (text == "OK") {
                        var obj = document.getElementById("<%=btnDerived5.ClientID %>");
                        obj.click();
                    }
                    else { $.messager.alert('<%= Cargo.Common.GetSystemNameAndVersion()%>', text, 'warning'); }
                }
        ...
        function clearDataGridSort(dgId) {
            var dg = $('#' + dgId);
            ...
            $.ajax({
                ...
                success: function (text) {
                    ...
                    if (type == 'alltype') {
                        ...
                        $('#dgTypeDetail').datagrid('loadData', dataList);

                        $("#detailID3").parent().find(".panel-header .panel-title").text('动销率-' + row.TypeName)
                    }
                    else if (type == 'house') {
            ...
            $.ajax({
                ...
                success: function (text) {
                    ...
                    if (type == 'alltype') {
                        ...
                        $("#detailID3").parent().find(".panel-header .panel-title").text('点击转换率-' + row.TypeName)
                    }
                    else if (type == 'house') {
                        $('#dgHouseDetail').datagrid({
                            columns: [[
        ...
        function GetClickConversionRateDetailV2(row, type) {
            showLoading()
            var StartDate = $('#TypeStartDate').datebox('getValue')
            var EndDate = $('#TypeEndDate').datebox('getValue')
            var HouseID = $('#TypeHouseID').combobox('getValue')
            ...
            $.ajax({
                url: "huiCaiApi.aspx?method=GetClickConversionRateDetailV2",
                type: 'post', data: {
                    HouseID: HouseID,
                    TypeID: row.TypeID,
                ...
                success: function (text) {
                    ...
                    if (type == 'Browsetype') {
                        ...
                        $('#dgTypeBrowseDetail').datagrid('loadData', dataList);
                    ...
                    if (type == 'alltype') {
                        $('#dgTypeDetail').datagrid({
                            columns: [[
                                ...
                                { title: '规格', field: 'Specs', width: '80px' },
                                {
                                    title: '大规格数', field: 'BigSizeCount', width: '80px', sortable: true, sorter: function (a, b) {
                                        return (parseInt(a) > parseInt(b) ? 1 : -1);
                                    }
                                },
                            ]]
                        ...
                        $("#detailID3").parent().find(".panel-header .panel-title").text('大规格销量率-' + row.TypeName)
                    }
                    else if (type == 'house') {
                        $('#dgHouseDetail').datagrid({
                            columns: [[
            ...
            $.ajax({
                ...
                success: function (text) {
                    ...
                    var columns = [[
                        ...
                        { title: '规格', field: 'Specs', width: '80px' },
                        {
                            title: '库存数', field: 'GlobalProPiece', width: '80px', sortable: true, sorter: function (a, b) {
                                return (parseInt(a) > parseInt(b) ? 1 : -1);
                            }
                        },
                    ]];
                    if (type == 'alltype') {
                        ...
                        $("#detailID3").parent().find(".panel-header .panel-title").text('本仓有售SKU缺货率-' + row.TypeName)
                    }
                    else if (type == 'house') {
                        $('#dgHouseDetail').datagrid({
                            columns: columns
            ...
            $.ajax({
                ...
                success: function (text) {
                    ...
                    if (type == 'alltype') {
                        ...
                        $("#detailID3").parent().find(".panel-header .panel-title").text('全网有售SKU缺货率-' + row.TypeName)
                    }
                    else if (type == 'house') {
                        $('#dgHouseDetail').datagrid({
                            columns: columns
        ...
        function GetAllUserCount(row) {
            showLoading()
            var StartDate = $('#UserStartDate').datebox('getValue')
            var EndDate = $('#UserEndDate').datebox('getValue')
            $.ajax({
                url: "huiCaiApi.aspx?method=GetAllUserCount",
                type: 'post', data: {
                    StartDate: StartDate,
                    EndDate: EndDate,
                    HouseID: row.HouseID,
                ...
                success: function (text) {
                    ...
                    $('#dgUserDetail').datagrid('loadData', dataList);

                    $("#UserDetailID").parent().find(".panel-header .panel-title").text('用户统计数-' + row.HouseName)
                }
            });
        }
        ...
        function DoSearchAllType() {
            var StartDate = $('#TypeV2StartDate').datebox('getValue')
            var EndDate = $('#TypeV2EndDate').datebox('getValue')
            var time1 = new Date(StartDate)
            var time2 = new Date(EndDate)
            if (time1 > time2) {
                $.messager.alert('<%= Cargo.Common.GetSystemNameAndVersion()%>', '开始日期不能大于结束日期！', 'warning'); return;
        ...
        function GetAllTypeData() {
            ...
            $.ajax({
                ...
                success: function (dt) {
                    hideLoading();
                    var data = JSON.parse(dt);
                    $('#dgTypeOrderDayStatistics').datagrid('loadData', data);
                }
            });
        }
      ...
      <%--      if (houseid == "" || houseid == null) {
            ...
            $.ajax({
                type: 'post',
                url: "huiCaiApi.aspx?method=QueryHCYC_TypeBrowseData",
                data: {
                    StartDate: $('#TypeBrowseStartDate').datebox('getValue'),
                    EndDate: $('#TypeBrowseEndDate').datebox('getValue'),
                    OrderType: $('#OrderType').combobox('getValue'),
                    HouseID: houseid,
                    TypeID: typeid
                },
                success: function (dt) {
                    hideLoading();
                    var data = JSON.parse(dt);
                    $('#dgBrowseView').datagrid('loadData', data);
                }
            });
        }
