...
namespace HouseServices.Common
{
    public class ToExcel
    {
        ...
        /// <summary>
        /// 将 DataTable 数据导出为 Excel 文件并保存到本地
        /// </summary>
        /// <param name="dt">要导出的 DataTable</param>
        /// <param name="filePath">保存 Excel 文件的完整路径（例如："C:\\Data\\MyData.xlsx"）</param>
        public static void DataTableToExcel(DataTable dt, string filePath)
        {
            // 创建工作簿对象，XSSFWorkbook 用于 .xlsx 格式
            HSSFWorkbook workbook = new HSSFWorkbook();

            // 创建工作表
            HSSFSheet sheet = workbook.CreateSheet("Sheet1");

            // 写入列头（即 DataTable 的列名）
            HSSFRow headerRow = sheet.CreateRow(0); // 第一行是标题行
            for (int i = 0; i < dt.Columns.Count; i++)
            {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                HSSFRow dataRow = sheet.CreateRow(i + 1); // 数据从第二行开始 (i+1)
                for (int j = 0; j < dt.Columns.Count; j++)
                {
                    // 将单元格值设置为字符串（可以根据需要进行更复杂的类型转换）
            ...
            try
            {
                ...
                {
                    workbook.Write(fs); // 将工作簿内容写入文件流
                }
                // 确保释放资源
                //workbook.Close();
                //System.Console.WriteLine($"Excel 文件已成功保存到: {filePath}");
            }
            catch (Exception ex)
            {
                //System.Console.WriteLine($"保存 Excel 文件时出错: {ex.Message}");
                // 可以在这里添加更详细的错误处理
            }
        }
    }
