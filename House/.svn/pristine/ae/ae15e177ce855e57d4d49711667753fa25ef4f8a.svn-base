    ...
    public class CargoClientManager
    {
        ...
        public Hashtable QueryCargoClient(int pIndex, int pNum, CargoClientEntity entity)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(entity.BizUserId)) { strSQL += " and a.BizUserId like '%" + entity.BizUserId + "%'"; }
                    ...
                    using (DataTable dd = conn.ExecuteDataTable(command))
                    {
                        foreach (DataRow idr in dd.Rows)
                        {
                            result.Add(new CargoClientEntity
                            {
                                ...
                                MulAddressOrder = string.IsNullOrEmpty(Convert.ToString(idr["MulAddressOrder"])) ? 0 : Convert.ToInt32(idr["MulAddressOrder"]),
                ...
                if (!string.IsNullOrEmpty(entity.CheckOutType)) { strCount += " and a.CheckOutType = '" + entity.CheckOutType + "'"; }
        ...
        public void AddCargoClient(CargoClientEntity entity)
        {
            ...
            string strSQL = @"INSERT INTO Tbl_Cargo_Client(ClientNum,ClientName,ClientShortName,ClientType,Address,ZipCode,Telephone,Fax,Boss,Cellphone,Email,Product,OP_DATE,Remark,DelFlag,ClientTypeID,ClientTypeName,City,Province,Country,HouseID,UserID,UserName,LimitMoney,TargetNum,ShopCode,TryeClientCode,TryeCompany,TryeClientType,LogisID,LogisName,LogisFee,PushAccount,BookCheck,AddData,CheckOutType,BusUserID,BusUserName,Discount,UpClientID,UpClientShortName,SettleHouseID,SettleHouseName,GroundPushName,ArrivePayLimit,IsBuy,NoType,BizUserId,MulAddressOrder) VALUES (@ClientNum,@ClientName,@ClientShortName,@ClientType,@Address,@ZipCode,@Telephone,@Fax,@Boss,@Cellphone,@Email,@Product,@OP_DATE,@Remark,@DelFlag,@ClientTypeID,@ClientTypeName,@City,@Province,@Country,@HouseID,@UserID,@UserName,@LimitMoney,@TargetNum,@ShopCode,@TryeClientCode,@TryeCompany,@TryeClientType,@LogisID,@LogisName,@LogisFee,@PushAccount,@BookCheck,@AddData,@CheckOutType,@BusUserID,@BusUserName,@Discount,@UpClientID,@UpClientShortName,@SettleHouseID,@SettleHouseName,@GroundPushName,@ArrivePayLimit,@IsBuy,@NoType,@BizUserId,@MulAddressOrder)  SELECT @@IDENTITY";
            try
            {
                entity.EnSafe();
                using (DbCommand cmd = conn.GetSqlStringCommond(strSQL))
                {
                    ...
                    conn.AddInParameter(cmd, "@IsBuy", DbType.Int32, entity.IsBuy);
                    conn.AddInParameter(cmd, "@NoType", DbType.String, entity.NoType);
                    conn.AddInParameter(cmd, "@BizUserId", DbType.String, entity.BizUserId);
                    conn.AddInParameter(cmd, "@MulAddressOrder", DbType.Int32, entity.MulAddressOrder);
        ...
        public List<CargoUpClientEntity> QueryBusinessID(int ClientID)
        {
            ...
            try
            {
                string strSQL = @" SELECT a.ID,a.DepName,a.MarketType FROM Tbl_Cargo_UpClientDep as a inner join Tbl_Cargo_Client as b on a.UpClientID=b.UpClientID WHERE ID>11 ";
                if (ClientID != 0)
                {
        ...
        public List<CargoUpClientEntity> QueryBusinessIDDefault(CargoUpClientEntity entity)
        {
            ...
            try
            {
                string strSQL = "SELECT ID,DepName,MarketType FROM Tbl_Cargo_UpClientDep WHERE ID>11";
        ...
        public List<CargoClientAcceptAddressEntity> QueryAcceptAddress(CargoClientAcceptAddressEntity entity)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(entity.ClientNum.ToString()) && entity.ClientNum != -1)
                {
                    strSQL += " and ClientNum=" + entity.ClientNum;
                }
        ...
        public Hashtable QueryPreRecord(int pIndex, int pNum, CargoClientPreRecordEntity entity)
        {
            ...
            try
            {
                ...
                //动态余额计算
                // 确保列表按时间顺序排列，这样动态余额才有意义
                var orderedTransactions = result.OrderBy(t => t.OP_DATE).ToList();
                ...
                foreach (var transaction in orderedTransactions)
                {
                    ...
                    {
                        currentBalance += (transaction.Money * (calculate == "0" ? 1 : -1));
                    }

                    // 将累加后的结果赋给当前记录的 RunningBalance 字段
                    transaction.DynamicBalance = currentBalance;
                }
        ...
        private string GetRecordTypeStr(string RecordType)
        {
            var result = string.Empty;
            switch (RecordType)
            {
                case "0": result = "收入"; break;
                case "1": result = "支出"; break;
                case "2": result = "退收"; break;
                default:
                    result = "无法识别";
                    break;
            }
        ...
        /// <param name="entity"></param>
        /// <returns></returns>
        public Hashtable GetClientPreRecordList(int pIndex, int pNum, CargoClientPreRecordEntity entity)
        {
            Hashtable resHT = new Hashtable();
            List<CargoClientPreRecordEntity> result = new List<CargoClientPreRecordEntity>();
            try
            {
                ...
                string strCount = $@" select Count(*) as TotalCount From ( select ClientID,TryeClientCode,ExID,(case when Remark='预付款收入' then 2 else RecordType end) as RecordType,OperaType,ISNULL(TypeID,0) as TypeID,RebateDate,OP_DATE, Sum(case rebatetype when '1' then Money else 0 end) as 'YB', Sum(case rebatetype when '2' then Money else 0 end) as 'BT', Sum(case rebatetype when '3' then Money else 0 end) as 'HP', Sum(case rebatetype when '4' then Money else 0 end) as 'ROSS', Sum(case rebatetype when '5' then Money else 0 end) as 'MT', Sum(case rebatetype when '6' then Money else 0 end) as 'SB',Sum(case rebatetype when '0' then Money else 0 end) as 'BX',REPLACE(REPLACE(stuff( (select ',' + Remark from Tbl_Cargo_ClientPreRecord where ClientID = cp.ClientID and TryeClientCode=cp.TryeClientCode and ExID=cp.ExID and RecordType=cp.RecordType and OperaType=cp.OperaType and TypeID=cp.TypeID and RebateDate=cp.RebateDate for xml path('')) ,1,1,''),',,',','),',,',',') as Remark From Tbl_Cargo_ClientPreRecord cp group by ClientID,TryeClientCode,ExID,RecordType,OperaType,TypeID,RebateDate,OP_DATE, Remark ) as a inner join Tbl_Cargo_Client as b on a.ClientID=b.ClientID  WHERE (1=1)";
                if (!entity.ClientID.Equals(0)) { strCount += " and a.ClientID = " + entity.ClientID; }
                //if (!entity.CargoPermisID.Equals(0)) { strSQL += " and b.HouseID in ('" + entity.CargoPermisID + "')"; }
                if (!entity.ClientNum.Equals(0)) { strCount += " and b.ClientNum = " + entity.ClientNum; }
                using (DbCommand cmd = conn.GetSqlStringCommond(strCount))
        ...
        /// <param name="entity"></param>
        /// <exception cref="ApplicationException"></exception>
        public void UpdateCargoClientPreReceiveMoney(CargoClientEntity entity)
        {
            try
            {
                entity.EnSafe();
                ...
                {
                    strSQL = $@"
                    update a set PreReceiveMoney=b.totalMoney
                    from Tbl_Cargo_Client as a
                    inner join (
                        select ClientID,sum((case when RecordType=0 then Money else -Money end)) totalMoney from Tbl_Cargo_ClientPreRecord
        ...
        public void AddUpClientDep(CargoUpClientEntity entity)
        {
            ...
            string strSQL = @"INSERT INTO tbl_Cargo_UpClientDep(UpClientID,DepName,SmallStockCheckFee,BigStockCheckFee,OP_ID,OP_Name,OP_DATE,MarketType) VALUES (@UpClientID,@DepName,@SmallStockCheckFee,@BigStockCheckFee,@OP_ID,@OP_Name,@OP_DATE,@MarketType)  SELECT @@IDENTITY";
            try
            {
                entity.EnSafe();
                using (DbCommand cmd = conn.GetSqlStringCommond(strSQL))
                {
                    ...
                    conn.AddInParameter(cmd, "@OP_DATE", DbType.DateTime, DateTime.Now);
                    conn.AddInParameter(cmd, "@MarketType", DbType.String, entity.MarketType);
        ...
        {
            string strSQL = @"UPDATE tbl_Cargo_UpClientDep SET UpClientID = @UpClientID,DepName = @DepName,SmallStockCheckFee = @SmallStockCheckFee,BigStockCheckFee = @BigStockCheckFee,OP_ID = @OP_ID,OP_Name = @OP_Name,OP_DATE = @OP_DATE,MarketType=@MarketType WHERE ID=@ID";
            try
            {
                entity.EnSafe();
                using (DbCommand cmd = conn.GetSqlStringCommond(strSQL))
...
(AccountNO,OrderNo,ProductID,Total,InsuranceFee,TransitFee,DeliveryFee,OverDueFee,OutStorageFee,StoReleaseFee,OrderModel,OtherFee,OtherExpensesFee,OpenOrderNo)
VALUES(@AccountNO,@OrderNo,@ProductID,@Total,@InsuranceFee,@TransitFee,@DeliveryFee,@OverDueFee,@OutStorageFee,@StoReleaseFee,@OrderModel,@OtherFee,@OtherExpensesFee,@OpenOrderNo)";
            try
            {
                entity.EnSafe();
                using (DbCommand cmd = conn.GetSqlStringCommond(strSQL))
        ...
        public void UpdateSuppAccountData(CargoSuppClientAccountGoodsEntity entity)
        {
            ...
            {
                conn.AddInParameter(cmd, "@OtherExpensesFee", DbType.Decimal, entity.OtherExpensesFee);
                conn.AddInParameter(cmd, "@Memo", DbType.String, string.IsNullOrEmpty(entity.Memo) ? "" : "【" + entity.Memo + "】");
                conn.AddInParameter(cmd, "@AccountNO", DbType.String, entity.AccountNO);
                conn.ExecuteNonQuery(cmd);
            }
        }
        ...
        public void UpdateIsSupplierTypeOrder(CargoSuppClientAccountGoodsEntity entity)
        {
            ...
            string strSQL = $@"UPDATE Tbl_Cargo_Order SET IsSupplierType = @IsSupplierType WHERE OrderNo IN ({inClauseParams})";
