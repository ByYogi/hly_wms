...
namespace House.Manager.Cargo
{
    ...
    public class CargoPriceManager
    {
        ...
        public List<CargoProductEntity> QueryInCargoProductInfo(CargoProductEntity showEnt)
        {
            ...
            try
            {
                string strSQL = @"select b.TypeName,a.GoodsCode from Tbl_Cargo_Product as a left join Tbl_Cargo_ProductType as b on a.TypeID=b.TypeID left join Tbl_Cargo_InContainerGoods as c on a.ProductID=c.ProductID left join Tbl_Cargo_Container as d on c.ContainerID=d.ContainerID left join Tbl_Cargo_Area as e on d.AreaID=e.AreaID left join Tbl_Cargo_ContainerGoods f on a.ProductID=f.ProductID ";
                if (!showEnt.FirstAreaID.Equals(0))
                {
                    strSQL += " left join Tbl_Cargo_Area as h on e.ParentID=h.AreaID left join Tbl_Cargo_Area as g on h.ParentID=g.AreaID ";
                }
                strSQL += " where (1=1) ";
                if (!showEnt.FirstAreaID.Equals(0))
                {
                ...
                if (showEnt.AnomalousPrice && showEnt.AnomalousUnitPrice)
                {
                    ...
                    strSQL += " and (a.TradePrice<=0 or (a.UnitPrice)<=0)";
                }
                else
                {
                    if (showEnt.AnomalousPrice) { strSQL += " and a.TradePrice<=0"; }
                    if (showEnt.AnomalousUnitPrice) { strSQL += " and (a.UnitPrice)<=0"; }
                }
        ...
        public List<CargoProductEntity> QueryInHouseProduct(CargoProductEntity entity)
        {
            ...
            try
            {
                ...
                strSQL += " FROM Tbl_Cargo_Product as a left join Tbl_Cargo_ProductType as b on a.TypeID=b.TypeID left join Tbl_Cargo_ContainerGoods as d on a.ProductID=d.ProductID left join Tbl_Cargo_Container as e on d.ContainerID=e.ContainerID left join Tbl_Cargo_Area as g on e.AreaID=g.AreaID left join Tbl_Cargo_House h on a.HouseID=h.HouseID";
                strSQL += " left join Tbl_Cargo_Area as j on g.ParentID=j.AreaID left join Tbl_Cargo_Area as k on j.ParentID=k.AreaID ";
                if (entity.CargoPermisID == "64") { strSQL += " left join Tbl_Cargo_PurchaseOrder po on po.FacOrderNo=a.SourceOrderNo "; }
                strSQL += " left join Tbl_Cargo_InContainerGoods as f on a.ProductID=f.ProductID left join Tbl_Cargo_ProductSource ps on a.Source=ps.Source WHERE (1=1) ";
                ...
                {
                    if (entity.AnomalousPrice) { strSQL += " and (a.TradePrice)<=0 "; }
                    if (entity.AnomalousUnitPrice) { strSQL += " and (a.UnitPrice)<=0"; }
                }
                //所属仓库ID
                    ...
                    using (DataTable dd = conn.ExecuteDataTable(command))
                    {
                        foreach (DataRow idr in dd.Rows)
                        {
                            ...
                            result.Add(new CargoProductEntity
                            {
                                ...
                                SpecsType = Convert.ToString(idr["SpecsType"]).Trim(),
        ...
        /// </summary>
        /// <param name="entity"></param>
        public void UpdateProductNextDayPrice(CargoProductEntity entity)
        {
            string strSQL = @"UPDATE Tbl_Cargo_Product set NextDayPrice=@NextDayPrice where ProductID=@ProductID";
            try
            {
                using (DbCommand cmd = conn.GetSqlStringCommond(strSQL))
                {
