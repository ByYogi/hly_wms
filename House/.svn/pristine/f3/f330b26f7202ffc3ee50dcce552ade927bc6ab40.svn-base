<%@ Page Language="C#" Title="修改密码" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="UpdatePassword.aspx.cs" Inherits="Supplier.systempage.UpdatePassword" %>
        ...
        <div id="saPanel" class="easyui-panel" title="修改密码" data-options="iconCls:'icon-eye',collapsible:true,fit:true"
            ...
            <table>
                <tr>
                    ...
                    <td>
                        <input id="LoginName" name="LoginName" class="easyui-textbox" data-options="prompt:'请输入用户登陆名',required:true,iconCls:'icon-man',readonly:true"
        ...
        function saveItem() {
            $('#fm').form('submit', {
                ...
                success: function (msg) {
                    var result = eval('(' + msg + ')');
                    if (result.Result == true) { $.messager.alert('<%= Supplier.Common.GetSystemNameAndVersion()%>', '修改密码成功，请妥善保存密码!', 'info'); } else {
                        $.messager.alert('<%= Supplier.Common.GetSystemNameAndVersion()%>', '修改密码失败：' + result.Message, 'warning');
                    }
                }
            })
        }
