<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="OrderInfoManager.aspx.cs" Inherits="Cargo.Weixin.OrderInfoManager" %>
...
<body>
    ...
    <script type="text/javascript">
        ...
        function MakeSureOrder(no) {
            $.confirm("您确定要确认此订单吗?", "确认订单?", function () {
                $.ajax({
                    url: 'myAPI.aspx?method=MakeSureOrder',
                    type: 'post', dataType: 'json', data: { data: no },
                    success: function (text) {
                        if (text.Result == true) {
                            $.alert("订单确认成功", "操作提示", function () {
                                WeixinJSBridge.call('closeWindow');
                            });
                        }
                        else {
                            $.alert('确认失败 失败原因：' + text.Message);
                        }
                    }
                });
            }, function () {
