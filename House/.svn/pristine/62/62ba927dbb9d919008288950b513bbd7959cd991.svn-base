...
";
        ...
        public List<CargoProductEntity> QueryProductByHouseIDsAndCodes(IEnumerable<int> houseIDs, IEnumerable<string> productCodes)
        {
            ...
            // hzj: 这里临时表改为用 唯一名真实表。防止BulkInsertData就算用全局临时表也访问不到的问题。SqlHelper类不让改动，只能这样变通处理。
            // 生成唯一表名
            string sessionID = System.Diagnostics.Process.GetCurrentProcess().Id.ToString(); //获取当前进程 ID（PID）
            string timestamp = DateTime.Now.Ticks.ToString();
            string tableName = $"Tbl_Temp_ProductQuery_{sessionID}_{timestamp}";
...
CREATE TABLE dbo.{tableName}(
    ...
    INDEX IX_ProductCode NONCLUSTERED (ProductCode)
);";
            using (DbCommand createCommand = conn.GetSqlStringCommond(createTableSQL))
            {
            ...
            {
                // 步骤2: 批量插入数据
                conn.BulkInsertData(queryParams, tableName);
...
INNER JOIN dbo.{tableName} t ON p.ProductCode = t.ProductCode
WHERE p.SpecsType = '5'
AND p.HouseID IN ({string.Join(",", houseIDSet)});";

                using (DbCommand queryCommand = conn.GetSqlStringCommond(querySQL))
                {
            ...
            {
                // 步骤4: 清理工作表（确保删除，即使出错也要清理）
                try
                {
                    string dropTableSQL = $"IF OBJECT_ID('dbo.{tableName}', 'U') IS NOT NULL DROP TABLE dbo.{tableName};";
                    using (DbCommand dropCommand = conn.GetSqlStringCommond(dropTableSQL))
                    {
                        conn.ExecuteNonQuery(dropCommand);
                    }
                }
                catch
                {
