<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="MyScanMoveOrder.aspx.cs" Inherits="Cargo.QY.MyScanMoveOrder" %>
        ...
        function ScanMove() {
            wx.scanQRCode({
                ...
                success: function (res) {
                    ...
                    $.ajax({
                        url: 'qyServices.aspx?method=ScanMoveOrderInCargo',
                        type: 'post', dataType: 'json', data: { ContainerCode: ContainerCode, TagCode: TagCode },
                        success: function (res) {
                            if (res.Result) {
                                $('#InBoundSuc')[0].play();
                                var tar = "<div class=\"weui-cell\"><div class=\"weui-cell__bd\"><p>" + res.Message + "</p></div><div class=\"weui-cell__ft\">移库入库成功</div></div>";
