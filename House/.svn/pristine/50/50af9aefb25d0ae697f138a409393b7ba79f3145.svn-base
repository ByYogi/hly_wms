<%@ Page Title="进货单退货" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="AddArrivalReturnOrder.aspx.cs" Inherits="Cargo.Order.AddArrivalReturnOrder" %>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            $('#dg').datagrid({
                ...
                columns: [[
                    {
                        title: '进货单号', field: 'OrderID', width: '90px', formatter: function (value) {
                            return "<span title='" + value + "'>" + value + "</span>";
                        }
                    },
                    {
                        title: '进货件数', field: 'Piece', width: '60px', align: 'right', styler: function (val, row, index) { return "color:#12bb1f;font-weight:bold;" }, formatter: function (value) {
                            return "<span title='" + value + "'>" + value + "</span>";
                        }
                    },
                    {
                        title: '进货价', field: 'UnitPrice', width: '60px', align: 'right', formatter: function (value) {
                            return "<span title='" + value + "'>" + value + "</span>";
                        }
                    },
                    {
                    ...
                    {
                        title: '客户编码', field: 'ClientNum', width: '60px', hidden: true
                    },
                    {
                    ...
                    {
                        title: '来货单号', field: 'FacOrderNo', width: '60px', formatter: function (value) {
                            return "<span title='" + value + "'>" + value + "</span>";
                        }
                    },
                ...
                groupFormatter: function (value, rows) {
                    return value + ' - 总件数：' + rows[0].TotalPiece + ' - 总费用：' + rows[0].TotalCharge + ' 元';
                },
        ...
        function dosearch() {
            ...
            gridOpts.url = 'orderApi.aspx?method=QueryOesOrderInfoForReturn';
            $('#dg').datagrid('load', {
    ...
    <div id="saPanel" name="SelectDiv1" class="easyui-panel" title="" data-options="iconCls:'icon-search'" style="width:100%">
        <table>
            <tr>
                ...
                <td>
                    <input id="AOrderNo" class="easyui-textbox" data-options="prompt:'请输入进货单号'" style="width: 100px"/>
                </td>
            ...
            <tr>
                <td style="text-align: right;">进货时间:
                </td>
                <td colspan="3">
        ...
        <form id="fmDep" class="easyui-form" method="post">
            ...
            <table style="width: 100%">
                <tr>
                    ...
                    <td>
                        <input name="Piece" id="APiece" class="easyui-numberbox" data-options="min:0,precision:0" readonly="true"  style="width: 60px;" />
                    </td>
                    <td style="text-align: right;">退货费用:
                    </td>
                    <td>
                        <input name="TransportFee" id="TransportFee" data-options="min:0,precision:2" class="easyui-numberbox" readonly="true"  style="width: 80px;" />
                    </td>
    ...
    <script type="text/javascript">
        ...
        function saveReturn() {
            ...
            $.messager.confirm('<%= Cargo.Common.GetSystemNameAndVersion()%>', '确定保存？', function (r) {
                if (r) {
                    ...
                    $('#fmDep').form('submit', {
                        url: 'orderApi.aspx?method=saveOesReturn',
        ...
        function dblClickOutCargo(Did) {
            ...
            for (var i = 0; i < rows.length; i++) {
                if (rows[i].OrderID != row.OrderID ) {
                    $.messager.alert('<%= Cargo.Common.GetSystemNameAndVersion()%>', '一次只能退货相同订单产品', 'warning');
                    return;
                }
            }
           ...
           <%-- if (row == null || row == "") {
            ...
            if (index < 0) {
                ...
                if (row) {
                    $('#dlg').dialog('open').dialog('setTitle', '订单：' + row.OrderID + ' 规格：' + row.Specs + '退货');
                    ...
                    $('#ActSalePrice').numberbox('setValue', row.UnitPrice);
                ...
                $.messager.alert('<%= Cargo.Common.GetSystemNameAndVersion()%>', '退货列表已存在该产品，请先删除再添加！', 'warning');
            }
        }
