...
namespace Cargo.Product
{
    public partial class productApi : BasePage
    {
        ...
        /// 商品上架导出实体
        /// </summary>
        public List<CargoContainerShowEntity> ShelvesExportData_Reserve
        {
            get
            {
                if (Session["ShelvesExportData_Reserve"] == null)
                {
                    Session["ShelvesExportData_Reserve"] = new List<CargoContainerShowEntity>();
                }
                return (List<CargoContainerShowEntity>)(Session["ShelvesExportData_Reserve"]);
            }
            set
            {
                Session["ShelvesExportData_Reserve"] = value;
            }
        }
        /// <summary>
        /// 商品上架导出
        /// </summary>
        public void QueryProductReserverShelvesForExport()
        {
            CargoContainerShowEntity queryEntity = new CargoContainerShowEntity();
            //queryEntity.Specs = Convert.ToString(Request["Specs"]);
            string spe = Convert.ToString(Request["Specs"]).ToUpper();
            ...
            var dataList = (List<CargoContainerShowEntity>)list["rows"];
            string err = "OK";
            if (dataList.Count > 0) { ShelvesExportData_Reserve = dataList; }
            else { err = "没有数据可以进行导出，请重新查询！"; }
            //JSON
            String json = JSON.Encode(err);
            Response.Clear();
        ...
        public void BatchUpdateProductReserveShelves()
        {
            CargoProductBus bus = new CargoProductBus();
            ErrMessage msg = new ErrMessage(); msg.Message = "";
            String json = Request["data"];
            ...
            foreach (Hashtable row in rows)
            {
                ...
                CargoProductShelvesEntity ent = new CargoProductShelvesEntity();
                ent.ID = Convert.ToInt64(row["OnSaleID"]);
                ent.ProductPrice = Convert.ToDecimal(row["ProductPrice"]);
                ent.SigningPrice = Convert.ToDecimal(row["SigningPrice"]);
                ent.minPurchase = Convert.ToDecimal(row["minPurchase"]);

                if (ent.ProductPrice == 0 && ent.SigningPrice == 0 && ent.minPurchase == 0) continue;

                list.Add(ent);
            }
            if (list.Where(w => w.ID != 0).ToList().Count>0)
            {
                bus.BatchUpdateProductReserveShelves(list, log);
            }
            //返回处理结果
            msg.Result = true;
            string res = JSON.Encode(msg);
