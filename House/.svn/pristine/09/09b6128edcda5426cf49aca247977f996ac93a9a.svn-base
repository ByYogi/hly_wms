<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="qyApprovePurchaseCheck.aspx.cs" Inherits="Cargo.QY.qyApprovePurchaseCheck" %>
...
<body ontouchstart>
    <header class="wy-header" style="position: fixed; top: 0; left: 0; right: 0; z-index: 200;">
        <div class="wy-header-title">采购订单审批</div>
    </header>
            ...
            <div class="weui-pay-m">
                <li class="weui-pay-order">
                    <dl class="weui-pay-line">
                        <dt class="weui-pay-label">采购单号：</dt>
                        <dd class="weui-pay-e" style="font-weight: bold;"><%=wxOrder.PurOrderNo %></dd>
                    </dl>
                    <dl class="weui-pay-line">
                        <dt class="weui-pay-label">采购数量：</dt>
                        <dd class="weui-pay-e" style="font-weight: bold;"><%=wxOrder.Piece %></dd>
                    </dl>
                    <dl class="weui-pay-line">
                        <dt class="weui-pay-label">总金额：</dt>
                        <dd class="weui-pay-e" style="font-weight: bold; color: #d81e06;">￥<%=wxOrder.TotalCharge %></dd>
                    </dl>
                    <dl class="weui-pay-line">
                        <dt class="weui-pay-label">供应商：</dt>
                        <dd class="weui-pay-e" style="font-weight: bold;"><%=wxOrder.PurchaserName %> </dd>
                    </dl>
                    <dl class="weui-pay-line">
                        <dt class="weui-pay-label">采购类型：</dt>
                        <dd class="weui-pay-e" style="font-weight: bold;"><%=PurchaseTypeName %></dd>
                    </dl>
                    <dl class="weui-pay-line">
                        <dt class="weui-pay-label">申请人：</dt>
                        <dd class="weui-pay-e" style="font-weight: bold;"><%=ApplyName %></dd>
                    </dl>
                    <dl class="weui-pay-line">
                        <dt class="weui-pay-label">备注：</dt>
                        <dd class="weui-pay-e" style="font-weight: bold;"><%=wxOrder.Remark %></dd>
                    </dl>
                </li>
            </div>
        </ul>
    ...
    <script type="text/javascript">
        ...
        function checkOk() {
            var ty = "<%=ty%>";
            var OID = "<%=OID%>";
            var ordn = "<%=wxOrder.PurOrderNo %>";
            var name = "<%=wxOrder.PurchaserName%>";
            var cellphone = "<%=wxOrder.DeliveryCellphone %>";
            ...
            $.confirm("", "您确定通过审批?", function () {
                $.showLoading();
                $.ajax("qyServices.aspx?method=PurchaseCheckOk&Reason=" + Reason + "&OrderNo=" + ordn + "&OID=" + OID + "&Name=" + name + "&Cellphone=" + cellphone + "&OrderType=" + OrderType, {
                    ...
                    success: function (text) {
                        ...
                        if (text.Result == true) {
                            $.alert("审批成功", "操作提示", function () {
                                if (ty == "1") {
                                    location.href = "qyMyPurchaseCheck.aspx";
                                } else {
                                    WeixinJSBridge.call('closeWindow');
                                }
                            });
        ...
        function checkNo() {
            ...
            $.confirm("", "您确定要拒审吗?", function () {
                $.showLoading();
                $.ajax("qyServices.aspx?method=PurchaseCheckNo&Reason=" + Reason + "&OrderNo=" + ordn + "&OID=" + OID + "&Name=" + name + "&Cellphone=" + cellphone + "&OrderType=" + OrderType, {
                    async: true, type: 'POST', dataType: 'json', //服务器返回json格式数据
                    success: function (text) {
                        $.hideLoading();
                        if (text.Result == true) {
