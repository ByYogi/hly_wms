<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="qyFactoryPurchaseCheck.aspx.cs" Inherits="Cargo.QY.qyFactoryPurchaseCheck" %>
...
<head runat="server">
    ...
    <meta name="description" content="锦湖工厂进货订单审批" />
...
<body>
            ...
            <div class="weui-pay-m">
                <li class="weui-pay-order">
                    <dl class="weui-pay-line">
                        ...
                        <dd class="weui-pay-e" style="font-weight: bold;"><%=orderEntity.OrderNo %></dd>
                    </dl>
                    <dl class="weui-pay-line">
                        <dt class="weui-pay-label">仓库名称：</dt>
                        <dd class="weui-pay-e" style="font-weight: bold;"><%=orderEntity.HouseName %></dd>
                    </dl>
                    <dl class="weui-pay-line">
                        <dt class="weui-pay-label">总 件 数：</dt>
                        <dd class="weui-pay-e" style="font-weight: bold;"><%=orderEntity.Piece %></dd>
                    </dl>
                    <dl class="weui-pay-line">
                        <dt class="weui-pay-label">总 金 额：</dt>
                        <dd class="weui-pay-e" style="font-weight: bold; color: #d81e06;">￥<%=orderEntity.TotalCharge %></dd>
                    </dl>
                    <dl class="weui-pay-line">
                        <dt class="weui-pay-label">申 请 人：</dt>
                        <dd class="weui-pay-e" style="font-weight: bold;"><%=orderEntity.ApplyName %></dd>
                    </dl>
                    <dl class="weui-pay-line">
                        <dt class="weui-pay-label">备注信息：</dt>
                        <dd class="weui-pay-e" style="font-weight: bold;"><%=orderEntity.Remark %></dd>
                    </dl>
                    <dl class="weui-pay-line" id="cResult">
                        <dt class="weui-pay-label">审批意见：</dt>
                        <dd class="weui-pay-e" style="font-weight: bold;"><%=orderEntity.CheckResult %></dd>
                    </dl>
                </li>
            </div>
        </ul>
        <div class="weui-cells weui-cells_form" style="font-size: 12px;">
            <div class="weui-cell" style="padding-left: 0px; padding-right: 10px; padding-top: 5px; padding-bottom: 5px; color: #e16531;">
                <div class="weui-cell__hd" style="width: 45%">
                    <label class="weui-label" style="width: 100%">&nbsp;&nbsp;&nbsp;&nbsp;进货产品明细</label>
                </div>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            var odn = "<%=orderEntity.OrderNo%>";
            if (odn == undefined || odn == "") {
                $.alert("该进货单不存在或已删除");
                return;
            }
            var ApplyStatus = "<%=orderEntity.ApplyStatus%>";
        ...
        function checkOk() {
            var ty = "<%=ty%>";
            var OID = "<%=orderEntity.OrderID%>";
            var ordn = "<%=orderEntity.OrderNo %>";
            ...
            $.confirm("", "您确定通过审批?", function () {
                $.showLoading();
                $.ajax("qyServices.aspx?method=FactoryPurchaseOrderCheckOk&Reason=" + Reason + "&OrderNo=" + ordn + "&OID=" + OID, {
        ...
        function checkNo() {
            ...
            $.confirm("", "您确定要拒审吗?", function () {
                $.showLoading();
                $.ajax("qyServices.aspx?method=FactoryPurchaseOrderCheckNo&Reason=" + Reason + "&OrderNo=" + ordn + "&OID=" + OID, {
                    async: true, type: 'POST', dataType: 'json', //服务器返回json格式数据
                    success: function (text) {
                        $.hideLoading();
                        if (text.Result == true) {
