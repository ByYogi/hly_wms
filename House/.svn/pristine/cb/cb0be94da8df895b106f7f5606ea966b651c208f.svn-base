    ...
    public class CargoInterfaceBus
    {
        ...
        public void scanTagInMaintenance(CargoProductEntity entity, LogEntity log)
        {
            ...
            using (TransactionScope scope = new TransactionScope(TransactionScopeOption.Required))
            {
                try
                {
                    ...
                    CargoProductEntity productEntity = productMan.QueryProductInfo(new CargoProductEntity { HouseID = entity.HouseID, TypeID = entity.TypeID, Specs = entity.Specs, Figure = entity.Figure, Batch = entity.Batch, ContainerCode = entity.ContainerCode, GoodsCode = entity.GoodsCode });
                    //判断产品表里是否存在，如果 没有新增
                ...
                try
                {
                    ...
                    switch (entity.ParentID)
                    {
                        ...
                        case 354:
                            ...
                            productEntity = productMan.QueryProductInfo(new CargoProductEntity { HouseID = entity.HouseID, TypeID = entity.TypeID, ContainerCode = entity.ContainerCode, GoodsCode = entity.GoodsCode, Batch = entity.Batch });
                            break;
                        default:
                            productEntity = productMan.QueryProductInfo(new CargoProductEntity { HouseID = entity.HouseID, TypeID = entity.TypeID, Specs = entity.Specs, ContainerCode = entity.ContainerCode, GoodsCode = entity.GoodsCode });
                            break;
                    }
                    //判断产品表里是否存在，如果 没有新增
                    if (productEntity == null || productEntity.ProductID.Equals(0) || productEntity.Numbers <= 0)
                ...
                try
                {
                    ...
                    if (isAddNew)
                    {
                        ...
                        if (basicPrice != null && basicPrice.PID != 0)
                        {
                            ...
                            if (basicPrice.TradePrice > 0) entity.SalePrice = Convert.ToDecimal(basicPrice.TradePrice);
                        ...
                        productMan.UpdateCargoProductStatus(new CargoProductEntity { InHouseTime = now, InCargoStatus = "1", ProductID = did });
                        productEntity.ProductID = did;
                        log.Memo = "移库入库操作：所属仓库：" + entity.HouseID.ToString() + "，产品名称：" + entity.ProductName + "，规格：" + entity.Specs + "，花纹：" + entity.Figure + "，型号：" + entity.Model + "，货品代码：" + entity.GoodsCode + "，周期批次：" + entity.Batch + "，载重指数：" + entity.LoadIndex.ToString() + "，速度级别：" + entity.SpeedLevel + "，货位代码：" + entity.ContainerCode + "，标签编码：" + entity.TagCode + "，入库数量：" + entity.Numbers.ToString() + "，操作人：" + log.UserID + "，产品ID：" + did.ToString() + "，入库时间：" + now.ToString("yyyy-MM-dd HH:mm:ss") + ",门店价" + entity.TradePrice.ToString() + ",小程序价:" + entity.SalePrice.ToString() + ",进仓价:" + entity.InHousePrice.ToString();
        ...
        public void SaveNextDayProductData(List<CargoProductEntity> list)
        {
            ...
            string GetKey(CargoProductEntity p) =>
                $"{p.SpecsType}_{p.ProductCode}_{p.BatchYear}_{p.TypeID}_{p.HouseID}_{p.SuppClientNum}";
            ...
            using (TransactionScope scope = new TransactionScope(TransactionScopeOption.Required))
            {
                ...
                foreach (var item in list)
                {
                    ...
                    {
                        //去重判断，如果有重复需要新增的产品，后一个的覆盖前一个产品数据
