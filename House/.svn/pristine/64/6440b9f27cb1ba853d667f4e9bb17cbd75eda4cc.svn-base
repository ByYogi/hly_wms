        ...
        $(document).ready(function () {
            $('#dg').datagrid({
                ...
                columns: [[
                    ...
                    {
                        title: '操作', field: '', width: '250px', formatter: function (value, row, index) {
                            var Action = "<a href='javascript:void(0);' onclick='DownloadFile(" + row.ID + ")'>下载</a>";
                            return Action;
                        }
                    }
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <script type="text/javascript">
        ...
        function SyncData(type=1) {
            $.messager.progress({ msg: '请稍后,正在更新中...' });
            $.ajax({
                type: 'post',
                url: "../House/houseApi.aspx?method=SyncStockData",
                data: { type },
        ...
        function DownloadFile(id) {
            ...
            var data = row.filter(a => a.ID == id);
            console.log(data, id)

            axios({
                url: "../House/houseApi.aspx?method=DownloadFile",
                method: 'GET',
                params: {                         // 这里是参数对象
                    ID: data[0].ID
                },
