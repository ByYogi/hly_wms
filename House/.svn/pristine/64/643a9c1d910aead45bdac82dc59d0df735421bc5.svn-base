...
namespace Cargo
{
    ...
    public static class Common
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static int GetWxMaxQuantity()
        {
            var quantityStr = ConfigurationSettings.AppSettings["maxOrderQuantity"];
            return string.IsNullOrEmpty(quantityStr) ? 50 : Convert.ToInt32(quantityStr);
        }
        /// <summary>
        /// 狄乐汽服商户号ID
        /// </summary>
        ...
        public static string GetMaxAccountNoNum(string hCode)
        {
            string result = string.Empty;
            CargoClientBus bus = new CargoClientBus();
            string key = DateTime.Now.ToString("yyyyMMdd");
        ...
        /// <param name="hCode"></param>
        /// <returns></returns>
        public static string GetMaxReserveOrderNumByCurrentDate(int hID, string hCode, out int OrderNum, int nextNum = 0)
        {
            string result = string.Empty;
            CargoOrderBus bus = new CargoOrderBus();
            ...
            {
                //不存在缓存
                int oMaxNum = bus.GetMaxReserveOrderNumByCurrentDate(new CargoOrderEntity { HouseID = hID, StartDate = DateTime.Now, EndDate = DateTime.Now }) + nextNum;
                RedisHelper.HashSet("HouseOrderNo", key, (oMaxNum + 1).ToString());
            ...
            result = "PRO" + hCode.Trim() + DateTime.Now.ToString("yyMMdd") + oStr;
            OrderNum = oNum + 1;
            return result;
        }
        /// <summary>
            ...
            try
            {
                ...
                if (accessToken != null)
                {
                    ...
                    for (int j = 0; j < SupplierList.Count; j++)
                    {
                        ...
                        string toUrl = "http://dlt.neway5.com/Weixin/wxOrderPurchaseSuccess.aspx?orderno=" + orderNo; //跳转url
                        string url = "https://api.weixin.qq.com/cgi-bin/message/template/send?access_token=" + accessToken;
                        //设置推送的模板，包括推送的openid、模板ID-templateId 、推送的信息：dt1.Rows[i]["XXX"]等
                        string temp = "{\"touser\": \"" + openId + "\"," +
                        ...
                        WriteTextLog("慧采云仓  次日达订单推送结果：" + results);
                        strReturn = SupplierList[j].Name + "——" + openId + "——" + "推送成功";
                        Common.WriteTextLog(strReturn);
                    }
                }
