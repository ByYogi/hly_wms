    ...
    public class CargoProductBasicPriceManeger
    {
        ...
        public Hashtable QueryProductBasicPrice(int pIndex, int pNum, CargoProductBasicPriceEntity entity)
        {
            ...
            try
            {
                ...
                strSQL += " order by a.ProYear desc,a.ProMonth desc,a.OP_DATE desc) as A where RowNumber > (" + pNum + "* (" + pIndex + "-1))";
                    ...
                    using (DataTable dd = conn.ExecuteDataTable(command))
                    {
                        foreach (DataRow idr in dd.Rows)
                        {
                            result.Add(new CargoProductBasicPriceEntity
                            {
                                ...
                                NextDayPrice = Convert.ToDouble(idr["NextDayPrice"]),
        ...
        public void SaveProductBasicPriceData(List<CargoProductBasicPriceEntity> entity)
        {
            ...
            foreach (var it in entity)
            {
                //if (!type)
                //{
                //    if (IsExistInputData(it.ProYear, it.ProMonth) > 0)
                //    {
                //        DeleteProductBasicPriceData(it.ProYear, it.ProMonth);
                //    }
                //    type = true;
                //}
                //AddProductBasicPriceData(it);

                CargoProductBasicPriceEntity priceData = productPricelist.Where(w => w.HouseID == it.HouseID && w.Born == it.Born && w.TypeID == it.TypeID  && w.Model == it.Model && w.GoodsCode == it.GoodsCode && w.Specs == it.Specs && w.Figure == it.Figure && w.LoadIndex == it.LoadIndex && w.SpeedLevel == it.SpeedLevel).FirstOrDefault();
                //判断当前导入数据是否存在，存在则修改
                //DataTable dt = IsExistInputData(it);
        ...
        {
            string strSQL = @"INSERT INTO Tbl_Cargo_ProductBasicPrice (HouseID,TypeID,Born,Assort,Model,GoodsCode,Specs,Figure,LoadIndex,SpeedLevel,HubDiameter,ProYear,ProMonth,ProQuarter,UnitPrice,TradePrice,FinalCostPrice,CostPrice,TaxCostPrice,NoTaxCostPrice,OP_DATE,ProductCode,CloudHouseType,InHousePrice,SalePrice,OESalePrice,NextDayPrice) VALUES (@HouseID,@TypeID,@Born,@Assort,@Model,@GoodsCode,@Specs,@Figure,@LoadIndex,@SpeedLevel,@HubDiameter,@ProYear,@ProMonth,@ProQuarter,@UnitPrice,@TradePrice,@FinalCostPrice,@CostPrice,@TaxCostPrice,@NoTaxCostPrice,@OP_DATE,@ProductCode,@CloudHouseType,@InHousePrice,@SalePrice,@OESalePrice,@NextDayPrice)";
            try
            {
                using (DbCommand cmd = conn.GetSqlStringCommond(strSQL))
                {
        ...
        public void UpdateProductPriceData(CargoProductBasicPriceEntity entity)
        {
            ...
            //}
            strSQL += " where HouseID=@HouseID and TypeID=@TypeID and Born=@Born and Assort=@Assort and GoodsCode=@GoodsCode and Specs=@Specs and Figure=@Figure and LoadIndex=@LoadIndex and SpeedLevel=@SpeedLevel and BelongMonth=@BelongMonth";
            if (!string.IsNullOrEmpty(entity.BelongDepart))
            {
                strSQL += " and (BelongDepart=@BelongDepart or BelongDepart IS NULL)";
            }
        ...
        {
            string strSQL = @"insert into Tbl_Cargo_ProductBasicPrice (TypeID, HouseID, Born, Assort, Model, GoodsCode, Specs, Figure, LoadIndex, SpeedLevel, HubDiameter, ProYear, ProMonth, ProQuarter, UnitPrice, TradePrice, FinalCostPrice, PreCostPrice, CostPrice, TaxCostPrice, NoTaxCostPrice, OP_DATE,OESalePrice,ProductCode,InHousePrice,NextDayPrice) select TypeID, @NewHouse, Born, Assort, Model, GoodsCode, Specs, Figure, LoadIndex, SpeedLevel, HubDiameter, @NewProYear, @NewProMonth, @NewProQuarter, UnitPrice, TradePrice, FinalCostPrice, PreCostPrice, CostPrice, TaxCostPrice, NoTaxCostPrice,GETDATE(),OESalePrice,ProductCode,InHousePrice,NextDayPrice from Tbl_Cargo_ProductBasicPrice where PID=@PID";
        ...
        {
            string strSQL = @"insert into Tbl_Cargo_ProductBasicPrice (TypeID, HouseID, Born, Assort, Model, GoodsCode, Specs, Figure, LoadIndex, SpeedLevel, HubDiameter, ProYear, ProMonth, ProQuarter, UnitPrice, TradePrice, FinalCostPrice, PreCostPrice, CostPrice, TaxCostPrice, NoTaxCostPrice,SalePrice,OESalePrice,ProductCode,InHousePrice,NextDayPrice, OP_DATE) select TypeID, @NewHouse, Born, Assort, Model, GoodsCode, Specs, Figure, LoadIndex, SpeedLevel, HubDiameter, @NewProYear, @NewProMonth, @NewProQuarter, UnitPrice, TradePrice, FinalCostPrice, PreCostPrice, CostPrice, TaxCostPrice, NoTaxCostPrice,SalePrice,OESalePrice,ProductCode,InHousePrice,NextDayPrice,GETDATE() from Tbl_Cargo_ProductBasicPrice where HouseID=@OldHouse and ProYear=@OldProYear and ProMonth=@OldProMonth";
        ...
        public void SynchronizePriceBasic(List<CargoProductBasicPriceEntity> entity)
        {
