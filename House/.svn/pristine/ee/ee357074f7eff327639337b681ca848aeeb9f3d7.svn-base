<%@ Page Title="商品上架" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="productShelves.aspx.cs" Inherits="Cargo.Product.productShelves" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    ...
    <script type="text/javascript">
            ...
            columns.push({
                title: '销售类型', field: 'SaleType', width: '60px', formatter: function (val, row, index) {
                    if (row.OnShelves == "1") {
                        ...
                        if (val == "0") { return "<span title='正价销售'>正价销售</span>"; }
                        else if (val == "1") { return "<span title='天天特价'>天天特价</span>"; }
                        else if (val == "3") { return "<span title='限时促销'>限时促销</span>"; }
                        else if (val == "4") { return "<span title='积分兑换'>积分兑换</span>"; } else { return ""; }
                    }
                }
            });
            columns.push({
                title: '积分数量', field: 'Consume', width: '60px', align: 'right', formatter: function (value) {
                    return "<span title='" + value + "'>" + value + "</span>";
                }
            });
            columns.push({
        ...
        function dosearch() {
            ...
            gridOpts.url = 'productApi.aspx?method=QueryInHouseProductData';
            $('#dg').datagrid('load', {
                Specs: $('#ASpecs').val(),
                Model: $('#AModel').val(),
                Figure: $('#AFigure').val(),
                ...
                BatchA: $('#ABatchWeekStart').numberbox('getValue'),
                BatchB: $('#ABatchWeekEnd').numberbox('getValue'),
                ...
                Piece: $("#APiece").combobox('getValue')//在库数量类型
            });
        }
    </script>
</asp:Content>
    ...
    <div id="saPanel" name="SelectDiv1" class="easyui-panel" title="" data-options="iconCls:'icon-search'" style="width: 100%">
        <table>
            <tr>
                ...
                <td>
                    <select class="easyui-combobox" id="ASaleType" style="width: 80px;" panelheight="auto">
                ...
                <td>
                    <select class="easyui-combobox" id="APiece" style="width: 80px;" panelheight="auto" editable="false">
                        <option value="-1">全部库存</option>
                        <option value="0">正常库存</option>
                        <option value="1">预警库存</option>
                        <option value="2">为零库存</option>
                    </select>
                </td>
            </tr>
            <tr>
                ...
                <td style="text-align: right;">批次周范围:
                </td>
                <td>
                    <input id="ABatchWeekStart" class="easyui-numberbox" data-options="prompt:'开始周'" style="width: 50px" />~
                     <input id="ABatchWeekEnd" class="easyui-numberbox" data-options="prompt:'结束周'" style="width: 50px" />
                </td>
                ...
                <td>
                    <select class="easyui-combobox" id="onShelves" style="width: 80px;" panelheight="auto">
                        <option value="-1">全部</option>
    ...
    <script type="text/javascript">
        ...
        function Export() {
            ...
            $.ajax({
                ...
                data: { Specs: Specs, Model: Model, Figure: Figure, PID: PID, SID: SID, ProductName: ProductName, Batch: Batch, BatchYear: BatchYear, LoadIndex: LoadIndex, onShelves: onShelves, SaleType: SaleType, HAID: HAID, HouseID: HouseID, Piece: Piece },
        ...
        function Unshelves() {
            ...
            $.messager.confirm('<%= Cargo.Common.GetSystemNameAndVersion()%>', '确定下架该商品？', function (r) {
                if (r) {
                    ...
                    $.ajax({
                        url: 'productApi.aspx?method=UnLoadshelves',
        ...
        function saveItem() {
            ...
            var Piece = Number($('#Piece').numberbox('getValue'));
            if (Number(HouseID) != 9 && Number(HouseID) != 12) {
                if (onNum > Piece) {
                    $.messager.alert('<%= Cargo.Common.GetSystemNameAndVersion()%>', '上架数量不能大于在库数量', 'warning');
                    return;
                }
            }
            ...
            $('#fm').form('submit', {
                url: 'productApi.aspx?method=SaveProductShelves',
        ...
        function shelves() {
            ...
            if (row) {
                $('#dlg').dialog('open').dialog('setTitle', '商品上架');
                $('#sj').text("上架数量");
                $('#fm').form('clear');
                $('#fm').form('load', row);
