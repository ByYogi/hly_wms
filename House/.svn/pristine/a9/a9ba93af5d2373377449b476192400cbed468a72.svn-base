...
Page({
  ...
  async bindSave(e) {
    ...
    const linkCompany = e.detail.value.linkCompany;
    ...
    if (this.data.pIndex > 0) {
      postData.provinceId = this.data.provinces[this.data.pIndex].name
    }
    if (this.data.cIndex > 0) {
      postData.cityId = this.data.cities[this.data.cIndex].name
    }
    if (this.data.aIndex > 0) {
      postData.districtId = this.data.areas[this.data.aIndex].name
    }    
    let apiResult
    if (this.data.id) {
  ...
  async readFromWx() {
    ...
    wx.chooseAddress({
      success: function (res) {
        ...
        const addressData = {}
        addressData.linkCompany = res.userName
