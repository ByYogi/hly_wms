    ...
    public class CargoInterfaceManager
    {
        ...
        public Hashtable QueryContiOrder(int pIndex, int pNum, saleOpenInfoResp entity)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(entity.OrderHouseWhere))
                {
                    strSQL += $@" and a.distributorSoldTo='{entity.OrderHouseWhere}' ";
                }
                    ...
                    using (DataTable dd = conn.ExecuteDataTable(command))
                    {
                        foreach (DataRow idr in dd.Rows)
                        {
                            result.Add(new saleOpenInfoResp
                            {
                                ...
                                warehouseProvince = Convert.ToString(idr["warehouseProvince"]),
                            });
                        }
                    }
                    #endregion
                ...
                if (!string.IsNullOrEmpty(entity.warehouseProvince)) { strCount += "and a.warehouseProvince='" + entity.warehouseProvince + "'"; }
        ...
        {
            Int64 did = 0;
            string strSQL = @"INSERT INTO Tbl_Cargo_ContiOrder(orderCode,orderSource,orderType,orderLabel,orderLabelStr,deliveryType,deliveryTypeDesc,customerCode,customerName,customerSubCode,sellerName,creditAmount,rebateAmount,nowTotalAmount,freightAmount,submitTime,remark,consigneeAddress,consigneeName,consigneeMobile,distributorSoldTo,warehouseCode,warehouseName,OPDATE,orderStatus,orderStatusStr,onlinePaidAmount,BidClientNum,BidClientName,HouseID,InCreateStatus,AreaID,OrderPiece,couponAmount,naCustomerName,warehouseProvince) VALUES (@orderCode,@orderSource,@orderType,@orderLabel,@orderLabelStr,@deliveryType,@deliveryTypeDesc,@customerCode,@customerName,@customerSubCode,@sellerName,@creditAmount,@rebateAmount,@nowTotalAmount,@freightAmount,@submitTime,@remark,@consigneeAddress,@consigneeName,@consigneeMobile,@distributorSoldTo,@warehouseCode,@warehouseName,@OPDATE,@orderStatus,@orderStatusStr,@onlinePaidAmount,@BidClientNum,@BidClientName,@HouseID,@InCreateStatus,@AreaID,@OrderPiece,@couponAmount,@naCustomerName,@warehouseProvince) SELECT @@IDENTITY";
            try
            {
                #region 订单保存
                entity.EnSafe();
...
";
        ...
        public Hashtable QueryCassMallOrder(int pIndex, int pNum, OrderHeader entity)
        {
            ...
            try
            {
              ...
              ,d.facilityId,d.facilityName,e.HouseID,e.AreaID,c.cityGeoName,f.OverDayNum,f.OverDueUnitPrice,cc.feeAmount,f.Person,users.LoginName,f.Cellphone,ff.Name OrderHouseName,iif(itemDescriptionCount>1,1,0) IsAllBatch
...
group by ltrim(rtrim(UserName))
                ...
                left join (
select  orderId,count(1) itemDescriptionCount from (
    select orderId, (case when itemDescription like '%上半年%' then 1 when itemDescription like '%下半年%' then 2 else 0 end) as itemDescription from Tbl_Cargo_CassMallOrderGoods
              ) as a
where itemDescription>0
group by  orderId
      ) as kk on a.orderId=kk.orderId
      where 1 = 1  ";
                    ...
                    using (DataTable dd = conn.ExecuteDataTable(command))
                    {
                        ...
                        foreach (DataRow idr in dd.Rows)
                        {
                            result.Add(new OrderHeader
                            {
                                ...
                                CargoOrderNo = Convert.ToString(idr["CargoOrderNo"]),
                                IsAllBatch = Convert.ToInt32(idr["IsAllBatch"]),
                            });
                        }
                    }
                    #endregion
      ...
      inner join Tbl_Cargo_CassMallOrderPostalAddress as c on a.orderId=c.orderId
        ...
        public List<CargoProductMappingEntity> GetProductMapping(OrderItem entity)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(entity.PartsNum)) { strSQL += $@" and REPLACE(a.CassCode, ' ', '') = REPLACE('{entity.PartsNum}', ' ', '')"; }
                using (DbCommand command = conn.GetSqlStringCommond(strSQL))
                {
                    using (DataTable dd = conn.ExecuteDataTable(command))
                    {
                ...
                using (DataTable dd = conn.ExecuteDataTable(command))
                {
                    foreach (DataRow idr in dd.Rows)
                    {
                        result.Add(new CargoProductEntity
                        {
                            ...
                            ShareHouseID = Convert.ToInt32(idr["ShareHouseID"]),//共享仓库ID 主仓
...
VALUES(@orderId,@orderItemSeqId,@adjustmentType,@adjustmentDesc,@adjustmentAmount)";
                ...
                string strSQL = $@"
                    select a.*,b.companyName as AconsigneeName,c.ReceiverName,c.address
                      from Tbl_Cargo_CassMallOrder as a
                               inner join Tbl_Cargo_CassMallOrderBuyer as b on a.CID = b.CID
                      inner join Tbl_Cargo_CassMallOrderPostalAddress as c on a.orderId=c.orderId
                      where 1 = 1  and a.orderId='{entity}'";
                using (DbCommand command = conn.GetSqlStringCommond(strSQL))
                {
                    using (DataTable dt = conn.ExecuteDataTable(command))
                    {
        ...
        public void HandleGoodsClassData()
        {
            ...
            string strSQLOut = "update Tbl_Cargo_Product set GoodsClass=1 where GoodsClass!=1 and SuppClientNum in (100143,980873,982170,394992,270319,484582,349629,989827,832291,958950,630009,172833,251253,643493,307811,405525,689267)";
...
INNER JOIN productPiece pp ON p.ProductID = pp.ProductID
            ...
            {
                throw new ApplicationException($"查询途虎同步商品数据失败，原因：{ex.Message}。\r\nSql语句：{strSQL}", ex);
            }
        ...
        /// 查询途虎商品代码映射
        /// </summary>
        /// <param name="productCodes">产品编码数组</param>
        /// <returns>ProductCode -> ShareCode 的字典映射</returns>
        public Dictionary<string, string> QueryTuhuProdCodes(string[] productCodes)
        {
            Dictionary<string, string> result = new Dictionary<string, string>();
            if (productCodes == null || productCodes.Length == 0) return result;

            string productCodeStr = "'" + string.Join("','", productCodes) + "'";
            string strSQL = $@"
                SELECT ProductCode, ShareCode
                FROM Tbl_Cargo_ProductMapping
                WHERE CorpClass = 4
                  AND ProductCode IN ({productCodeStr})";
            ...
            using (DbCommand command = conn.GetSqlStringCommond(strSQL))
            {
                using (DataTable dt = conn.ExecuteDataTable(command))
                {
                    foreach (DataRow idr in dt.Rows)
                    {
                        string productCode = Convert.ToString(idr["ProductCode"]);
                        string shareCode = Convert.ToString(idr["ShareCode"]);
                        if (!string.IsNullOrEmpty(productCode) && !result.ContainsKey(productCode))
                        {
                            result[productCode] = shareCode;
                        }
                    }
                }
            }
        ...
        public void SaveTMallOutHouseData(CargoTMallEntity entity)
        {
            ...
            {
                string strSQL = $@"
insert into Tbl_Cargo_TMallOrder(userid, consigneecityid, sendtype, consigneephone, originbillfirstchannel, originbillsecondchannel, originbillthirdchannel, requirearrivetime, isexpresssheetencrypted, extend, customercontact, outboundnoticeno, originbillno, originbilltype, urgentlevel, consigneeprovincename, consigneecityname, consigneecountyname, warehousecode, saletype, warehousename, thirdwarehousecode, noticecreatetime, buyerremark, sellerremark, deliverremark, originbilltime, requireoutwarehousetime, isallowlack, isencrypted, alioaid, consigneecontacts, customername, paytime, consigneedetail, consigneecountyid, consigneeprovinceid,OP_DATE) 
...
";
                ...
                using (DbCommand cmd = conn.GetSqlStringCommond(strSQL))
                {
                    ...
                    conn.AddInParameter(cmd, "@requirearrivetime", DbType.DateTime2, entity.requireArriveTime_Actual);
                    ...
                    conn.AddInParameter(cmd, "@noticecreatetime", DbType.DateTime2, entity.noticeCreateTime_Actual);
                    ...
                    conn.AddInParameter(cmd, "@originbilltime", DbType.DateTime2, entity.originBillTime_Actual);
                    conn.AddInParameter(cmd, "@requireoutwarehousetime", DbType.DateTime2, entity.requireOutWarehouseTime_Actual);
                    ...
                    conn.AddInParameter(cmd, "@customername", DbType.String, entity.customerName ?? "");
                    conn.AddInParameter(cmd, "@paytime", DbType.DateTime2, entity.payTime_Actual);
                    ...
                    conn.AddInParameter(cmd, "@OP_DATE", DbType.DateTime2, DateTime.Now);
                    cdid = Convert.ToInt64(conn.ExecuteScalar(cmd));
                }
