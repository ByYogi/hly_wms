...
Page({
  data: {
    ...
    peisongType: '0', // 配送方式 kd：急送 17  ps：普送 0，默认是ps普送,zq 分别表示快递/到店自取
    remark: '',
    shopIndex: -1,
    pageIsEnd: false,
  ...
  async goCreateOrder(){
    ...
    if (wx.getStorageSync('needIdCheck') == 1) {      
      ...
      if (res.code == 0 && !res.data._base.isIdcardCheck) {
        wx.navigateTo({
  ...
  createOrder: function (e) {
    ...
    if ((postData.peisongType == '17' ||postData.peisongType == '0') && that.data.curAddressData && that.data.curAddressData.provinceId) {
      postData.provinceId = that.data.curAddressData.provinceId;
    }
    if ((postData.peisongType == '17' ||postData.peisongType == '0')&& that.data.curAddressData && that.data.curAddressData.cityId) {
      postData.cityId = that.data.curAddressData.cityId;
      postData.cityStr=that.data.curAddressData.cityStr;
    }
    if ((postData.peisongType == '17' ||postData.peisongType == '0')&& that.data.curAddressData && that.data.curAddressData.districtId) {
      postData.districtId = that.data.curAddressData.districtId;
    }
    if (e && that.data.isNeedLogistics > 0 && (postData.peisongType == '17'|| postData.peisongType == '0')) {
      ...
      if (postData.peisongType == '17'||postData.peisongType == '0') {
        postData.address = that.data.curAddressData.address;
        postData.linkMan = that.data.curAddressData.linkMan;
        postData.linkCompany = that.data.curAddressData.linkCompany;
  ...
  processYunfei() {
    ...
    for (let i = 0; i < goodsList.length; i++) {
      ...
      _goodsJsonStr.notes = carShopBean.notes
  ...
  async getUserApiInfo() {
    ...
    if (res.code == 0) {
      this.setData({
        bindMobileStatus: res.data._base.mobile ? 1: 1, // 账户绑定的手机号码状态
        mobile: res.data._base.mobile,
      })
    }
  },
