        ...
        $(function () {
            ...
            $('#dgHouseAim').datagrid({
                ...
                columns: [[
                    ...
                    { title: '销售量', field: 'Piece', width: '60px', align: 'right' },
                    { title: '月目标', field: 'AimQuantity', width: '60px', align: 'right' },
                    {
                        title: '完成率', field: 'PercentComplete', width: '60px', align: 'right', formatter: function (val) {
                            if (val == 0) {
                                return ''
                            }
                            return val + "%"
            ...
            $('#dgOrderDayStatistics').datagrid({
                ...
                columns: [[
                    {
                        title: '品牌名称', field: 'TypeName', width: '110px', formatter: function (val, item) {
                            //if (item.TypeName == "footer") return '';
                            return val
                        }
                    },
                    {
                        title: '仓库名称', field: 'HouseName', width: '110px', formatter: function (val, item) {
                            if (item.TypeName == "footer") return '';
                            return val
                        }
                    },
                    {
                        title: '销量', field: 'Piece', width: '70px', align: 'right', formatter: function (val, item) {
                            if (item.TypeName == "footer") return '';
                            return val
                        }
                    },
                    {
                        title: '销售SKU', field: 'DayStatisticsCount', width: '90px', align: 'right', formatter: function (val, item) {
                            if (item.HouseName == "汇总：") return '';
                            return val
                        }
                    },
                    {
                        title: '目标SKU', field: 'StockNum', width: '90px', align: 'right', formatter: function (val, item) {
                            if (item.HouseName == "汇总：") return '';
                            return val
                        }
                    },
                    ...
                    {
                        title: '库存', field: 'ProPiece', width: '90px', align: 'right', formatter: function (val, item) {
                            if (item.TypeName == "footer") return '';
                            return val
                        }
                    },
                    {
                        title: 'SKU满足率', field: 'SatisfactionRate', width: '90px', align: 'right', formatter: function (val, item) {
                            if (item.HouseName == "汇总：") return '';
                            return val + "%"
                        }
                    },
                    {
                        title: '动销率', field: 'SalesRate', width: '90px', align: 'right', formatter: function (val, item) {
                            if (item.HouseName == "汇总：") return '';
                            return val + "%"
                        },
                    ...
                    {
                        title: '月库存周转数', field: 'InventoryTurnover', width: '90px', align: 'right', formatter: function (val, item) {
                            if (item.HouseName == "汇总：") return '';
                            return val
                        }
                    },
                    {
                        title: '点击转化率', field: 'ClickConversionRate', width: '90px', align: 'right', formatter: function (val, item) {
                            if (item.HouseName == "汇总：") return '';
                            return val + "%"
                        },
                        styler: function (value, row, index) {
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
            ...
            <table id="saPanelTab">
                <tr>
                    ...
                    <td>
                        ...
                        <%--<a href="#" class="easyui-linkbutton" iconcls="icon-search" plain="false" onclick="DoSearch()">&nbsp;同&nbsp;步&nbsp;</a>--%>&nbsp;&nbsp;
    ...
    <div style="width: 100%; height: 100%;" id="ChartBox">
        <div style="width: 100%; display: flex; align-items: center;">
            <div style="width: 24%;" id="pn23" data-options="tools:'#tl1'" class="easyui-panel" title="销售量">
