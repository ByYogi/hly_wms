<%@ Page Language="C#" Title="订单销售明细" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="OrderSaleDetails.aspx.cs" Inherits="Supplier.Report.OrderSaleDetails" %>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            $('#dg').datagrid({
                ...
                toolbar: '#toolbar',
                columns: [[
                    { title: '', field: '', checkbox: true, width: '5px' },
                    {
                        title: '出库仓库', field: 'HouseName', width: '100px', formatter: function (value) {
                            return "<span title='" + value + "'>" + value + "</span>";
                        }
                    },
                    { title: '开单时间', field: 'OP_DATE', width: '130px', formatter: DateTimeFormatter },
                    {
                    ...
                    //        if (val == "0") { return "<span title='销售单'>销售单</span>"; }
                    //        else if (val == "1") { return "<span title='退货单'>退货单</span>"; }
                    //        else { return ""; }
                    //    }
                    //},
                    {
                        title: '订单号', field: 'OrderNo', width: '130px', formatter: function (value) {
                            return "<span title='" + value + "'>" + value + "</span>";
                        }
                    },
                    {
                        title: '客户名称', field: 'AcceptUnit', width: '150px', formatter: function (value) {
                            return "<span title='" + value + "'>" + value + "</span>";
                        }
                    },
                    {
                    ...
                    {
                        title: '规格', field: 'Specs', width: '110px', formatter: function (value) {
                            return "<span title='" + value + "'>" + value + "</span>";
                        }
                    },
                    {
                    ...
                    { title: '批次', field: 'Batch', width: '100px' },
                    {
                    ...
                    {
                        title: '数量', field: 'Piece', width: '80px', align: 'right', formatter: function (value, row) {
                            if (row.OrderModel == "1") {
                                return "-" + value;
                            } else { return value; }
                        }
                    },
                    { title: '成本单价', field: 'CostPrice', width: '80px', align: 'right', },//InHousePrice
                    {
                        title: '总成本', field: 'TotalCostPrice', width: '90px', align: 'right', formatter: function (value, row) {
                            if (row.OrderModel == "1") {
                                return "-" + value;
                            } else { return value; }
                        }
                    },//InHousePrice * Piece
                    { title: '销售单价', field: 'ActSalePrice', width: '90px', align: 'right' },
                    //{
                    //    title: '销售额', field: 'SalePrice', width: '80px', align: 'right', formatter: function (value, row) {
                    //        if (row.OrderModel == "1") {
                    //            return "-" + value;
                    //        } else { return value; }
                    //    }
        ...
        function dosearch() {
            ...
            gridOpts.url = 'reportApi.aspx?method=QueryOrderSaleDetailsInfo';
            $('#dg').datagrid('load', {
                AcceptUnit: $('#AAcceptUnit').val(),//客户名称
                OrderNo: $("#AOrderNo").val(),//订单号
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <div id="saPanel" name="SelectDiv1" class="easyui-panel" title="" data-options="iconCls:'icon-search'" style="width: 100%">
        <table>
            <tr>
                ...
                <td>
                    <input id="AAcceptUnit" class="easyui-textbox" data-options="prompt:'请输入客户名称'" style="width: 95px" />
                </td>
                <td style="text-align: right;">订单号:
                </td>
                <td>
                    <input id="AOrderNo" class="easyui-textbox" data-options="prompt:'请输入订单号'" style="width: 95px" />
                </td>
    ...
    <script type="text/javascript">
        ...
        function exportData() {
            ...
            $.ajax({
                url: "reportApi.aspx?method=QueryOrderSaleDetailsInfoExport&SID=" + $("#ASID").combobox('getValue') +
                    "&HouseID=" + $("#AHouseID").combobox('getValue') + "&Specs=" + $('#ASpecs').val() +
                    "&Figure=" + $("#AFigure").val() + "&AcceptUnit=" + $("#AAcceptUnit").val() + "&OrderNo=" + $("#AOrderNo").val() +
