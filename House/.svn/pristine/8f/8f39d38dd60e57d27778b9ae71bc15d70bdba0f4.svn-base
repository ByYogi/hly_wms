...
namespace HouseServices
{
    public partial class Service1 : ServiceBase
    {
        ...
        public Service1()
        {
            ...
            System.Timers.Timer ReserverTime = new System.Timers.Timer();

            //20251226 由于马牌系统关闭库存同步接口，关闭
            //马牌订单和库存同步接口定时器
            ...
            //开思发货通知
            CassMallSyncTime.Elapsed += SendDeliveryNotice; //全量同步
            CassMallSyncTime.Interval = 5 * 60 * 1000;//5分钟跳动一次 测试阶段
            CassMallSyncTime.Enabled = true;


            //预订单删除未支付订单
            ReserverTime.Elapsed += DelReserveOrder; //
            ReserverTime.Interval = 2 * 60 * 60 * 1000; ;//2小时跳动一次 测试阶段
            ReserverTime.Enabled = true;
        }
                    ...
                    if (File.Exists(filePath))
                    {
                        ...
                        if (Directory.Exists(fileFolder))
                        {
                            ...
                            foreach (string file in files)
                            {
                                ...
                                // 判断是否超过一个月
                                if (createTime < DateTime.Now.AddMonths(-2))
                                {
                                    // file 变量是文件的完整路径
                                    File.Delete(file);
                                }
        ...
        public void ExportDataTableToExcel(System.Data.DataTable table, string fileName, out string filePath)
        {
            // 获取项目根目录（或你可换成指定目录）
        ...
        private void TmallInventorySync(object sender, ElapsedEventArgs e)
        {
            ...
            const string API_URL = "https://opengw.tmallyc.com/api/inventory/skuInventorySyncFacade/inventoryChange";
            ...
            try
            {
                ...
                {
                    HouseIDStr = "93,101,138,97,91,132,135,136,106",
                    TypeID = 34
                });
                ...
                foreach (var itemHouse in houseGroup)
                {
                    ...
                    for (int i = 0; i < totalBatches; i++)
                    {
                        ...
                        try
                        {
                            ...
                            {
                                successCount++;
                                log.Memo = $@"{currTIme}:天猫同步详情：返回参数:{jsonResponse}:" + $"仓库 {itemHouse.Key.ToString()} 批次 {i + 1}/{totalBatches} 成功 ({currentBatch.Count}条)";
                                logbus.InsertLog(log);
                            }
                            else
                            {
                    ...
                    if (failMessages.Count > 0)
                    {
                        ...
                        log.Status = "1";
                        log.Memo = $@"{currTIme}:仓库:[{itemHouse.Key}]:天猫同步完成详情:" + string.Join(",", failMessages);
                        logbus.InsertLog(log);
                    }
                }
        ...
        public Dictionary<string, string> logisticsList { get; set; } = new Dictionary<string, string>
{
    { "京东快递", "京东快递" },
    { "德邦快递", "德邦快递" },
    { "达达快递", "达达快递" },
    ...
    { "DFH", "东方汇" },
    { "YC", "远长" },
    { "DISTRIBUTOR_13484485", "顺心捷达" },
    { "DISTRIBUTOR_1710055", "邮政标准快递" },
    { "GZLT", "飞远配送" },
    { "WND", "WnDirect" },
    { "DISTRIBUTOR_30292473", "大食品商家自配" },
    { "DISTRIBUTOR_30464910", "丰网速运" },
    { "DISTRIBUTOR_13159132", "菜鸟大件-日日顺配" },
    { "DISTRIBUTOR_13148625", "菜鸟大件-中铁配" },
    { "PKGJWL", "派易国际物流77" },
    { "DISTRIBUTOR_13222803", "中通快运" },
    { "DISTRIBUTOR_30493846", "平安达腾飞快递" },
    { "DISTRIBUTOR_13211725", "跨越速运" },
    { "YUD", "长发" },
    { "DTW", "大田" },
    { "CYEXP", "长宇" },
    { "POST", "中国邮政" },
    ...
    { "THRID_DELIVERY_TMYCCGZPS", "自配送" },
        ...
        private void SendDeliveryNotice_Tmall(object sender, ElapsedEventArgs e)
        {
            ...
            const string APP_KEY = "2025120243201";
            const string APP_SECRET = "1d6c1fb8c356b5b405c674f4c572426d57362a9b";
            const string API_URL = "https://opengw.tmallyc.com/api/scfgoods/ycCloudOutboundFacade/stockOutByOriginNo";
            const int BATCH_DELAY_MS = 1500;
            ...
            try
            {
                ...
                foreach (var item in orderList)
                {
                    ...
                    try
                    {
                        ...
                        var outboundContact= item.logisticsName == "THRID_DELIVERY_TMYCCGZPS" ? item.DeliveryDriverName : "";
                        requestData.Add("outboundContact", outboundContact);
                        ...
                        //requestData.Add("logisticsName", logisticsList[item.logisticsName]  ?? string.Empty);
                        var key = item.logisticsName ?? string.Empty;
                        requestData.Add("logisticsName", logisticsList.TryGetValue(key, out var value) ? value : string.Empty);
                        ...
                        var phone = item.logisticsName == "THRID_DELIVERY_TMYCCGZPS" ? item.DeliveryDriverName : item.outboundContactPhone;
                        requestData.Add("syncType", item.syncType.ToString()); // int 转字符串
                        requestData.Add("remark", item.remark ?? string.Empty);
                        requestData.Add("logisticsCode", item.logisticsName ?? string.Empty);
                        requestData.Add("outboundContactPhone", phone ?? string.Empty);
        ...
        private void SendDeliveryNotice(object sender, ElapsedEventArgs e)
        {
            LogEntity log = new LogEntity();
            log.IPAddress = "";
            log.Moudle = "服务管理";
            ...
            {
                //获取开思订单未推送的订单
                var orderList = Bus.QueryNoDeliveryPushData();
                foreach (var item in orderList)
                {
                    var callbackData = JsonConvert.DeserializeObject<CassMallDTO>(item.shipments[0].cassMallDTO);
                    #region 开思订单审核
                    //SendOrderAudit(item, callbackData, log);
                    //log.NvgPage = "开思-发货通知";
                    //log.Status = "0";
                    #endregion
                    log.Status = "0";
                    log.Memo = $@"订单号[{item.orderId}]:请求参数:" + JsonConvert.SerializeObject(item);
                    logbus.InsertLog(log);
                    //获取订单信息
                    string url = CassApi;
                    var dataStr = PostHttpRequest_Post(url, JsonConvert.SerializeObject(item), "cassec.oms.shipment.callback"
                        , sessionDic[callbackData.pushTargetId]
                        , "gzshly60be866dbe52", log);
                    var dataString = JsonConvert.DeserializeObject<shipmentInfo>(dataStr);
                    //保存 推送记录
                    nwBus.AddCassMallData(new CargoCassMallEntity { SourceType = 0, SourceAction = "Callback", orderId = callbackData.data.orderId, ResJson = dataStr });

                    log.Status = "0";
                    log.Memo = $@"订单号[{item.orderId}]:开思发货完成，返回参数:" + dataStr;
                    logbus.InsertLog(log);

                    var resultData = JsonConvert.DeserializeObject<LogisticShipmentResponse>(dataStr);

                    if (resultData.Result[0].ShipmentDesc.Contains("订单已发货，不可重复发货") || resultData.Result[0].ShipmentDesc.Contains("发货成功"))
                    {
                        //回写订单推送状态
                        Bus.UpdateCassDeliveryPushStatus(item.orderId, "1");
                    }
                    // 每次循环结束后等待1秒（1000毫秒），兼容.NET 4.5.1
                    Thread.Sleep(1000);
                }
            }
            catch (WebException webEx)
            {
                log.Status = "1";
                log.Memo = $@"{currTIme}:开思HTTP请求异常:" + webEx.Message;
                logbus.InsertLog(log);
                if (webEx.Response is HttpWebResponse errorResponse)
                {
                    using (errorResponse)
                    ...
                    {
                        log.Status = "1";
                        log.Memo = $@"{currTIme}:开思错误响应:" + reader.ReadToEnd();
                        logbus.InsertLog(log);
                    }
                }
            }
            ...
            {
                log.Status = "1";
                log.Memo = $@"{currTIme}:开思推送异常:" + ex.ToString();
                logbus.InsertLog(log);
        ...
        /// 开思订单审核推送
        /// </summary>
        private void SendOrderAudit(CassMallPushParam order, CassMallDTO callbackData, LogEntity log)
        {
            LogBus logbus = new LogBus();
            try
            {
        ...
        public string PostHttpRequest_Get(string url, string Data, string apiName, string APISession, string appKey)
        {
            string result = string.Empty;
        ...
        public string PostHttpRequest_Post(string url, string Data, string apiName, string APISession, string appKey, LogEntity log)
        {
            LogBus logbus = new LogBus();
            string result = string.Empty;
            HttpWebRequest request = (HttpWebRequest)HttpWebRequest.Create(url);
            ...
            // 5. 传递POST请求体（业务数据Data）：将JSON字符串写入请求流
            if (!string.IsNullOrEmpty(Data))
            {
                byte[] postDataBytes = Encoding.UTF8.GetBytes(Data);
                // 设置请求体长度（必须，否则服务端可能无法正确解析）
            ...
            {
                // 若无业务数据，设置ContentLength为0（避免服务端报错）
            ...
            {
                // 捕获HTTP错误（如400/500），便于调试
                using (Stream errorStream = ex.Response?.GetResponseStream())
                using (StreamReader reader = new StreamReader(errorStream ?? Stream.Null, Encoding.UTF8))
                {
                    string errorMsg = reader.ReadToEnd();
                    log.Status = "1";
                    log.Memo = $@"开思推送异常：{ex.Message}，服务端返回：{errorMsg}";
                    logbus.InsertLog(log);
                    throw new Exception($"POST请求失败：{ex.Message}，服务端返回：{errorMsg}", ex);
                }
            }
        ...
        #region 预订单
        private void DelReserveOrder(object sender, ElapsedEventArgs e)
        {
            LogEntity log = new LogEntity();
            log.IPAddress = "";
            log.Moudle = "订单管理";
            log.Status = "0";
            log.NvgPage = "删除未支付预订单";
            log.UserID = "2029";
            log.Operate = "A";
            CargoOrderBus bus = new CargoOrderBus();
            LogBus logBus = new LogBus();
            ...
            try
            {
                List<CargoReserveOrderEntity> dataList = bus.QueryNoRerverOrder(new CargoReserveOrderEntity { });
                var batchSize = 10;
                int totalBatches = (dataList.Count + batchSize - 1) / batchSize;
                for (int i = 0; i < totalBatches; i++)
                {
                    var dataList2 = (dataList.Skip(i * batchSize).Take(batchSize).ToList());
                    bus.DelReserveOrder(dataList2);
                }
