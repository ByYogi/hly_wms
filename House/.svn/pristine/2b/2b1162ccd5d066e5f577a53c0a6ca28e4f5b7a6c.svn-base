...
namespace Cargo.House
{
    public partial class houseApi : BasePage
    {
        ...
        public void QueryALLHouseStockData()
        {
            ...
            {
                HouseName = "汇总：",
                Piece = list[0].TotalPiece,
                OverDueFee = list[0].TotalOnlyOverDayFee,
                InCargoStatus = ""
            });
            Hashtable resHT = new Hashtable();
            resHT["rows"] = list;
            resHT["total"] = list[0].TotalRowNum;
        ...
        public void SaveSafeStockData()
        {
            ...
            try
            {
                ...
                ent.MinStock = Convert.ToInt32(Request["MinStock"]);
                ent.MaxStock = Convert.ToInt32(Request["MaxStock"]);
        ...
        public void SaveStockTake()
        {
            ...
            {
                queryEntity.TypeID = Convert.ToInt32(Request["TypeID"]);
                queryEntity.TypeName = bus.QueryProductTypeName(new CargoProductTypeEntity { TypeID = Convert.ToInt32(Request["TypeID"]) });
            }
            try
            {
        ...
        public void QueryHouseShareList()
        {
            ...
            queryEntity.MainHouseID = Convert.ToInt32(Request["HouseID"]);
            CargoHouseBus bus = new CargoHouseBus();
        ...
        public void saveFile()
        {
            ...
            newData.Columns.Add("StockNum");
            newData.Columns.Add("MinStock");
            newData.Columns.Add("MaxStock");
            ...
            for (int i = 0; i < data.Rows.Count; i++)
            {
                ...
                {
                    if (!IsNumber(data.Rows[i][2].ToString().Trim()))
                    {
                        abnormalCount++;
                        msg += "第" + (i + 1) + "行数据最小库存数量有误\r\n";
                        continue;
                    }
                    data.Rows[i][2] = data.Rows[i][2].ToString().Trim();
                }
                else
                {
                    data.Rows[i][2] = 0;
                ...
                {
                    if (!IsNumber(data.Rows[i][3].ToString().Trim()))
                    {
                        abnormalCount++;
                        msg += "第" + (i + 1) + "行数据最大库存数量有误\r\n";
                        continue;
                    }
                    data.Rows[i][3] = data.Rows[i][3].ToString().Trim();
                }
                else
                {
                    data.Rows[i][3] = 0;
                ...
                int StockNum = Convert.ToInt32(Convert.ToString(data.Rows[i][4]).Trim());
                int MaxStock = Convert.ToInt32(Convert.ToString(data.Rows[i][3]).Trim());
                int MinStock = Convert.ToInt32(Convert.ToString(data.Rows[i][2]).Trim());
                string ProductCode = Convert.ToString(data.Rows[i][1]).Trim();
                ...
                else
                {
                    ...
                    TypeName = Convert.ToString(row[33]).Trim();
                    GoodsCode = Convert.ToString(row[3]).Trim();
                ...
                DataRow[] NewRows = newData.Select("ProductCode='" + ProductCode + "'");
            ...
            if (newData.Rows.Count > 0)
            {
                for (int i = 0; i < newData.Rows.Count; i++)
                {
                    CargoSafeStockEntity ent = new CargoSafeStockEntity();
                    ent.ExcelNo = i + 1;
                    ent.HouseID = Convert.ToString(newData.Rows[i][0]);
                    ent.HouseName = Convert.ToString(newData.Rows[i][1]).Trim();
                    ent.AreaID = Convert.ToInt32(newData.Rows[i][2]);
                    ent.AreaName = Convert.ToString(newData.Rows[i][3]);
                    ent.TypeID = Convert.ToInt32(newData.Rows[i][4]);
                    ent.TypeName = Convert.ToString(newData.Rows[i][5]).Trim();
                    ent.GoodsCode = Convert.ToString(newData.Rows[i][6]).Trim();
                    ent.Specs = Convert.ToString(newData.Rows[i][7]).Trim();
                    ent.Figure = Convert.ToString(newData.Rows[i][8]).Trim();
                    ent.LoadIndex = Convert.ToString(newData.Rows[i][9]).Trim();
                    ent.SpeedLevel = Convert.ToString(newData.Rows[i][10]).Trim();
                    ent.StockNum = Convert.ToInt32(newData.Rows[i][11]);
                    ent.MinStock = Convert.ToInt32(newData.Rows[i][12]);
                    ent.MaxStock = Convert.ToInt32(newData.Rows[i][13]);
                    ent.ProductCode = Convert.ToString(newData.Rows[i][14]).Trim();
                    ent.ProductName = Convert.ToString(newData.Rows[i][15]).Trim();
                    ent.Model = Convert.ToString(newData.Rows[i][16]).Trim();
        ...
        public void SaveImportData()
        {
            ...
            try
            {
                ...
                bus.SaveImportSafeStock(entityFiles, log);
