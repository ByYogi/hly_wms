...
SELECT
	...
	SUM (
		CASE
            ...
            -- 场景1：9个月周期跨前年和本年
			(
				a.BatchYear = SUBSTRING ( DATENAME( YEAR, DATEADD( MONTH, - 9, GETDATE( ) ) ), 3, 2 ) 
				AND a.BatchWeek > DATEPART( WEEK, DATEADD( MONTH, - 9, GETDATE( ) ) ) 
				AND SUBSTRING ( DATENAME( YEAR, DATEADD( MONTH, - 9, GETDATE( ) ) ), 3, 2 ) <> SUBSTRING ( DATENAME( YEAR, GETDATE( ) ), 3, 2 ) 
			) 
			OR 
            -- 场景2：9个月周期完全在本年（包含符合周数条件的记录）
			(
				a.BatchYear = SUBSTRING ( DATENAME( YEAR, GETDATE( ) ), 3, 2 ) 
				AND (
                -- 子条件：要么是跨年后的本年所有记录，要么是同年内周数符合条件的记录
					(
						SUBSTRING ( DATENAME( YEAR, DATEADD( MONTH, - 9, GETDATE( ) ) ), 3, 2 ) <> SUBSTRING ( DATENAME( YEAR, GETDATE( ) ), 3, 2 ) 
					) 
					OR ( a.BatchWeek > DATEPART( WEEK, DATEADD( MONTH, - 9, GETDATE( ) ) ) ) 
				) 
				) THEN
				b.Piece ELSE 0 
			END
		...
		FROM
			...
			INNER JOIN Tbl_Cargo_Area ca2 ON d.ParentID = ca2.AreaID
			INNER JOIN Tbl_Cargo_Area ca3 ON ca2.ParentID = ca3.AreaID 
		WHERE
			a.InCargoStatus = '1' 
			AND a.IsLockStock = 0 
			AND b.Piece <> 0 
            ...
            strSQL += "GROUP BY a.HouseID,ca3.Name ";
            strSQL = $@"

WITH ESInQty AS(
SELECT 
    HouseID,
    SUM(CASE WHEN CAST(ETATime AS DATE) = CAST(GETDATE() AS DATE) 
             THEN (ReplyNumber - fo.InPiece) ELSE 0 END) AS QtyToday,
    SUM(CASE WHEN CAST(ETATime AS DATE) = DATEADD(DAY, 1, CAST(GETDATE() AS DATE)) 
             THEN (ReplyNumber - fo.InPiece) ELSE 0 END) AS QtyTomorrow,
    SUM(CASE WHEN CAST(ETATime AS DATE) = DATEADD(DAY, 2, CAST(GETDATE() AS DATE)) 
             THEN (ReplyNumber - fo.InPiece) ELSE 0 END) AS QtyDayAfterTomorrow
FROM Tbl_Cargo_FactoryOrder fo
WHERE ETATime IS NOT NULL 
  AND ActualArrivalTime IS NULL 
  AND CAST(ETATime AS DATE) >= CAST(GETDATE() AS DATE)
  AND ProductCode IN('{entity.ProductCode}')
  AND (ReplyNumber - fo.InPiece) > 0
GROUP BY HouseID
)
, InventoryCycle AS (
{strSQL}
...
SELECT * FROM InventoryCycle ic
LEFT JOIN ESInQty eq ON ic.HouseID = eq.HouseID
ORDER BY Stock DESC,Piece DESC
";
            using (DbCommand command = conn.GetSqlStringCommond(strSQL))
            {
                using (DataTable dt = conn.ExecuteDataTable(command))
                {
                    foreach (DataRow idr in dt.Rows)
                    {
                        result.Add(new CargoGtmcProOrderDetailEntity
                        {
                            ...
                            QtyToday = idr.Field<int?>("QtyToday") ?? 0,
                            QtyTomorrow = idr.Field<int?>("QtyTomorrow") ?? 0,
                            QtyDayAfterTomorrow = idr.Field<int?>("QtyDayAfterTomorrow") ?? 0,
                        });
                    }
                }
            }
        ...
        public void SaveGTMCImportData(CargoGtmcProOrderEntity entity)
        {
            entity.EnSafe();
            Int64 did = 0;
            string strSQL = @"INSERT INTO Tbl_GTMC_ProOrder(TakeNo,GtmcNo,SourceCode,SourceName,OrderOwner,OrderOwnerName,OrderStatus,OrderNo,HouseID,OPID,OPName";
            ...
            strSQL += ",BusinessID,Piece,Route,FrequentNo,IncomeNo,MainRoute,SplitLine,FactoryNo,OrderFileName,OrderType,Memo) VALUES (@TakeNo,@GtmcNo,@SourceCode,@SourceName,@OrderOwner,@OrderOwnerName,@OrderStatus,@OrderNo,@HouseID,@OPID,@OPName";
            if ((entity.TakeTime.ToString("yyyy-MM-dd") != "0001-01-01" && entity.TakeTime.ToString("yyyy-MM-dd") != "1900-01-01"))
            {
                strSQL += " ,@TakeTime";
            }
            ...
            try
            {
                ...
                using (DbCommand cmd = conn.GetSqlStringCommond(strSQL))
                {
                    ...
                    conn.AddInParameter(cmd, "@OrderOwner", DbType.Byte, entity.OrderOwner.HasValue ? (object)entity.OrderOwner.Value : DBNull.Value);
                    conn.AddInParameter(cmd, "@OrderOwnerName", DbType.String, !string.IsNullOrEmpty(entity.OrderOwnerName) ? (object)entity.OrderOwnerName : DBNull.Value);
                    conn.AddInParameter(cmd, "@OrderStatus", DbType.String, entity.OrderStatus);
                    conn.AddInParameter(cmd, "@OrderNo", DbType.String, entity.OrderNo);
        ...
        public Dictionary<string, string> QueryOrderOwnerBySourceCodes(string[] sourceCodes)
        {
...
";
                string strSQL = @"
SELECT
	DISTINCT b.UpClientShortName,
	a.SourceCode
FROM
	Tbl_GTMC_ProOrder a
	INNER JOIN (
		SELECT * FROM Tbl_Cargo_Client
		WHERE
			ShopCode IS NOT NULL
			AND ShopCode <> ''
			AND ShopCode <> '0'
			AND UpClientShortName <> '0'
			AND ISNULL(UpClientShortName, '') <> ''
	) b ON a.SourceCode = b.ShopCode
INNER JOIN ##IDs IDs ON IDs.ID = a.SourceCode";
                List<SqlParameter> sqlParameters = new List<SqlParameter>();
                int batchSize = 1000;
                if (!sourceCodes.Any()) { return result; }

                var groupedPrdctIDs = sourceCodes.Select((row, index) => new { row, index })
...
";
                    ...
                    using (DataTable dt = conn.ExecuteDataTable(cmd))
                    {
                        foreach (DataRow idr in dt.Rows)
                        {
                            var sourceCode = idr.Field<string>("SourceCode");
                            var upClientShortName = idr.Field<string>("UpClientShortName");
                            if(!result.Keys.Any(c => c == sourceCode))
                            {
                                result.Add(sourceCode, upClientShortName);
                            }
                        }
                    }
                }
        ...
        public Hashtable QueryBatchImportOrderData(int pIndex, int pNum, CargoGtmcProOrderEntity entity)
        {
            ...
            try
            {
                ...
                if (!entity.LastMonth.Equals(0))
                {
                    ...
                    {
                        strSQL += " AND ((p.BatchWeek>DATEPART(week,DATEADD(month," + (entity.LastMonth * -1) + ",getdate())) AND p.BatchYear=SUBSTRING(DateName(year,dateadd(month," + (entity.LastMonth * -1) + ",GetDate()) ),3,2)) or p.BatchYear=SUBSTRING(DateName(year,dateadd(YEAR,0,GetDate()) ),3,2))";
                    }
                }
                ...
                if (!string.IsNullOrEmpty(entity.OrderOwnerName)) { strSQL += " AND po.OrderOwnerName like '%" + entity.OrderOwnerName?.Trim() + "%'"; }
                ...
                strSQL += ") as A where RowNumber > (" + pNum + "* (" + pIndex + $"-1)) AND A.RowNumber <= ({pNum} * {pIndex})";
                strSQL += "\r\n ORDER BY RowNumber OPTION (HASH JOIN)";
                using (DbCommand command = conn.GetSqlStringCommond(strSQL))
                {
                    #region 获取数据
                    using (DataTable dd = conn.ExecuteDataTable(command))
                    {
                        foreach (DataRow idr in dd.Rows)
                        {
                            result.Add(new CargoGtmcProOrderEntity
                            {
                                ...
                                OrderOwnerName = idr.Field<string>("OrderOwnerName"),
                                ExterOrderAlloNum = string.IsNullOrEmpty(Convert.ToString(idr["ExterOrderAlloNum"])) ? 0 : Convert.ToInt32(idr["ExterOrderAlloNum"]),
                            });
                        }
                    }
                ...
                if (!entity.LastMonth.Equals(0))
                {
                    ...
                    {
                        strCount += " AND ((p.BatchWeek>DATEPART(week,DATEADD(month," + (entity.LastMonth * -1) + ",getdate())) AND p.BatchYear=SUBSTRING(DateName(year,dateadd(month," + (entity.LastMonth * -1) + ",GetDate()) ),3,2)) or p.BatchYear=SUBSTRING(DateName(year,dateadd(YEAR,0,GetDate()) ),3,2))";
                    }
                }
        ...
        public Hashtable QueryData(int pIndex, int pNum, CargoFactoryOrderEntity entity)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(entity.FacOrderNo)) { strSQL += $@" and a.FacOrderNo in({entity.FacOrderNo})"; }
                ...
                if (!string.IsNullOrEmpty(entity.OP_Name)) { strSQL += " and a.OP_Name like '%" + entity.OP_Name + "%'"; }
                ...
                if ((entity.ETAStartDate.ToString("yyyy-MM-dd") != "0001-01-01" && entity.ETAStartDate.ToString("yyyy-MM-dd") != "1900-01-01"))
                {
                    strSQL += " and a.ETATime>='" + entity.ETAStartDate.ToString("yyyy-MM-dd") + "'";
                }
                if ((entity.ETAEndDate.ToString("yyyy-MM-dd") != "0001-01-01" && entity.ETAEndDate.ToString("yyyy-MM-dd") != "1900-01-01"))
                {
                    strSQL += " and a.ETATime<'" + entity.ETAEndDate.AddDays(1).ToString("yyyy-MM-dd") + "'";
                }
                ...
                if (!string.IsNullOrEmpty(entity.FacOrderNo)) { strCount += $@" and a.FacOrderNo in({entity.FacOrderNo})"; }
    ...
    @TradePrice,@SaleMoney,@ReceiveName,@ReceiveCity,@ReceiveMobile,@BelongMonth,@Born,@Assort,@Source,@ProductName,@WhetherTax,@HLYReturnID,@SourceHouse,@OP_DATE,@OP_Name,@BelongDepart,@Company,@SpecsType,@Supplier,@SuppClientNum,@SupplierAddress,@ProductCode,@BusinessID,@OwnerShip,@PurchaseSupplier)";
            ...
            try
            {
                using (DbCommand cmd = conn.GetSqlStringCommond(strSQL))
                {
                    ...
                    if (entity.BelongMonth.Length<=0)
                    {
                        entity.BelongMonth = DateTime.Now.ToString("yyyyMM");
                    }
        ...
        {
            string inSQL = @"update Tbl_Cargo_FactoryOrder set ActualArrivalTime=iif(isnull(ActualArrivalTime,'')='',getdate(),ActualArrivalTime) where ID=@ID";
            try
            {
                using (DbCommand cmd = conn.GetSqlStringCommond(inSQL))
                {
