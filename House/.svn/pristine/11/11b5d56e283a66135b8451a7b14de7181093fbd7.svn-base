    ...
    public class CargoProductBus
    {
        ...
        public void Unshelves(List<CargoProductShelvesEntity> entity, LogEntity log)
        {
            ...
            try
            {
                ...
                foreach (var it in entity)
                {
                    log.Memo = "下架商品：商品ID：" + it.ID.ToString() + "，产品名称：" + it.ProductName.Trim() + "，类型ID：" + it.TypeID.ToString() + "，型号：" + it.Model.Trim() + "，货品代码：" + it.GoodsCode.Trim() + "，花纹：" + it.Figure + "，数量：" + it.OnSaleNum.ToString() + "，产品ID：" + it.ProductID.ToString();
                    lw.WriteLog(log);
                }
            }
            catch (ApplicationException ex)
