    ...
    public partial class Service1 : ServiceBase
    {
        ...
        void RemoveTimeoutUnpaidOrder_Elapsed(object sender, System.Timers.ElapsedEventArgs e)
        {
            ...
            try
            {
                ...
                //取最新一条同步数据
                saleOrderStatusOpenQueryResp queryResp = nwBus.QuerySaleOrderStatusOpenData(new saleOrderStatusOpenQueryResp { });
                ...
                if (res.Count > 0)
                {
                    ...
                    for (int i = 0; i < res.Count; i++)
                    {
                        ...
                        entity.data.Add(new Data
                        {
                            ...
                            ProductName = res[i].ProductName,
                            TypeName = typename,
                            SalePrice = Convert.ToDecimal(res[i].SalePrice) + Convert.ToDecimal(15),
                            StockNum = res[i].StockNum,
                            Batch = res[i].Batch,
                            HouseID = res[i].HouseID,//汕头仓+ Convert.ToDecimal(GetContiPrice())
                        });
        ...
        private void AutoUnShelve()
        {
            ...
            List<CargoProductShelvesEntity> shelve = new List<CargoProductShelvesEntity>();
