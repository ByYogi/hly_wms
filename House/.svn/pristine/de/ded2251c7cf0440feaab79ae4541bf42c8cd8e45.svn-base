<%@ Page Title="线网关系" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="LineNetMapping.aspx.cs" Inherits="Cargo.House.LineNetMapping" %>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            $('#dg').datagrid({
                ...
                toolbar: '#toolbar',
                columns: [[
                    { title: '', field: 'MID', checkbox: true, width: '5%' },
                    //{
                    //    title: '所在省', field: 'Province', width: '8%', formatter: function (value) {
                    //        return "<span title='" + value + "'>" + value + "</span>";
                    //    }
                    //},
                    //{
                    //    title: '城市名称', field: 'City', width: '8%', formatter: function (value) {
                    //        return "<span title='" + value + "'>" + value + "</span>";
                    //    }
                    //},
                    {
                    ...
                    {
                        title: '客户名称', field: 'ClientShortName', width: '15%', formatter: function (value) {
                            return "<span title='" + value + "'>" + value + "</span>";
                        }
                    },
                    {
                        title: '店代码', field: 'ShopCode', width: '7%', formatter: function (value) {
                            return "<span title='" + value + "'>" + value + "</span>";
                        }
                    },
                    {
                        title: '出库仓库', field: 'AreaName', width: '7%', formatter: function (value) {
                            return "<span title='" + value + "'>" + value + "</span>";
                        }
                    },
                    {
                        title: '出库级别', field: 'OrderLevel', width: '8%', formatter: function (value) {
                            return "<span title='" + value + "'>" + value + "</span>";
                        }
                    },
            ...
            $('#AHouseID').combobox({
                ...
                onSelect: function (rec) {
                    ...
                    $('#AClientNum').combobox('textbox').bind('focus', function () { $('#AClientNum').combobox('showPanel'); });
                    $('#AClientNum').combobox({
                        url: '../Client/clientApi.aspx?method=AutoCompleteClient&HouseID=' + rec.HouseID
                    });
                }
            });
            $('#AClientNum').combobox('clear');
            var url = '../Client/clientApi.aspx?method=AutoCompleteClient&HouseID=<%=UserInfor.HouseID%>';
            $('#AClientNum').combobox('reload', url);
            ...
            //    onSelect: function (rec) {
            //        $('#ACity').combobox('textbox').bind('focus', function () { $('#ACity').combobox('showPanel'); });
            //        $('#ACity').combobox('clear');
            //        var url = '../House/houseApi.aspx?method=QueryCityData&PID=' + rec.ID;
            //        $('#ACity').combobox('reload', url);
            //    }
            //});
            //$('#AProvince').combobox('textbox').bind('focus', function () { $('#AProvince').combobox('showPanel'); });
        ...
        function dosearch() {
            ...
            gridOpts.url = 'houseApi.aspx?method=QueryLineNetMapp';
            $('#dg').datagrid('load', {
                ClientNum: $('#AClientNum').combobox('getValue'),
                ShopCode: $('#AShopCode').textbox('getValue'),
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <div id="saPanel" name="SelectDiv1" class="easyui-panel" title="" data-options="iconCls:'icon-search'" style="width: 100%">
        <table>
            <tr>
                <%--<td style="text-align: right;">所在省:</td>
                <td>
                    <input id="AProvince" class="easyui-combobox" style="width: 120px;" />
                </td>
                <td style="text-align: right;">城市名称:</td>
                <td>
                    <input id="ACity" class="easyui-combobox" style="width: 120px;" data-options="valueField:'City',textField:'City'" />

                </td>--%>
                <td style="text-align: right;">区域大仓:
                </td>
                <td>
                    <input id="AHouseID" class="easyui-combobox" style="width: 120px;" />
                </td>
                 <td style="text-align: right;">客户名称:</td>
                <td>
                    <input id="AClientNum" class="easyui-combobox" style="width: 180px;" data-options="valueField:'ClientNum',textField:'ClientShortName'" />
                </td>
                 <td style="text-align: right;">店代码:</td>
                <td>
                    <input id="AShopCode" class="easyui-textbox" style="width: 120px;" />
                </td>
