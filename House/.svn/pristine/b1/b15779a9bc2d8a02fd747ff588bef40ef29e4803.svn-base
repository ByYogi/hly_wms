...
namespace House.Business.Cargo
{
    ...
    public class CargoReportBus
    {
        ...
        public Dictionary<string, object> HCYCStatistics(CargoDLTDataStatisEntity entity)
        {
            ...
            #endregion
            #region 统计订单
            List<CargoDLTDataStatisEntity> Order = man.HCYC_Order_Statistics(new CargoDLTDataStatisEntity() { HouseID = entity.HouseID });
            if (Order.Count > 0)
            {
            ...
            {
                Date = Convert.ToDateTime(a.Key).ToString("MM-dd"),
                Num = a.Count(),
        ...
        public Dictionary<string, object> HCYCStatisticsData(CargoDLTDataStatisEntity entity)
        {
            ...
            #endregion
            #region 统计订单
            List<CargoDLTDataStatisEntity> Order = man.HCYC_Order_StatisticsData(new CargoDLTDataStatisEntity() { StartDate = entity.StartDate, EndDate = entity.EndDate });
            if (Order.Count > 0)
            {
                Dictionary<string, object> OrderKey = new Dictionary<string, object>
                {
        ...
        public List<CargoHouseAimEntity> QueryHCYC_House(CargoHouseAimEntity entity)
        {
            ...
            foreach (var item in List)
            {
                if (item.AimQuantity>0)
                    item.IsCompletedAim = Convert.ToInt32((item.AimQuantity / 30 * days))<= item.Piece;
            }
        ...
        public List<CargoOrderDayStatisticsEntity> QueryHCYC_BrandData(CargoHouseAimEntity entity)
        {
            ...
            foreach (var item in List)
            {
                ...
                item.SalesRate = 0;
                if (item.ProCount > 0 && item.DayStatisticsCount > 0)
                    item.SalesRate = Math.Round(Convert.ToDouble((item.DayStatisticsCount)) / Convert.ToDouble(item.ProCount) * 100, 1);
