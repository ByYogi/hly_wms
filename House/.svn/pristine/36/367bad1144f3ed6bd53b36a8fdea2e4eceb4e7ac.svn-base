...
namespace House.Manager.Cargo
{
    ...
    public class CargoInterfaceManager
    {
            ...
            for (int i = 0; i < totalBatches; i++)
            {
                ...
                foreach (var entity in batch)
                {
                    sqlBuilder.AppendLine(
                        $"UPDATE Tbl_Cargo_ContainerGoods SET Piece={entity.Piece} WHERE ProductID={entity.ProductID};");

                    sqlBuilder.AppendLine(
                        $"UPDATE Tbl_Cargo_Product SET " +
                        $"SalePrice={entity.SalePrice}, InHousePrice={entity.InHousePrice}, UnitPrice={entity.UnitPrice} " +
        ...
        public List<CargoProductEntity> QueryNextDayStockSync(CargoProductEntity entity)
        {
            ...
            strSQL += " inner join Tbl_Cargo_HouseBrandShare as m on b.HouseID=m.ShareHouseID and b.TypeID=m.BrandID and m.DelFlag=0) as t where t.RN=1  ";
            using (DbCommand command = conn.GetSqlStringCommond(strSQL))
            {
                using (DataTable dd = conn.ExecuteDataTable(command))
                {
                    foreach (DataRow idr in dd.Rows)
                    {
                        result.Add(new CargoProductEntity
                        {
                            ...
                            SalePrice = Convert.ToDecimal(idr["SalePrice"]),//销售价
        ...
        public DataTable ConvertToProductDataTable(List<CargoProductEntity> list)
        {
            DataTable table = new DataTable();

            // 定义列，名称和类型需和数据库表 Tbl_Cargo_Product 对应
            table.Columns.Add("Supplier", typeof(string));
            table.Columns.Add("SuppClientNum", typeof(long));
            table.Columns.Add("SpecsType", typeof(string));
            table.Columns.Add("ProductCode", typeof(string));
            table.Columns.Add("ProductName", typeof(string));
            table.Columns.Add("HouseID", typeof(int));
            table.Columns.Add("Specs", typeof(string));
            table.Columns.Add("Figure", typeof(string));
            table.Columns.Add("LoadIndex", typeof(string));
            table.Columns.Add("SpeedLevel", typeof(string));
            ...
            table.Columns.Add("TreadWidth", typeof(int));
            table.Columns.Add("FlatRatio", typeof(int));
            table.Columns.Add("Meridian", typeof(string));
            table.Columns.Add("HubDiameter", typeof(int));
            table.Columns.Add("SpeedMax", typeof(int));
            table.Columns.Add("Weight", typeof(decimal));
            table.Columns.Add("UnitPrice", typeof(decimal));
            table.Columns.Add("TradePrice", typeof(decimal));
            table.Columns.Add("InHousePrice", typeof(decimal));
            table.Columns.Add("PackageNum", typeof(int));
            table.Columns.Add("PackageWeight", typeof(decimal));
            table.Columns.Add("BatchYear", typeof(int));
            table.Columns.Add("BatchWeek", typeof(int));
            table.Columns.Add("OP_DATE", typeof(DateTime));
            table.Columns.Add("Source", typeof(string));
            table.Columns.Add("BelongMonth", typeof(string));
            table.Columns.Add("OperaType", typeof(string));
            table.Columns.Add("Born", typeof(string));
            table.Columns.Add("Assort", typeof(string));
            table.Columns.Add("BelongDepart", typeof(string));
            table.Columns.Add("Company", typeof(string));
            table.Columns.Add("InHouseTime", typeof(string));
            table.Columns.Add("InCargoStatus", typeof(string));
            table.Columns.Add("ShareHouseID", typeof(int));
            ...
            foreach (var entity in list)
            {
                ...
                row["SuppClientNum"] = entity.SuppClientNum;
                row["SpecsType"] = entity.SpecsType ?? "";
                row["ProductCode"] = entity.ProductCode ?? "";
                row["ProductName"] = (entity.TypeName ?? "") + "轮胎";
                row["HouseID"] = entity.HouseID;
                row["Specs"] = entity.Specs ?? "";
                row["Figure"] = entity.Figure ?? "";
                row["LoadIndex"] = entity.LoadIndex ?? "";
                row["SpeedLevel"] = entity.SpeedLevel ?? "";
                row["SalePrice"] = entity.MiniProPrice;
                row["Batch"] = entity.Batch ?? "";
                row["Numbers"] = entity.Numbers;
                row["TypeID"] = entity.TypeID;
                row["Model"] = entity.Model ?? "";
                row["GoodsCode"] = entity.GoodsCode ?? "";
                row["TreadWidth"] = entity.TreadWidth;
                row["FlatRatio"] = entity.FlatRatio;
                row["Meridian"] = "R";  // 默认
                row["HubDiameter"] = entity.HubDiameter;
                row["SpeedMax"] = entity.SpeedMax;
                row["Weight"] = 0m;  // 默认
                row["UnitPrice"] = entity.InHousePrice;
                row["TradePrice"] = entity.MiniProPrice;
                row["InHousePrice"] = entity.InHousePrice;
                row["PackageNum"] = entity.PackageNum;
                row["PackageWeight"] = entity.PackageWeight;

                // 防止字符串长度不够导致异常
                int batchYear = 0;
                int batchWeek = 0;
                if (!string.IsNullOrEmpty(entity.Batch) && entity.Batch.Length >= 4)
                {
                    int.TryParse(entity.Batch.Substring(2, 2), out batchYear);
                    int.TryParse(entity.Batch.Substring(0, 2), out batchWeek);
                }
                row["BatchYear"] = batchYear;
                row["BatchWeek"] = batchWeek;

                row["OP_DATE"] = DateTime.Now;
                row["Source"] = "83";
                row["BelongMonth"] = entity.BelongMonth ?? "";
                row["OperaType"] = "0";
                row["Born"] = entity.Born ?? "";
                row["Assort"] = entity.Assort ?? "";
                row["BelongDepart"] = "1";
                row["Company"] = "0";
                row["InHouseTime"] = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
                row["InCargoStatus"] = entity.InCargoStatus ?? "";
                row["ShareHouseID"] = entity.ShareHouseID;
        ...
        {
            var dataTable = ConvertToProductDataTable(products);
            ...
            CREATE TABLE #TempProducts (
                ...
                Numbers INT, TypeID INT, Model NVARCHAR(100), GoodsCode NVARCHAR(100),
                ...
                UnitPrice DECIMAL(18,2), TradePrice DECIMAL(18,2), InHousePrice DECIMAL(18,2),
            ...
            (
                ...
                LoadIndex, SpeedLevel, SalePrice, Batch, Numbers, TypeID, Model, GoodsCode,
                TreadWidth, FlatRatio, Meridian, HubDiameter, SpeedMax, Weight,
                UnitPrice, TradePrice, InHousePrice,
                PackageNum, PackageWeight, BatchYear, BatchWeek,
                OP_DATE, Source, BelongMonth, OperaType, Born, Assort,
        ...
        ";
                ...
                using (var reader = cmd.ExecuteReader())
                {
                    ...
                    {
                        // 给原始实体赋值主键
                        var entity = products[index];
                        entity.ProductID = reader.GetInt64(0);
