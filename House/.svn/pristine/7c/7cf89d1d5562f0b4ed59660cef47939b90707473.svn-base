...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
   ...
   <td colspan="2" style="color: #ad2859; padding-left: 55px;">提示：小程序价=进仓价+进仓价*0.9%</td>
                    ...
                    <tr>
                        ...
                        <td colspan="2" style="color: #ad2859; padding-left: 55px;">提示：小程序价=进仓价+进仓价*交易通道费定价</td>
                    </tr>
                    <tr>
    <td colspan="2" style="color: #ad2859; padding-left: 55px;">交易通道费具体定价请询问客服</td>
</tr>--%>
                </table>
            </form>
        </div>
    ...
    <script type="text/javascript">
        ...
        //    var typeBoxValue = $('#TypeID').numberbox('getValue');
        //    var TypeID = parseInt(typeBoxValue || 0, 10);
        //    console.log()
        //    var salePrice = Number($('#InHousePrice').numberbox('getValue')) + Number($('#InHousePrice').numberbox('getValue') * OtherFeeRatio.Ratio(TypeID));
        ...
        //输入进仓价后 自动计算小程序价
        function qh() {
            var inHousePrice = Number($('#InHousePrice').numberbox('getValue'));
            var typeBoxValue = $('#TypeID').val();
            var TypeID = parseInt(typeBoxValue || 0, 10);
            if (isNaN(inHousePrice) || inHousePrice <= 0) {
            ...
            $.ajax({
                ...
                success: function (result) {
                    var salePrice = inHousePrice + (inHousePrice * result);
                    var newSalePrice = Math.ceil(salePrice);
                    $('#SalePrice').numberbox('setValue', newSalePrice);
                },
                error: function (err) {
                    console.error("获取费率失败：", err);
                    var defaultRatio = 0.025;
                    var salePrice = inHousePrice + (inHousePrice * defaultRatio);
                    var newSalePrice = Math.ceil(salePrice);
                    $('#SalePrice').numberbox('setValue', newSalePrice);
                }
            });
