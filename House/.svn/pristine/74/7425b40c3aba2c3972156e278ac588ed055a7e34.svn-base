<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="qyMyApprove.aspx.cs" Inherits="Cargo.QY.qyMyApprove" %>
    ...
    <script type="text/javascript">
        ...
        function ajaxpage(page) {
            $.ajax({
                ...
                url: 'qyServices.aspx?method=QueryUnCheckMyOrderModify',
                ...
                success: function (rs) {
                    ...
                    for (var i = 0; i < rs.rows.length; i++) {
                        ...
                        $('#ltlUnCheck').append("<div class=\"weui-panel__bd\" style=\"width:100%;border-bottom: #e4dfdf 1px solid;\"><div class=\"weui-media-box weui-media-box_text\" style=\"padding-top: 3px;padding-bottom: 3px;\"><h4 class=\"weui-media-box__title\" style=\"font-size:15px;margin-bottom: 3px;\">所属仓库：<em class=\"num\">" + rs.rows[i].HouseName + "----" + oty + "</em></h4><p class=\"weui-media-box__desc\">订单号：" + rs.rows[i].OrderNo + "</p><p class=\"weui-media-box__desc\">业务员：" + rs.rows[i].SaleManName + "</p><p class=\"weui-media-box__desc\">改价原因：" + rs.rows[i].Reason + "</p><p class=\"weui-media-box__desc\">申请时间：" + rs.rows[i].ApplyName + " " + DateTimeFormatter(rs.rows[i].ApplyDate) + "</p></div></div>")
                    }
        ...
        function getCheck(page) {
            $.ajax({
                type: "POST", dataType: "json",
                url: 'qyServices.aspx?method=QueryCheckMyOrderModify',
                data: { page: page, rows: 10 },
                beforeSend: function () {
                    $("#loadingCheck").show();
                },
