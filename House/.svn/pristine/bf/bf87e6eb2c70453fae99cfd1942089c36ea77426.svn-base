...
namespace House.Manager.Cargo
{
    ...
    public class CargoRealFactoryPurchaseOrderManager
    {
        ...
        public void AddRealPurchaseOrder(CargoRealFactoryPurchaseOrderEntity entity, CargoReserveParam Reserve = null)
        {
            Int64 PurOrderID = 0;
            ...
            if (Reserve != null && Reserve.isReserver) AddReserveProcureMentOrder(PurOrderID, Reserve);
        ...
        /// </summary>
        /// <param name="entity"></param>
        public void AddReserveProcureMentOrder(Int64 PurOrderID, CargoReserveParam cargo)
        {
            #region 订单保存
            StringBuilder sb = new StringBuilder();

            foreach (var item in cargo.ReserveOrderNo)
            {
                sb.AppendLine($@";insert into Tbl_Cargo_ReserveProcurementOrder(reserveorderno, purorderid, purorderno) select '{item}',PurOrderID,PurOrderNo from Tbl_Cargo_RealFactoryPurchaseOrder where PurOrderID={PurOrderID}");

            }

            using (DbCommand cmd = conn.GetSqlStringCommond(sb.ToString()))
            {
        ...
        public Hashtable QueryCargoRealPurchase(int pIndex, int pNum, CargoRealFactoryPurchaseOrderEntity entity)
        {
            ...
            #region 获取订单明细
            string strSQL = @" SELECT TOP " + pNum + $@"* FROM (select * from ( select ROW_NUMBER() OVER(ORDER BY CreateDate DESC) AS RowNumber,*,iif(PurchaseType=1 and PurchaseInStoreType=2,1,(case when InCargoQty <= 0 then 0 when InCargoQty = fInCargoQty then 1 when fInCargoQty > 0 then 2 else 0 end)) as PurchaseInStoreState from ( SELECT DISTINCT a.*,b.PurchaserName,b.PurchaserShortName, b.HouseID,b.Telephone,
                      ...
                      isnull((select sum(isnull(Piece,0))
                                     from Tbl_Cargo_RealFactoryPurchaseOrderGoods as bb
                                              inner join Tbl_Cargo_FactoryOrder as c
                                                        on c.FacOrderNo = a.PurOrderNo and c.ProductCode = bb.ProductCode and a.PurOrderID=bb.PurOrderID),0) InCargoQty --获取入库总数
                        ,(case when (select isnull(count(1),0) from Tbl_Cargo_FactoryFile as files where files.FacOrderNo = a.PurOrderNo)>0 then 1 else 0 end) IsDocument,bb.PurGoodsCount,c.HouseID as BHouseID,e.Name as BHouseName
                        FROM Tbl_Cargo_RealFactoryPurchaseOrder as a inner join Tbl_Cargo_Purchaser as b on a.PurchaserID=b.PurchaserID inner join (select PurOrderID,count(1) as PurGoodsCount from Tbl_Cargo_RealFactoryPurchaseOrderGoods as bb group by bb.PurOrderID) as bb on a.PurOrderID=bb.PurOrderID 
                  left join Tbl_Cargo_RealFactoryPurchaseHouse as c on a.PurOrderID=c.PurOrderID
                           left join Tbl_Cargo_House as e on c.HouseID=e.HouseID
WHERE (1=1)";
            ...
            if (((entity.ETATimeStart.ToString("yyyy-MM-dd") != "0001-01-01" && entity.ETATimeStart.ToString("yyyy-MM-dd") != "1900-01-01")) || ((entity.ETATimeEnd.ToString("yyyy-MM-dd") != "0001-01-01" && entity.ETATimeEnd.ToString("yyyy-MM-dd") != "1900-01-01")))
            {
                strSQL += $@"and exists (select 1 from Tbl_Cargo_RealFactoryPurchaseHouse as ry where ry.PurOrderID=a.PurOrderID ";
                if ((entity.ETATimeStart.ToString("yyyy-MM-dd") != "0001-01-01" && entity.ETATimeStart.ToString("yyyy-MM-dd") != "1900-01-01"))
                {
                    strSQL += " and ry.ETATime>='" + entity.ETATimeStart.ToString("yyyy-MM-dd") + "'";
                }
                if ((entity.ETATimeEnd.ToString("yyyy-MM-dd") != "0001-01-01" && entity.ETATimeEnd.ToString("yyyy-MM-dd") != "1900-01-01"))
                {
                    strSQL += " and ry.ETATime<'" + entity.ETATimeEnd.AddDays(1).ToString("yyyy-MM-dd") + "'";
                }
                strSQL += $@")";
            }
            if (!string.IsNullOrEmpty(entity.CargoPermisID))
            {
                strSQL += $@" and exists(select 1 from Tbl_Cargo_RealFactoryPurchaseHouse as ph where ph.HouseID in({entity.CargoPermisID}) and ph.PurOrderID=a.PurOrderID)";
            }

            if (!string.IsNullOrEmpty(entity.TypeName) || !string.IsNullOrEmpty(entity.GoodsCode) || !string.IsNullOrEmpty(entity.Specs) || !string.IsNullOrEmpty(entity.Figure))
            {
                strSQL += $@"
                and exists (
                select * from Tbl_Cargo_RealFactoryPurchaseOrderGoods as goods
                         inner join Tbl_Cargo_ProductSpec as ps on goods.ProductCode=ps.ProductCode
                         inner join Tbl_Cargo_ProductType as pt on goods.TypeID=pt.TypeID
                         where a.PurOrderID=goods.PurOrderID
                ";
                if (!string.IsNullOrEmpty(entity.TypeName)) strSQL += $@" and pt.TypeName like '%{entity.TypeName}%'";
                if (!string.IsNullOrEmpty(entity.GoodsCode)) strSQL += $@" and ps.GoodsCode like '%{entity.GoodsCode}%'";
                if (!string.IsNullOrEmpty(entity.Specs)) strSQL += $@" and ps.Specs like '%{entity.Specs}%'";
                ...
                using (DataTable dd = conn.ExecuteDataTable(command))
                {
                    foreach (DataRow idr in dd.Rows)
                    {
                        result.Add(new CargoRealFactoryPurchaseOrderEntity
                        {
                            ...
                            PurDepartID = idr["PurDepartID"] == DBNull.Value ? 0 : Convert.ToInt32(idr["PurDepartID"]),
                            ReplyNum = idr["ReplyNum"] == DBNull.Value ? 0 : Convert.ToInt32(idr["ReplyNum"]),
                            Piece = idr["Piece"] == DBNull.Value ? 0 : Convert.ToInt32(idr["Piece"]),
                            PurchaserID = idr["PurchaserID"] == DBNull.Value ? 0 : Convert.ToInt32(idr["PurchaserID"]),
                            PurGoodsCount = idr["PurGoodsCount"] == DBNull.Value ? 0 : Convert.ToInt32(idr["PurGoodsCount"]),
                            TransportFee = idr["TransportFee"] == DBNull.Value ? 0.0M : Convert.ToDecimal(idr["TransportFee"]),
                            OtherFee = idr["OtherFee"] == DBNull.Value ? 0.0M : Convert.ToDecimal(idr["OtherFee"]),
                            TotalCharge = idr["TotalCharge"] == DBNull.Value ? 0.0M : Convert.ToDecimal(idr["TotalCharge"]),
                            ...
                            BHouseID = Convert.ToString(idr["BHouseID"]),
                            BHouseName = Convert.ToString(idr["BHouseName"]),
            ...
            {
                strSQL += $@" and exists(select 1 from Tbl_Cargo_RealFactoryPurchaseHouse as ph where ph.HouseID='{entity.HouseID}' and ph.PurOrderID=a.PurOrderID)";
            }
        ...
        public List<CargoRealFactoryPurchaseHouseEntity> cargoRealFactoryPurchaseHouses(CargoRealFactoryPurchaseHouseEntity entity)
        {
            ...
            string strSQL = "select a.*, b.Name as HouseName, c.LogisticName,min(ActualArrivalTime) as ActualArrivalTime from Tbl_Cargo_RealFactoryPurchaseHouse as a inner join Tbl_Cargo_House as b on a.HouseID = b.HouseID left join Tbl_Cargo_Logistic as c on a.LogisID = c.ID left join Tbl_Cargo_RealFactoryPurchaseOrder as bb on a.PurOrderID = bb.PurOrderID left join Tbl_Cargo_FactoryOrder as cc on bb.PurOrderNo = cc.FacOrderNo and ActualArrivalTime is not null Where (1 = 1) ";
            if (!entity.PurOrderID.Equals(0)) { strSQL += " and a.PurOrderID=" + entity.PurOrderID; }
            strSQL += $@"  group by a.DeliveryOrderNo ,a.ETATime ,a.FacOrderNo ,a.FPID ,a.HouseID ,Name ,a.LogisAwbNo ,a.LogisID ,LogisticName ,a.OP_DATE ,a.OrderNum ,a.PurOrderID ,a.ReplyNum";
            using (DbCommand command = conn.GetSqlStringCommond(strSQL))
            {
                using (DataTable dt = conn.ExecuteDataTable(command))
                {
                    foreach (DataRow idr in dt.Rows)
                    {
                        var ETATime = string.IsNullOrEmpty(Convert.ToString(idr["ETATime"])) ? "" : Convert.ToDateTime(idr["ETATime"]).ToString("yyyy-MM-dd");
                        var ActualArrivalTime = string.IsNullOrEmpty(Convert.ToString(idr["ActualArrivalTime"])) ? "" : Convert.ToDateTime(idr["ActualArrivalTime"]).ToString("yyyy-MM-dd");
                        double ArrivalError = 0;
                        if (!string.IsNullOrEmpty(ETATime) && !string.IsNullOrEmpty(ActualArrivalTime))
                        {

                            var yj = Convert.ToDateTime(ETATime);
                            var dh = Convert.ToDateTime(ActualArrivalTime);
                            TimeSpan daysSpan = new TimeSpan(dh.Ticks - yj.Ticks);
                            ArrivalError = daysSpan.TotalDays;
                        }
                        result.Add(new CargoRealFactoryPurchaseHouseEntity()
                        {
                            ...
                            ETATime = string.IsNullOrEmpty(Convert.ToString(idr["ETATime"])) ? Convert.ToDateTime("0001-01-01") : Convert.ToDateTime(idr["ETATime"]),
                            ActualArrivalTime = string.IsNullOrEmpty(Convert.ToString(idr["ActualArrivalTime"])) ? Convert.ToDateTime("0001-01-01") : Convert.ToDateTime(idr["ActualArrivalTime"]),
        ...
        public RealFactoryPODetailListDto QueryRealFactoryPODetail(RealFactoryPODetailParams queryParams)
        {
...
FROM (
    SELECT
	    DISTINCT ROW_NUMBER() OVER (ORDER BY c.TypeName ASC, b.Specs ASC, b.Figure, pog.PurOrderID DESC, pog.ProductCode) AS RowNumber,
	    po.PurOrderNo,
	    po.PurDepart,
	    po.OwnerShip,
	    po.WhetherTax,
	    ...
	    b.LoadIndex
    FROM
	    Tbl_Cargo_RealFactoryPurchaseOrderGoods AS pog
	    INNER JOIN Tbl_Cargo_RealFactoryPurchaseOrder AS po ON po.PurOrderID = pog.PurOrderID
	    INNER JOIN Tbl_Cargo_Purchaser as p on po.PurchaserID = p.PurchaserID
	    INNER JOIN Tbl_Cargo_ProductSpec AS b ON pog.ProductCode = b.ProductCode
...
");
                ...
                if (!string.IsNullOrWhiteSpace(queryParams.TypeName))
                {
                    conditions.Add("TypeName LIKE '%' + @TypeName + '%'");
                    parameters.Add(new SqlParameter("@TypeName", SqlDbType.NVarChar, 50) { Value = queryParams.TypeName });
                }
                if (!string.IsNullOrWhiteSpace(queryParams.Specs))
                {
                    conditions.Add("Specs LIKE '%' + @Specs + '%'");
                    parameters.Add(new SqlParameter("@Specs", SqlDbType.NVarChar, 50) { Value = queryParams.Specs });
                }
                if (!string.IsNullOrWhiteSpace(queryParams.Figure))
                {
                    conditions.Add("Figure LIKE '%' + @Figure + '%'");
                    parameters.Add(new SqlParameter("@Figure", SqlDbType.NVarChar, 50) { Value = queryParams.Figure });
                }
                if (!string.IsNullOrWhiteSpace(queryParams.GoodsCode))
                {
                    conditions.Add("GoodsCode LIKE '%' + @GoodsCode + '%'");
                    parameters.Add(new SqlParameter("@GoodsCode", SqlDbType.NVarChar, 50) { Value = queryParams.GoodsCode });
                }
                if (!string.IsNullOrWhiteSpace(queryParams.PurOrderNo))
                {
                    conditions.Add("PurOrderNo LIKE '%' + @PurOrderNo + '%'");
                    parameters.Add(new SqlParameter("@PurOrderNo", SqlDbType.NVarChar, 50) { Value = queryParams.PurOrderNo });
                }
                if (!string.IsNullOrWhiteSpace(queryParams.PurchaserName))
                {
                    conditions.Add("PurchaserName LIKE '%' + @PurchaserName + '%'");
                    parameters.Add(new SqlParameter("@PurchaserName", SqlDbType.NVarChar, 50) { Value = queryParams.PurchaserName });
                }
                if (!string.IsNullOrWhiteSpace(queryParams.ProductCode))
                {
                    conditions.Add("pog.ProductCode LIKE '%' + @ProductCode + '%'");
                    parameters.Add(new SqlParameter("@ProductCode", SqlDbType.NVarChar, 50) { Value = queryParams.ProductCode });
                }
                if (queryParams.PurchaseType.HasValue)
                {
                    conditions.Add("PurchaseType = @PurchaseType");
                    parameters.Add(new SqlParameter("@PurchaseType", SqlDbType.Int) { Value = queryParams.PurchaseType.Value });
                }
                if (queryParams.StartDate.HasValue)
                {
                    conditions.Add("CreateDate >= @StartDate");
                    parameters.Add(new SqlParameter("@StartDate", SqlDbType.DateTime) { Value = queryParams.StartDate.Value.Date });
                }
                if (queryParams.EndDate.HasValue)
                {
                    conditions.Add("CreateDate < @EndDate");
                ...
                using (DbCommand command = conn.GetSqlStringCommond(strSQL))
                {
                    ...
                    using (DataTable dt = conn.ExecuteDataTable(command))
                    {
                        List<RealFactoryPODetailDto> sqlData = new List<RealFactoryPODetailDto>();
                        bool isFirst = true;
                        foreach (DataRow dr in dt.Rows)
                        {
                            if (isFirst)
                            {
                                result.DataTotal = dr.Field<int>("TotalCount");
                                isFirst = false;
                            }
                            sqlData.Add(new RealFactoryPODetailDto
                            {
                                PurOrderNo = dr.Field<string>("PurOrderNo"),
                                PurDepart = dr.Field<string>("PurDepart"),
                                OwnerShip = dr.Field<byte?>("OwnerShip"),
                                WhetherTax = dr.Field<byte?>("WhetherTax"),
                                PurchaserName = dr.Field<string>("PurchaserName"),
                                PurchaseType = dr.Field<byte?>("PurchaseType"),
                                PurchaseInStoreType = dr.Field<byte?>("PurchaseInStoreType"),
                                CreateAwb = dr.Field<string>("CreateAwb"),
                                PaymentMethod = dr.Field<int?>("PaymentMethod"),
                                ...
                                PurchasePrice = dr.Field<decimal?>("PurchasePrice"),
        ...
        public List<CargoRealFactoryPurchaseOrderGoodsEntity> cargoRealFactoryPurchaseOrderGoodsExport(CargoRealFactoryPurchaseOrderGoodsEntity entity)
        {
            ...
            string strSQL = $@"select a.*,
                                ...
                                from Tbl_Cargo_RealFactoryPurchaseOrderGoods as a
                                         ...
                                         left join Tbl_Cargo_FactoryOrder as e on e.FacOrderNo = d.PurOrderNo and e.ProductCode = a.ProductCode
                                         ...
                                         --left join (select * from dbo.Tbl_Cargo_Product tcp where tcp.SourceOrderNo is not null and tcp.SourceOrderNo<>'' ) tcp on d.PurOrderNo = tcp.SourceOrderNo and b.GoodsCode = tcp.GoodsCode
                                         left join (select SourceOrderNo,GoodsCode,max(ProductID) as ProductID from Tbl_Cargo_Product
                                                         group by SourceOrderNo,GoodsCode
                                                         ) tcp on d.PurOrderNo = tcp.SourceOrderNo and b.GoodsCode = tcp.GoodsCode and
                                                                        tcp.SourceOrderNo is not null and tcp.SourceOrderNo <> ''
                                         left join (select ProductID,max(InHouseTime) as InHouseTime from Tbl_Cargo_InContainerGoods group by ProductID) tcig on tcp.ProductID = tcig.ProductID
                                where (1 = 1) ";

            if (entity.PurOrderArrID.Count > 0) { strSQL += $@" and a.PurOrderID in({string.Join(",", entity.PurOrderArrID)})"; }
            strSQL += $@" order by PurOrderNo";
            //if (!entity.FPID.Equals(0)) { strSQL += " and a.FPID=" + entity.FPID; }
            using (DbCommand command = conn.GetSqlStringCommond(strSQL))
            {
                using (DataTable dt = conn.ExecuteDataTable(command))
                {
                    foreach (DataRow idr in dt.Rows)
                    {
                        resultGodds.Add(new CargoRealFactoryPurchaseOrderGoodsEntity
                        {
                            ...
                            Model = Convert.ToString(idr["Model"]),
                            GoodsName = Convert.ToString(idr["ProductName"]),
                            InPiece = string.IsNullOrEmpty(Convert.ToString(idr["InPiece"])) ? 0 : Convert.ToInt32(idr["InPiece"]), //入库数量
                            IsDocument = Convert.ToInt32(idr["IsDocument"]), //上传单据
                            InCargoStatus = string.IsNullOrEmpty(Convert.ToString(idr["InCargoStatus"])) ? -1 : Convert.ToInt32(idr["InCargoStatus"]), //入库状态
...
g.*
	...
	WHERE (1=1) ";
        ...
        {
            string strSQL = @"UPDATE Tbl_Cargo_RealFactoryPurchaseHouse Set LogisAwbNo=@LogisAwbNo,ETATime=@ETATime,LogisID=@LogisID,DeliveryOrderNo=@DeliveryOrderNo Where FPID=@FPID";
            using (DbCommand cmd = conn.GetSqlStringCommond(strSQL))
            {
                conn.AddInParameter(cmd, "@FPID", DbType.Int64, entity.FPID);
                ...
                conn.AddInParameter(cmd, "@DeliveryOrderNo", DbType.String, entity.DeliveryOrderNo);
        ...
        /// </summary>
        /// <param name="entity"></param>
        public void UpdateFactoryOrder(CargoRealFactoryPurchaseHouseEntity entity)
        {
            string strSQL = @"UPDATE c SET DeliveryOrderNo=@DeliveryOrderNo,ETATime=@ETATime from Tbl_Cargo_RealFactoryPurchaseHouse as a inner join Tbl_Cargo_RealFactoryPurchaseOrder as b on a.PurOrderID=b.PurOrderID inner join Tbl_Cargo_FactoryOrder as c on b.PurOrderNo=c.FacOrderNo where a.PurOrderID=@PurOrderID";
            using (DbCommand cmd = conn.GetSqlStringCommond(strSQL))
            {
                conn.AddInParameter(cmd, "@PurOrderID", DbType.Int64, entity.PurOrderID);
                conn.AddInParameter(cmd, "@DeliveryOrderNo", DbType.String, entity.DeliveryOrderNo);
