...
// 小程序开发api接口工具包，https://github.com/gooking/wxapi
   var API_BASE_URL = 'https://dlt.neway5.com';
  //var API_BASE_URL = 'http://localhost:8808'; 
  var subDomain = '-';

var request = function request(url, needSubDomain, method, data) {
  // var _url = API_BASE_URL + (needSubDomain ? '/' + subDomain : '') + url;
  var _url = API_BASE_URL + "/Interface/WeiChatRoute.ashx";
  var header = {
...
module.exports = {
  ...
  request: request,
  checkHouseSession: function checkHouseSession(data) {
    return request('/WeiChatRoute.ashx', false, 'post', data);
  ...
  banners: function banners(data) {
    data.cmd="GetBannerList";
    return request('/banner/list', true, 'get', data);
  },
  goodsCategory: function goodsCategory(token) {
    return request('/shop/goods/category/all', true, 'post', {
      token: token, 
      cmd:'GetGoodsCategoryInfo'
    });
  },
  ...
  noticeList: function noticeList(token) {
    return request('/notice/list', true, 'post', {
      token: token, 
      pageSize: 5,
      cmd:'GetNoticeList'
    });
  ...
  defaultAddress: function defaultAddress(token) {
    return request('/user/shipping-address/default/v2', true, 'post', {
      token: token,
      cmd:'GetDefaultAcceptAddressInfo'
    });
  },
  addressDetail: function addressDetail(token, id) {
    return request('/user/shipping-address/detail/v2', true, 'post', {
      id: id,
      token: token,
      cmd:'GetAcceptAddressInfo'
    });
  },
  ...
  SaveOrderStatus: function SaveOrderStatus(token, orderId) {
    return request('/order/close', true, 'post', {
      orderId: orderId,
      token: token,
      cmd:'SaveOrderStatus'
  ...
  province: function province() {
    return request('/common/region/v2/province', false, 'post',{cmd:'GetAllProvincesInfo'});
  },
  city: () => {
  ...
  uploadFile: function uploadFile(token, tempFilePath) {
    ...
    var uploadUrl = API_BASE_URL + "/Interface/WeiChatRoute.ashx?cmd=SaveWxUserAvatarNickName";
    return new Promise(function (resolve, reject) {
      wx.uploadFile({
        url: uploadUrl,
  ...
  modifyUserInfo: function modifyUserInfo(data) {
    data.cmd="ModifyUserInfo";
    return request('/user/modify', true, 'post', data);
  },
  SaveWxUserNickName: function SaveWxUserNickName(data) {
    data.cmd="SaveWxUserNickName";
    return request('/user/modify', true, 'post', data);
  },
  ModifyUserCellphone: function ModifyUserCellphone(data) {
    data.cmd="ModifyUserCellphone";
    return request('/user/modify', true, 'post', data);
  },
  ...
  shippingCarInfo: function shippingCarInfo(token) {
    return request('/shopping-cart/info', true, 'post', {
      token: token,
      cmd:'GetShippingCarInfo'
    });
  },
  shippingCarInfoAddItem: function shippingCarInfoAddItem(token, key, number, sku, addition) {
