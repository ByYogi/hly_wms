...
namespace House.Manager.Cargo
{
    ...
    public class CargoFinanceManager
    {
        ...
        private void AddRecordAwbID(long recordID, string awbID)
        {
        ...
        /// 新增财务收支记录信息
        /// </summary>
        private long AddCardInfo(CargoCashierEntity awb)
        {
            awb.EnSafe();
        ...
        public CargoOrderEntity QueryOrder(CargoOrderEntity entity)
        {
            ...
            string strSQL = "SELECT B.ThrowGood,B.OrderNo,A.HouseID,C.RefundType,ISNULL(B.TransportFee,0) as TransportFee,B.Piece,B.WXOrderNo AS AffectAwbNO,D.WXOrderNo,PayStatus,RefundCheckStatus,A.TransitFee,A.TotalCharge,WXPayOrderNo,A.* ,OrderNum ,B.AwbStatus,RefundCheckStatus,B.BusinessID,C.CargoDepart FROM Tbl_WX_Order  A  JOIN Tbl_Cargo_FinanceRecord D on D.WxOrderNo=A.OrderNo  LEFT JOIN Tbl_Cargo_House C ON a.HouseID=C.HouseID left JOIN Tbl_Cargo_Order B ON A.OrderNo=B.WXOrderNo   where 1=1 ";
            if (entity.HouseID != 0)
            {
        ...
        public List<CargoOrderGoodsEntity> QuryOrderGoods(string OrderNo)
        {
            ...
            string strSQL = $"SELECT E.Piece,E.SupplySalePrice,T.TypeName,T.TypeParentName,F.ProductCode,Batch,F.ProductID,Source,F.ProductName,F.Born,F.Assort,F.TypeID,F.Model,F.Specs,F.LoadIndex,F.SpeedLevel,F.Figure,F.GoodsCode,F.InHousePrice,UnitPrice,TradePrice,SalePrice,CostPrice,TaxCostPrice,NoTaxCostPrice,SpecsType,Supplier,SuppClientNum,SupplierAddress,E.HouseID,E.AreaID,E.ContainerCode,E.ActSalePrice FROM Tbl_Cargo_OrderGoods E   JOIN  Tbl_Cargo_Product F ON F.ProductID=E.ProductID JOIN Tbl_Cargo_ProductType T ON T.TypeID = F.TypeID WHERE 1=1 AND E.OrderNo='{OrderNo}'";
        ...
        {
            Int64 did = 0;
            string strSQL = @"INSERT INTO Tbl_Cargo_Expense(ExName,ExDepart,ExDate,ReceiveName,CardBank,ReceiveNumber,ChargeType,ExCharge,Reason,OP_ID,OperaName,OperaID,ExpenseDate,ApproveID,OP_DATE,UserID,UserName,CheckTime,HouseID,NextCheckID,NextCheckName,ExType,ClientID,ClientName) VALUES (@ExName,@ExDepart,@ExDate,@ReceiveName,@CardBank,@ReceiveNumber,@ChargeType,@ExCharge,@Reason,@OP_ID,@OperaName,@OperaID,@ExpenseDate,@ApproveID,@OP_DATE,@UserID,@UserName,@CheckTime,@HouseID,@NextCheckID,@NextCheckName,@ExType,@ClientID,@ClientName) SELECT @@IDENTITY";
            try
            {
                entity.EnSafe();
                using (DbCommand cmd = conn.GetSqlStringCommond(strSQL))
        ...
        public Hashtable QueryOnlyMyExpense(int pIndex, int pNum, CargoExpenseEntity entity)
        {
            ...
            try
            {
                ...
                #region 组装查询SQL语句
                string strSQL = @" SELECT TOP " + pNum + " *  FROM ";
                strSQL += "(select DISTINCT ROW_NUMBER() OVER (ORDER BY a.ExpenseDate DESC) AS RowNumber,a.ExID,a.ExName,a.ExDepart,a.ExDate,a.ReceiveName,a.ReceiveNumber,a.ChargeType,a.ExCharge,a.OperaID,a.OperaName,a.Reason,a.ExpenseDate,a.Status,a.CheckStatus,a.DenyReason,ISNULL(a.UserID,0) as UserID,a.UserName,a.CheckTime,a.OP_ID,a.NextCheckID,a.NextCheckName,a.ApproveID,b.ApproveName,a.ExType,ISNULL(a.ClientID,0) as ClientID,a.ClientName,a.HouseID from Tbl_Cargo_Expense as a inner join Tbl_Cargo_ExpenseApproveSet as b on a.ApproveID=b.ID where (1=1)";
                //报销单号
                if (!string.IsNullOrEmpty(ar))
                {
                    // strSQL += " and a.ExID = " + entity.ExID + "";
        ...
        public Hashtable QueryMyExpense(int pIndex, int pNum, CargoExpenseEntity entity)
        {
            ...
            try
            {
                ...
                #region 组装查询SQL语句
                string strSQL = @" SELECT TOP " + pNum + " *  FROM ";
                strSQL += "(select DISTINCT ROW_NUMBER() OVER (ORDER BY a.ExpenseDate DESC) AS RowNumber,a.ExID,a.ExName,a.ExDepart,a.ExDate,a.ReceiveName,a.ReceiveNumber,a.ChargeType,a.ExCharge,a.OperaID,a.OperaName,a.Reason,a.ExpenseDate,a.Status,a.CheckStatus,a.DenyReason,ISNULL(a.UserID,0) as UserID,a.UserName,a.CheckTime,a.OP_ID,a.NextCheckID,a.NextCheckName,a.ApproveID,b.ApproveName,a.ExType,ISNULL(a.ClientID,0) as ClientID,a.ClientName,c.CheckType,c.ReadStatus,a.HouseID from Tbl_Cargo_Expense as a inner join Tbl_Cargo_ExpenseApproveSet as b on a.ApproveID=b.ID inner join Tbl_Cargo_ApproveCheck as c on a.ExID=c.ApproveID where (1=1) and c.ApproveType='0'";
                //报销单号
                if (!string.IsNullOrEmpty(ar))
                {
                    // strSQL += " and a.ExID = " + entity.ExID + "";
        ...
        public List<CargoExpenseEntity> QueryExpenseMyCheckForExport(CargoExpenseEntity entity)
        {
            ...
            try
            {
                ...
                #region 组装查询SQL语句
                string strSQL = @"select DISTINCT a.ExDepart,a.ExID,a.ExName,a.ExDate,a.ReceiveName,a.ReceiveNumber,a.ChargeType,a.ExCharge,a.Reason,a.Status,a.DenyReason,a.CheckStatus,a.UserID,a.UserName,a.NextCheckID,a.NextCheckName,a.ExType,a.ClientID,a.ClientName,a.OperaID,a.ExpenseDate,a.OperaName,b.ZID,b.FID,b.SID,b.SName,b.HappenDate,b.Memo,b.Summary,b.DetailCharge,a.CheckTime,f.FName from Tbl_Cargo_Expense as a inner join Tbl_Cargo_ExpenseDetail as b on a.ExID=b.ExID left join Tbl_Cargo_FirstSubject as f on b.FID=f.FID inner join Tbl_Cargo_ApproveCheck as c on a.ExID=c.ApproveID where (1=1) and c.ApproveType='0' ";
                //报销单号
                if (!string.IsNullOrEmpty(ar))
                {
                    // strSQL += " and a.ExID = " + entity.ExID + "";
        ...
        {
            string strSQL = "Insert into Tbl_Cargo_Order(OrderNo, OrderNum, HAwbNo, LogisAwbNo, LogisID, Dep, Dest, Piece, Weight, Volume, InsuranceFee,TransitFee, TransportFee, DeliveryFee, OtherFee, TotalCharge, Rebate, CheckOutType, TrafficType, DeliveryType, AcceptUnit, AcceptPeople, AcceptTelephone, AcceptCellphone, AcceptAddress, CreateAwb, CreateDate, CheckStatus,Signer, SignTime, OP_ID, OP_DATE, AwbStatus, Remark, SaleManID, SaleManName, SaleCellphone, HouseID,CreateAwbID, OrderType, FinanceSecondCheck, FinanceSecondCheckName, FinanceSecondCheckDate, OrderModel,ClientNum, WXOrderNo, ThrowGood, TranHouse, OutHouseName, ModifyPriceStatus, PayClientNum, PayClientName,BelongHouse, IsPrintPrice, AccountNo, PostponeShip, OutCargoTime, DeliverySettlement, OrderAging, PrintNum,PollStatus, CheckDate, PickStatus, LineID, LineName, ShopCode, SuppClientNum,BusinessID)";
            strSQL += " (SELECT '" + entity.OrderNo + "', '" + entity.OrderNum + "', B.HAwbNo, B.LogisAwbNo, B.LogisID, B.Dep, B.Dest, @Piece, B.Weight, B.Volume,B.InsuranceFee, B.TransitFee, @TransportFee, B.DeliveryFee, B.OtherFee, @TotalCharge, B.Rebate, B.CheckOutType,B.TrafficType, B.DeliveryType, B.AcceptUnit, B.AcceptPeople, B.AcceptTelephone, B.AcceptCellphone, B.AcceptAddress,'" + entity.CreateAwb + "', getdate(), B.CheckStatus, B.Signer, B.SignTime, '" + entity.CreateAwbID + "',  getdate(), B.AwbStatus, B.Remark, B.SaleManID, B.SaleManName, B.SaleCellphone, B.HouseID,'" + entity.CreateAwbID + "', B.OrderType, B.FinanceSecondCheck,B.FinanceSecondCheckName, B.FinanceSecondCheckDate, '" + entity.OrderModel + "', B.ClientNum, B.WXOrderNo, '" + entity.ThrowGood + "',B.TranHouse, B.OutHouseName, B.ModifyPriceStatus, B.PayClientNum, B.PayClientName, B.BelongHouse,B.IsPrintPrice, B.AccountNo, B.PostponeShip, B.OutCargoTime, B.DeliverySettlement, B.OrderAging, B.PrintNum,B.PollStatus, B.CheckDate, B.PickStatus, B.LineID, B.LineName, B.ShopCode, B.SuppClientNum,B.BusinessID FROM Tbl_WX_Order AS A INNER JOIN  Tbl_Cargo_Order AS B ON A.OrderNo = B.WXOrderNo INNER JOIN Tbl_Cargo_House AS C ON B.HouseID = C.HouseID WHERE   (B.HouseID =@HouseID) AND (A.WXPayOrderNo =@WXPayOrderNo) )";
            using (DbCommand cmd = conn.GetSqlStringCommond(strSQL))
            {
        ...
        public List<CargoOrderEntity> QueryOrderForBillCash(CargoOrderEntity entity)
        {
            ...
            try
            {
                ...
                string strSQL = @"SELECT DISTINCT  b.ClientID as AClientID,p.Supplier,b.ClientNum as AClientNum,g.RelateOrderNo, a.* FROM Tbl_Cargo_OrderGoods AS g LEFT JOIN Tbl_Cargo_Order AS a ON a.OrderNo = g.OrderNo LEFT JOIN Tbl_Cargo_Product AS p ON g.ProductID = p.ProductID LEFT JOIN Tbl_Cargo_Client AS b ON a.SuppClientNum = b.ClientNum RIGHT JOIN Tbl_WX_Order AS c ON a.WXOrderNo = c.OrderNo WHERE (1 = 1) ";
                //下单方式
                if (!string.IsNullOrEmpty(entity.AccountNo)) { strSQL += " and a.AccountNo = '" + entity.AccountNo + "'"; }
                if (!string.IsNullOrEmpty(entity.OrderType)) { strSQL += " and a.OrderType = '" + entity.OrderType + "'"; }
                //订单号 
