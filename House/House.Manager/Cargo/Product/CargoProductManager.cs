    ...
    public class CargoProductManager
    {
        ...
        public Hashtable QueryAllPageProductStockInfo(int pIndex, int pNum, CargoProductEntity entity, bool isNextDay = false)
        {
            ...
            try
            {
                ...
                resHT["rows"] = result;
                #region 总数目
                string strCount = @"Select Count(*) as TotalCount,sum(b.Piece) as TotalNum,sum(b.Piece*p.InHousePrice) as TotalMoney from Tbl_Cargo_Product as p inner join Tbl_Cargo_ContainerGoods as b on p.ProductID=b.ProductID Where (1=1)";
                if (isNextDay)
                {
        ...
        {
            string strSQL = @"insert into Tbl_Cargo_Product (ProductName,TypeID,Model,GoodsCode,Specs,Figure,TreadWidth,FlatRatio,Meridian,HubDiameter,LoadIndex,SpeedLevel,SpeedMax,Size,Weight,UnitPrice,Numbers,CostPrice,TradePrice,SalePrice,Package,PackageNum,PackageWeight,InHouseTime,InCargoStatus,Batch,BatchYear,BatchWeek,HouseID,Source,SourceOrderNo,BelongMonth) select ProductName,TypeID,Model,GoodsCode,Specs,Figure,TreadWidth,FlatRatio,Meridian,HubDiameter,LoadIndex,SpeedLevel,SpeedMax,Size,Weight,@UnitPrice,@Numbers,@CostPrice,@TradePrice,@SalePrice,Package,PackageNum,PackageWeight,InHouseTime,@InCargoStatus,@Batch,@BatchYear,@BatchWeek,@HouseID,@Source,@SourceOrderNo,@BelongMonth from Tbl_Cargo_Product where ProductID=@ProductID";
            try
            {
                using (DbCommand cmd = conn.GetSqlStringCommond(strSQL))
                {
        ...
        public void AddCargoProductShelves(CargoProductShelvesEntity entity)
        {
            entity.EnSafe();
            Int64 did = 0;
            string strSQL = @"INSERT INTO Tbl_Cargo_Shelves(ProductName,TypeID,ProductID,OnSaleNum,Title,Memo,OP_DATE,FileName,SaleType,Consume,ProductPrice,ProductCode";
            if (entity.SaleType.Equals("3"))// || entity.SaleType.Equals("1")
            {
                strSQL += ",AdvertStartDate,AdvertEndDate";
            }
            strSQL += ") VALUES (@ProductName,@TypeID,@ProductID,@OnSaleNum,@Title,@Memo,@OP_DATE,@FileName,@SaleType,@Consume,@ProductPrice,@ProductCode";
            if (entity.SaleType.Equals("3"))// || entity.SaleType.Equals("1")
            {
                strSQL += ",@AdvertStartDate,@AdvertEndDate";
            }
        ...
        {
            entity.EnSafe();
            string strSQL = @"Update Tbl_Cargo_Shelves Set OnSaleNum=@OnSaleNum,Title=@Title,Memo=@Memo,OP_DATE=@OP_DATE,SaleType=@SaleType,Consume=@Consume,ShelveStatus=@ShelveStatus,ProductPrice=@ProductPrice";
        ...
        public void Unshelves(List<CargoProductShelvesEntity> entity)
        {
            ...
            foreach (CargoProductShelvesEntity it in entity)
            {
                ...
                strSQL.AppendLine($@";Update Tbl_Cargo_Shelves set ShelveStatus='1',OnSaleNum=0,SaleType='0',OP_DATE='{DateTime.Now}' Where ID={it.ID}");
                    ...
                    using (DataTable dt = conn.ExecuteDataTable(command))
                    {
                        foreach (DataRow idr in dt.Rows)
                        {
                            result.Add(new CargoProductShelvesEntity
                            {
                                ...
                                AdvertStartDate = Convert.ToDateTime(idr["AdvertStartDate"]),
                                AdvertEndDate = Convert.ToDateTime(idr["AdvertEndDate"]),
                                Consume = Convert.ToInt32(idr["Consume"]),
                                ProductCode = Convert.ToString(idr["ProductCode"]),
        ...
        {
            //处理商城促销产品在售数量
            string strU = "update Tbl_Cargo_Shelves set OnSaleNum=c.Piece from (select a.ProductID,a.Piece from Tbl_Cargo_ContainerGoods as a inner join Tbl_Cargo_Shelves as b on a.ProductID=b.ProductID where b.SaleType=1 and b.ShelveStatus=0) as c where Tbl_Cargo_Shelves.ProductID=c.ProductID";
        ...
        /// </summary>
        /// <returns></returns>
        public List<CargoProductEntity> QueryProductByHousIDs(IEnumerable<int> housIDList)
        {
            List<CargoProductEntity> result = new List<CargoProductEntity>();
            string strSQL = "select ProductID, SpecsType,ProductCode, BatchYear, TypeID, HouseID, SuppClientNum from Tbl_Cargo_Product where (1=1)";
            //if (!entity.HouseID.Equals(0)) { strSQL += " and HouseID=@HouseID"; }
            if (housIDList.Any())
            {
                strSQL += " and HouseID in (";
                foreach (var houseid in housIDList)
                {
                    strSQL += $"{houseid},";
                }
