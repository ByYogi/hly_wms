    ...
    public class CargoWeiXinManager
    {
        ...
        public List<WXOrderEntity> QueryWeixinOrderInfo(int pIndex, int pNum, WXOrderEntity entity)
        {
            ...
            try
            {
                ...
                strSQL += "(SELECT DISTINCT ROW_NUMBER() OVER (ORDER BY a.CreateDate DESC) AS RowNumber,a.*,b.OrderNo as CargoOrderNo,c.wxOpenID,ISNULL(b.OrderID,0) as OrderID,c.ClientNum From Tbl_WX_Order as a left join Tbl_Cargo_Order as b on a.OrderNo=b.WXOrderNo inner join Tbl_WX_Client as c on a.WXID=c.ID Where (1=1) ";
                //微信用户ID
        ...
        public void DeleteWeixinOrder(WXOrderEntity entity)
        {
            ...
            string strDel = "Delete From Tbl_WX_OrderProduct where OrderID=@OrderID";
...
select WXID, SUM(Cash) as FX From dbo.Tbl_Taobao_CashBack where WXID=" + entity.WXID + " and CashType=0 and payment!=0";
        ...
        public List<MiniProOrderEntity> QueryMyMiniProOrder(int pIndex, int pNum, WXOrderEntity entity)
        {
            ...
            string strSQL = "SELECT TOP " + pNum + " * from (SELECT ROW_NUMBER() OVER (ORDER BY b.CreateDate desc) AS RowNumber,c.Address as SupplierAddress, c.ClientShortName as SupperName,b.SuppClientNum,b.Name,b.Cellphone,b.ID,b.OrderNo,b.Piece,b.TotalCharge,b.CreateDate,b.PayStatus,b.OrderStatus,b.WXPayOrderNo,a.CompanyName,b.ThrowGood,b.RefundCheckStatus,b.SendType From Tbl_WX_Client as a inner join Tbl_WX_Order as b on a.ID=b.WXID inner join Tbl_Cargo_Client as c on b.SuppClientNum=c.ClientNum  where (1=1) ";
            ...
            if (!string.IsNullOrEmpty(entity.PayStatus))
            {
                ...
                {
                    strSQL += " and b.PayStatus in ('2','3')";
                }
                else
                {
                    strSQL += " and b.PayStatus='" + entity.PayStatus + "'";
                }
            }
        ...
        public List<WXOrderEntity> QueryRefundOrderData(string orderNo)
        {
            ...
            string strSQL = $@"select a.*,b.Name as HouseName,c.CompanyName,d.IsTransitFee,d.TransitFee from Tbl_WX_Order as a inner join tbl_Cargo_house as b on a.HouseID = b.HouseID inner join Tbl_WX_Client as c on a.WXID = c.ID inner join Tbl_Cargo_Order as d on a.OrderNo = d.WXOrderNo and d.OrderModel = 1 where a.OrderNo='{orderNo}'";
            using (DbCommand cmdAdd = conn.GetSqlStringCommond(strSQL))
            {
                using (DataTable dt = conn.ExecuteDataTable(cmdAdd))
                {
