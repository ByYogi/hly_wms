...
GROUP BY b.TypeID, b.HouseID, b.ProductCode, ca.RootArea;

CREATE UNIQUE INDEX IX_#tempCurStock
ON #tempCurStock(ProductCode,TypeID,HouseID,AreaID)
INCLUDE(Piece);
");
...
FROM (
    ...
    GROUP BY a.TypeID, a.HouseID, a.ProductCode, a.AreaID
    ...
    GROUP BY ts.TypeID, ts.HouseID, ts.ProductCode, ts.AreaID
) t
GROUP BY t.TypeID, t.HouseID, t.ProductCode, t.AreaID;

CREATE UNIQUE INDEX IX_#tempOrderStats
ON #tempOrderStats(ProductCode,TypeID,HouseID,AreaID)
...
FROM (
    ...
    FROM Tbl_Cargo_DailySaleStatic a
    WHERE (1 = 1) @{conditions}
    GROUP BY a.TypeID, a.ProductCode
    ...
    FROM #tempTodaySale ts
    GROUP BY ts.TypeID, ts.ProductCode
) t
GROUP BY t.TypeID, t.ProductCode;

CREATE UNIQUE INDEX IX_#tempTotalSalePiece
ON #tempTotalSalePiece(TypeID,ProductCode)
INCLUDE(Piece);
");
            conditions = new List<string>();
            // 制单日期范围
            ...
            strBld.Append(@"
PRINT('------------ 在途库存 ------------');
SELECT ProductCode, TypeID, HouseID, SUM(Piece) Piece INTO #tempITI FROM (
-- 移库单
SELECT
    p.ProductCode,
    p.TypeID,
...
GROUP BY p.ProductCode, p.TypeID, mo2.NewHouseID
UNION ALL
...
GROUP BY 
	rog.ProductCode, rog.TypeID, ro.FromHouse
) a 
WHERE (1=1) @{conditions}
GROUP BY ProductCode, TypeID, HouseID;
");
            conditions = new List<string>();
            ...
            if (entity.TypeID != 0)
            {
                conditions.Add($"TypeID = {entity.TypeID}");
            }
            //if (entity.ParentID != 0)
            //{
...
GROUP BY ProductCode, TypeID, HouseID, AreaID;
CREATE UNIQUE INDEX IX_#tempAvgSale
ON #tempAvgSale(ProductCode,TypeID,HouseID,AreaID)
...
INNER JOIN (SELECT MAX(SID) SID FROM Tbl_Cargo_SafeStock GROUP BY ProductCode, AreaID) s2  --去重复
    ON s.SID = s2.SID
            ...
            using (DbCommand command = conn.GetSqlStringCommond(strSQL))
            {
                ...
                using (DataTable dd = conn.ExecuteDataTable(command))
                {
                    foreach (DataRow idr in dd.Rows)
                    {
                        ...
                        int doi = 0; //库存度天数
                        double dayAvgSaleDouble = avgSaleNum == 0 ? 0 : Math.Ceiling(avgSaleNum / 30d);
                        int dayAvgSaleNum = Convert.ToInt32(Math.Ceiling(dayAvgSaleDouble)); //日均销量

                        if (curNum > 0 && dayAvgSaleNum > 0)
                        {
                            doi = Convert.ToInt32(Math.Floor((double)(curNum / dayAvgSaleNum)));
...
GROUP BY c.TypeID, c.HouseID, c.ProductCode;
CREATE UNIQUE INDEX IX_#tempOrderStats
ON #tempOrderStats(ProductCode,TypeID,HouseID)
");
            conditions = new List<string>();
...
FROM 
  ...
  LEFT JOIN Tbl_Cargo_ProductType AS j ON f.TypeID = j.TypeID
    LEFT JOIN #tempAvgSale AS avs
       ...
       AND f.TypeID = avs.TypeID
       AND f.ProductCode = avs.ProductCode
       AND f.AreaID = avs.AreaID
WHERE
  (1 = 1)
