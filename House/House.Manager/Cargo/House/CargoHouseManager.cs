...
namespace House.Manager.Cargo
{
    ...
    public class CargoHouseManager
    {
        ...
        {
            List<CargoHouseEntity> result = new List<CargoHouseEntity>();
            string strSQL = @"SELECT HouseID,Name,Address,DepCity,CargoDepart FROM Tbl_Cargo_House WHERE DelFlag='0' ";
            try
            {
                using (DbCommand cmd = conn.GetSqlStringCommond(strSQL))
                {
        ...
        {
            List<CargoAreaEntity> result = new List<CargoAreaEntity>();
            string strSQL = @"SELECT AreaID,Name,Code,Longitude,Latitude,Address FROM Tbl_Cargo_Area WHERE (DelFlag=0 or DelFlag is null) and HouseID=@HouseID ";
...
left join Tbl_Cargo_Area as b on a.AreaID=b.AreaID where a.ContainerType=@ContainerType and b.HouseID=@HouseID";
        ...
        public DataTable ConvertToContainerGoodsTable(List<CargoContainerGoodsEntity> list)
        {
            var dt = new DataTable();
            dt.Columns.Add("ContainerID", typeof(int));
            dt.Columns.Add("TypeID", typeof(int));
            dt.Columns.Add("ProductID", typeof(long));
            dt.Columns.Add("Piece", typeof(int));
            dt.Columns.Add("Weight", typeof(decimal));
            dt.Columns.Add("InHouseTime", typeof(DateTime));
            dt.Columns.Add("IsPrintInCargo", typeof(string));
            dt.Columns.Add("InCargoID", typeof(string));
            ...
            foreach (var item in list)
            {
                ...
                row["InHouseTime"] = DateTime.Now;
                row["IsPrintInCargo"] = item.IsPrintInCargo ? "1" : "0";
                row["InCargoID"] = item.InCargoID;
                dt.Rows.Add(row);
            }
        ...
        {
            DataTable table = ConvertToContainerGoodsTable(list);
            using (var conn = new SqlConnection(SqlHelper.dbConnectionString))
            {
                conn.Open();
                using (var bulkCopy = new SqlBulkCopy(conn))
                {
                    bulkCopy.DestinationTableName = "Tbl_Cargo_ContainerGoods";

                    // 明确指定映射关系
                    bulkCopy.ColumnMappings.Add("ContainerID", "ContainerID");
                    bulkCopy.ColumnMappings.Add("TypeID", "TypeID");
                    bulkCopy.ColumnMappings.Add("ProductID", "ProductID");
                    bulkCopy.ColumnMappings.Add("Piece", "Piece");
                    bulkCopy.ColumnMappings.Add("Weight", "Weight");
                    bulkCopy.ColumnMappings.Add("InHouseTime", "InHouseTime");
                    bulkCopy.ColumnMappings.Add("IsPrintInCargo", "IsPrintInCargo");
                    bulkCopy.ColumnMappings.Add("InCargoID", "InCargoID");
        ...
        public List<CargoSafeStockEntity> QuerySafeStockData(CargoSafeStockEntity entity)
        {
            ...
            string strSQL = @"select a.*,ISNULL(b.Piece,0) as CurNum,ISNULL(c.SaleNum,0) as SaleNum,case when a.StockNum-ISNULL(b.Piece,0)<=0 then 0 else a.StockNum-ISNULL(b.Piece,0) end as LessNum from ( SELECT s.SID,s.HouseID,h.Name as HouseName,s.AreaID,a.Name as AreaName,pt.ParentID,pt.TypeParentName as ParentName, s.TypeID,pt.TypeName,s.Specs,s.Figure,s.GoodsCode,s.LoadIndex,s.SpeedLevel,s.StockNum,s.MaxStock,s.MinStock,s.OPID,s.OP_DATE,s.ProductCode from Tbl_Cargo_SafeStock s inner join Tbl_Cargo_House h on s.HouseID=h.HouseID inner join Tbl_Cargo_Area a on s.AreaID=a.AreaID inner join Tbl_Cargo_ProductType pt on s.TypeID=pt.TypeID WHERE (1=1)";

            if (!string.IsNullOrEmpty(entity.HouseID)) { strSQL += " and s.HouseID  in (" + entity.HouseID + ")"; }
            if (entity.AreaID != 0) { strSQL += " and a.AreaID = " + entity.AreaID; }
            ...
            strSQL += ") as a left join (select b.TypeID,b.HouseID,b.ProductCode, sum(a.Piece) as Piece From Tbl_Cargo_ContainerGoods as a inner join Tbl_Cargo_Product as b on a.ProductID=b.ProductID and a.Piece>0 and b.SpecsType!=5 inner join  Tbl_Cargo_ProductType as c on b.TypeID=c.TypeID inner join Tbl_Cargo_Container as d on a.ContainerID=d.ContainerID inner join Tbl_Cargo_Area as e on  d.AreaID=e.AreaID inner join Tbl_Cargo_Area as f on e.ParentID=f.AreaID inner join Tbl_Cargo_Area as g on f.ParentID=g.AreaID and g.IsShowStock=0 and g.HouseID=b.HouseID Where (1=1)";
            ...
            strSQL += " left join (select SUM(b.Piece) as SaleNum,c.TypeID,c.HouseID,c.ProductCode from Tbl_Cargo_Order as a inner join Tbl_Cargo_OrderGoods as b on a.OrderNo=b.OrderNo and a.OrderModel=0 and a.ThrowGood!=25 ";
            ...
            strSQL += " inner join Tbl_Cargo_Product as c on b.ProductID=c.ProductID and a.HouseID=c.HouseID inner join Tbl_Cargo_ProductType as d on c.TypeID=d.TypeID and c.SpecsType!=5 ";
            ...
            strSQL += " order by a.StockNum-ISNULL(b.Piece,0) desc";
            using (DbCommand command = conn.GetSqlStringCommond(strSQL))
            {
                #region 获取数据
                using (DataTable dd = conn.ExecuteDataTable(command))
        ...
        public List<CargoSafeStockEntity> QueryWarnStockData(CargoSafeStockEntity entity)
        {
            ...
            if (!string.IsNullOrEmpty(entity.HouseID)) { strSQL += " and a.HouseID  in (" + entity.HouseID + ")"; }
            if (!entity.TypeID.Equals(0)) { strSQL += " and a.TypeID=" + entity.TypeID; }
            if (!entity.AreaID.Equals(0) && !entity.HouseID.Equals("9")) { strSQL += " and e.ParentID=" + entity.AreaID; }
            strSQL += " group by a.GoodsCode,f.HouseID,f.StockNum,f.AreaID,f.Specs,f.Figure,f.ProductCode,f.LoadIndex,f.SpeedLevel,a.TypeID,g.Name,h.Name,j.TypeName order by SUM(b.Piece)-f.StockNum asc";
            using (DbCommand command = conn.GetSqlStringCommond(strSQL))
            {
                #region 获取数据
                using (DataTable dd = conn.ExecuteDataTable(command))
        ...
        /// </summary>
        /// <param name="entity"></param>
        public int SaveStockTake(CargoStockTakeEntity entity)
        {
            int StockID = AddStockTake(entity);
            string strSQL = "insert into Tbl_Cargo_StockTakeContainer(ContainerID,ContainerCode,StockNum,StockID) select a.ContainerID,b.ContainerCode,SUM(a.Piece) as StockNum," + StockID + " from Tbl_Cargo_ContainerGoods as a inner join Tbl_Cargo_Container as b on a.ContainerID = b.ContainerID and a.Piece>0 inner join Tbl_Cargo_Area as c on b.AreaID = c.AreaID ";
            if (!entity.TypeID.Equals(0)) { strSQL += " and a.TypeID = " + entity.TypeID; }
            if (!entity.HouseID.Equals(0)) { strSQL += " and c.HouseID = " + entity.HouseID; }
            ...
            string strTag = "insert into Tbl_Cargo_StockTakeTag(StockID,ProductID,TagCode,ContainerID) select " + StockID + ",a.ProductID,g.TagCode,a.ContainerID from Tbl_Cargo_ContainerGoods as a inner join Tbl_Cargo_Container as b on a.ContainerID = b.ContainerID and a.Piece > 0 inner join Tbl_Cargo_Area as c on b.AreaID = c.AreaID ";
                ...
                using (DataTable dd = conn.ExecuteDataTable(command))
                {
                    foreach (DataRow idr in dd.Rows)
                    {
                        result.Add(new CargoStockTakeTagEntity
                        {
                            ...
                            StockStatus = Convert.ToString(idr["StockStatus"]),
                            ScanDate = string.IsNullOrEmpty(Convert.ToString(idr["ScanDate"])) ? Convert.ToDateTime("0001-01-01") : Convert.ToDateTime(idr["ScanDate"]),
                            ProductName = Convert.ToString(idr["ProductName"]),
                            Batch = Convert.ToString(idr["Batch"]),
                            Remark = Convert.ToString(idr["Remark"]),
                            TypeName = Convert.ToString(idr["TypeName"]),
