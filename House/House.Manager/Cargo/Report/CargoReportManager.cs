...
{(!string.IsNullOrEmpty(entity.ThrowGood) && entity.ThrowGood != "0" ? $@" and a.ThrowGood='{entity.ThrowGood}'" : "")}
...
";
        ...
        public List<CargoHouseAimEntity> QueryHCYC_House(CargoHouseAimEntity entity)
        {
            ...
            {(entity.OrderType == 0 ? "" : $@" and ThrowGood in({entity.OrderType})")}
            --and OrderModel=0
            group by HouseID ,Year,Month
                          ) as a2
             left join (
                select HouseID,sum(case when OrderModel=0 then Piece else -Piece end) as PrevPiece,Year from Tbl_Cargo_OrderDayStatistics as a
...
{(entity.OrderType != 0 ? $@" and ThrowGood in(@OrderType)" : "")} 
...
left join(
      ...
      where a.TypeID = @TypeID and a.HouseID=@HouseID {(entity.OrderType != 0 ? $@" and ThrowGood in(@OrderType)" : "")} {(entity.SuppClientNum != 0 ? $@" and a.SuppID=@SuppID" : "")}
        and DayStatisticsTime >= @StartDate and DayStatisticsTime<=@EndDate
...
left join(
        select a.TypeID,  a.ProductCode, d.Specs,sum(case when OrderModel=0 then Piece else -Piece end) as Piece from Tbl_Cargo_OrderDayStatistics as a
         inner join Tbl_Cargo_ProductSpec as d on a.ProductCode = d.ProductCode
      where a.TypeID = @TypeID {(entity.OrderType != 0 ? $@" and ThrowGood in(@OrderType)" : "")} {(entity.SuppClientNum != 0 ? $@" and a.SuppID=@SuppID" : "")}
                         and DayStatisticsTime >= @StartDate and DayStatisticsTime<=@EndDate
...
{(entity.SuppClientNum != 0 ? $@" and SuppClientNum='{entity.SuppClientNum}' " : "")}
    ...
    WHERE c.CreateDate >= '{entity.StartDate.ToString("yyyy-MM-dd")}'
           ...
           {(entity.OrderType != 0 ? $@" and c.ThrowGood in({entity.OrderType})" : "")}
...
group by a.HouseID
   ...
   , InventoryData as (select e.HouseID
          ...
          ,UserTransaction as (
             ...
             , COUNT(DISTINCT wxClients.ClientNum) AS StoresUniqueClientCount 
                ...
                where  a.CreateDate >= @StartDate and a.CreateDate <= @EndDate  and a.OrderType = '4'  {(entity.OrderType != 0 ? $@" and ThrowGood in({entity.OrderType})" : "")}
  ...
  , ReusltData as (select orders.HouseID, count(distinct case when OrderModel = 0 then orders.OrderNo end) as OrderCount , count(distinct case when OrderModel = 0 then ProductCode end) as DayStatisticsCount
                    ...
                    where orders.CreateDate >= @StartDate and orders.CreateDate <= @EndDate  and orders.OrderType = '4'  {(entity.OrderType != 0 ? $@" and ThrowGood in({entity.OrderType})" : "")}
