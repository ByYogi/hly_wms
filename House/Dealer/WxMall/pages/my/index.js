...
Page({
    ...
    userCellphone:0, //0:没有拿到手机号 1：拿到手机号
    hasUserInfo:false
  },
	onLoad() {
  ...
  async getUserApiInfo() {
    ...
    if (res.code == 0) {
      ...
      if (res.data._base.mobile) {
        _data.userMobile = res.data._base.mobile
        _data.userCellphone=1
      }
      if (res.data._base.nick && res.data._base.avatarUrl) {
        _data.userInfoStatus = 2
      } else {
        _data.userInfoStatus = 1
      }
      if(res.data._base.nick=="微信用户"){
        _data.hasUserInfo=true
      }else{
        _data.hasUserInfo=false
      }
     
      if (this.data.order_hx_uids && this.data.order_hx_uids.indexOf(res.data._base.id) != -1) {
        _data.canHX = true // 具有扫码核销的权限
      }
      const adminUserIds = wx.getStorageSync('adminUserIds')
      if (adminUserIds && adminUserIds.indexOf(res.data._base.id) != -1) {
        _data.isAdmin = true
      }
  ...
  async wxgetuserphone(e){
    ...
    //console.log(e)
  // 更新用户资料
  await WXAPI.ModifyUserCellphone({
    token: wx.getStorageSync('token'),
    code:e.detail.code,
    avatarUrl: "",
    nick:"",
