<%@ Page Title="盘点管理" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true"
    ...
    <asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
        <script type="text/javascript">
            ...
            function openUpldSTDocModal() {
                ...
                if (row) {
                    ...
                    $('#dlgUploadPic').dialog('open').dialog('setTitle', '上传盘点单（' + row.StockID + ')');
                    ...
                    gridOpts.url = 'orderApi.aspx?method=QueryCargoFactoryFile&FacOrderNo=' + row.StockID + '&HouseID=' + row.HouseID;
                }
            }
            ...
            function printHandler() {
                ...
                $.messager.progress({ msg: '正在获取数据...' });
                $.ajax({
                    url: 'houseApi.aspx?method=QueryStockTakeDataByStockIDs',
                    type: 'post',
                    dataType: 'text',
                    data: { StockIDs: JSON.stringify(rows.map(x => x.StockID)) },
                    success: function (resps) {
                        $.messager.progress("close");
                        genHTMLtoPrint(JSON.parse(resps));

                    }
                })

                function genHTMLtoPrint(data) {
                    var columns = []; // 第一组列定义
                    var data = data;
                    var title = "盘点单"
                    columns = [
                        {
                            title: '序号', field: 'SN', formatter: function (_, __, index) {
                                return index + 1
                            }
                        },
                        {
                            title: '盘点单号', field: 'StockID'
                        },
                        {
                            title: '品牌', field: 'TypeName'
                        },
                        {
                            title: '规格', field: 'Specs'
                        },
                        {
                            title: '花纹', field: 'Figure'
                        },
                        {
                            title: '载速', field: 'LoadIndex', formatter: function (val, row, index) {
                                return row.LoadIndex + row.SpeedLevel
                            }
                        },
                        {
                            title: '货品代码', field: 'GoodsCode'
                        },
                        {
                            title: '货位代码', field: 'ContainerCode'
                        },
                        {
                            title: '标签号码', field: 'TagCode'
                        },
                        {
                            title: '扫描状态', field: 'ScanStatus',
                            formatter: function (val, row, index) {
                                switch (val) {
                                    case "0":
                                        return "未扫描";
                        ...
                        {
                            title: '盘点结果', field: 'Remark'
                        },
                        {
                            title: '扫描人', field: 'ScanUserName'
                        },
                        { title: '扫描时间', field: 'ScanDate', formatter: DateTimeFormatter },
        ...
        <head>
            <meta charset="utf-8">
            <title>${title || '盘点单'}</title>
            <style>
                body { font-family: Arial, sans-serif; }
                h4 { text-align: center; }
                table { border-collapse: collapse; width: 100%; }
                th, td { border: 1px solid #333; padding: 6px; text-align: center; font-size: 10px !important; }
    ...
    `;
                    ...
                    // // var win = window.open('', '_blank', 'width=1000,height=800,scrollbars=yes,resizable=yes');
    ...
    <asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
        ...
        <div id="productTag" class="easyui-dialog" style="width: 800px; height: 540px; padding: 0px" closed="true"
        ...
        <div id="dlgUploadPic" class="easyui-dialog" style="width: 500px; height: 400px; padding: 1px 1px" closed="true"
            buttons="#dlgUploadPic-buttons">
            <form id="fmUploadPic" class="easyui-form" method="post" enctype="multipart/form-data">
                ...
                <table>
                    <tr>
                        <td>
                            <input style="width: 250px; height: 30px;" id="InsurePic" name="InsurePic"
                                class="easyui-filebox"
                                data-options="prompt:'来货订单照片',buttonText:'点击选择',onChange:change_photo" />
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <div id="InsureImgdiv" style="display: none;">
        ...
        <script type="text/javascript">
            ...
            function showStockTagGrid() {
                ...
                columns.push({
                    ...
                    formatter: function (val, row, index) { if (val == "0") { return "未扫描"; } else if (val == "1") { return "正常"; } else if (val == "2") { return "已出库"; } else if (val == "3") { return "未入库"; } else if (val == "4") { return "货位错误"; } else if (val == "5") { return "非盘点单标签"; } else { return "未扫描"; } }
                });
