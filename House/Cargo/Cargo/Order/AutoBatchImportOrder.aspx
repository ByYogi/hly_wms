<%@ Page Title="汽配批量导入开单" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="AutoBatchImportOrder.aspx.cs" Inherits="Cargo.Order.AutoBatchImportOrder" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            //});
            columns.push({
                title: '订单类型', field: 'OrderType', formatter: function (val, row, index) {
            ...
            columns.push({ title: '客户编码', field: 'SourceCode' });
        ...
        //导入
        function Import() {
            $('#dlg').dialog('open').dialog('setTitle', '批量导入订单数据');
            showData();
        ...
        function editItemByID(Did) {
            ...
            if (row) {
                $('#dlgedit').dialog('open').dialog('setTitle', '查看订单：' + row.GtmcNo);
                $('#fmDep').form('clear');
                $('#fmDep').form('load', row);
                showGrid();
                $("#btnSave").hide();
                ...
                gridOpts.url = 'orderApi.aspx?method=QueryAutoPartOrderByID&TID=' + row.TID ;
            }
        }
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
        ...
        <table id="saPanelTab">
            <tr>
               <%-- <td style="text-align: right;">路线名:
                ...
                <td style="text-align: right;" id="shry">客户编码:
                </td>
                <td>
                    <input id="SourceCode" class="easyui-textbox" data-options="prompt:'请输入客户编码'" style="width: 90px">
                </td>
            ...
            <tr>
              <%--  <td style="text-align: right;">便次:
                ...
                <td>
                    <input id="SourceName" class="easyui-textbox" data-options="prompt:'请输入客户名称'" style="width: 90px">
                </td>
    ...
    <div id="dlgPL" class="easyui-dialog" style="width: 1000px; height: 550px; padding: 2px 2px" closed="true" buttons="#dlgPL-buttons">
        ...
        <div id="dgPLInporttoolbar">
            ...
            <a href="../upload/sFile/汽配销售单导入模板.xls" id="dowloadTaishiHouse" class="easyui-linkbutton" iconcls="icon-application_put" plain="false">&nbsp;下载模板&nbsp;</a>&nbsp;&nbsp;
        ...
        <form id="fmDep" class="easyui-form" method="post">
            ...
            <table id="saPanel">
                <tr>
                  ...
                  --%>
                     <td style="text-align: right;">预录单号:
                    </td>
                    <td>
                    ...
                    <td>
                        <input name="SourceCode" class="easyui-textbox" style="width: 100px;" />
                    </td>
                    <td style="text-align: right;">客户名称:
                    </td>
                    <td>
    ...
    <script type="text/javascript">
        ...
        function showGrid() {
            ...
            columns.push({ title: '客户编码', field: 'SourceCode' });
            columns.push({ title: '客户名称', field: 'SourceName' });
            columns.push({ title: '产品大类', field: 'TypeParentName' });
            columns.push({ title: '产品小类', field: 'TypeName' });
            columns.push({ title: '产品代码', field: 'GoodsCode' });
            ...
            columns.push({ title: '库存数量', field: 'Stock', align: 'right' });
        ...
        function showPLData() {
            ...
            columns.push({ title: '客户编码', field: 'PayClientNum'});
            columns.push({ title: '客户名称', field: 'PayClientName' });
            columns.push({ title: '产品大类', field: 'TypeParentName' });
            columns.push({ title: '产品小类', field: 'TypeName' });
            ...
            //columns.push({ title: '纳入时间', field: 'TakeTime', width: '130px', formatter: DateTimeFormatter });
            columns.push({ title: '订单数量', field: 'Piece', align: 'right' });
            columns.push({ title: '销售价格', field: 'SalePrice', align: 'right' });
            //columns.push({
        ...
        function ajaxFilePLUpload() {
            ...
            $.ajaxFileUpload({
                url: 'orderApi.aspx?method=savePLOrderFile',
        ...
        function btnPLSaveData() {
            ...
            $.messager.confirm('<%= Cargo.Common.GetSystemNameAndVersion()%>', msg, function (r) {
                if (r) {
                    ...
                    $.ajax({
                        url: 'orderApi.aspx?method=SavePLAutoImportData',
        ...
        function btnSaveData() {
            ...
            if (IsExistOrder == 1) {
                $.messager.alert('<%= Cargo.Common.GetSystemNameAndVersion()%>', '已存在该订单号数据，请核对后保存!！', 'warning');
                return;
            }
            var msg = "确定保存？";
            ...
            $.messager.confirm('<%= Cargo.Common.GetSystemNameAndVersion()%>', msg, function (r) {
                if (r) {
                    ...
                    var json = JSON.stringify(rows)
                    $.ajax({
                        url: 'orderApi.aspx?method=SaveAutoPartImportData',
