        ...
        public void saveOrderBillAccount()
        {
            ...
            try
            {
                ...
                foreach (Hashtable grid in GridRows)
                {
                    ...
                    string orderNo = grid["OrderNo"] == null ? "" : Convert.ToString(grid["OrderNo"]);
                ...
                List<CargoOrderGoodsEntity> OrderGoodsList = new List<CargoOrderGoodsEntity>();
                if (orderIDList.Count() > 0)
                {
                    OrderGoodsList = fin.cargoSupplySalePrice(new CargoOrderGoodsEntity() { OrderNoStr = string.Join("','", orderNoList) });
                }
                ...
                foreach (Hashtable grid in GridRows)
                {
                    ...
                    int OrderID = Convert.ToInt32(grid["OrderID"]); 
                    //查询进仓价总价
                    List<CargoOrderGoodsEntity> OrderGoods = OrderGoodsList.Where(w => w.OrderNo == OrderNo).ToList();
                    ...
                    var OrderCoupons = couponList.Where(w => w.OrderID == OrderID).ToList();

                    foreach (var item in OrderCoupons)
                    {
                        //查询优惠卷   平台卷
                        ...
                        if (item.CouponType == "0" && item.ActualType == "2")
                        {
                            var halfMoney = item.Money / 2;
                            TotalCharge += halfMoney;
                            InsuranceFee -= halfMoney;
                        }
                    }

                    string couponType = Convert.ToString(grid["CouponType"])??"0";
                    if (!OrderCoupons.Any() && InsuranceFee != 0 && couponType == "0")
                    {
                        //TotalCharge += InsuranceFee;
                        InsuranceFee = 0;
                    }
                    ...
                    //    OutStorageFee = 0;
                    //}
                    if(Convert.ToDateTime(grid["CreateDate"]) > Convert.ToDateTime("2025-09-01")) { 
                    //出仓费减半
                    ...
                    var InOrderGoods = OrderGoods.Where(w => w.OrderNo == Convert.ToString(grid["OrderNo"])).ToList();
                    //进仓价
                    if (InOrderGoods.Any(w => w.SupplySalePrice == 0))
                    {
                        msg.Message+= Convert.ToString(grid["OrderNo"])+ "无进仓价,不可分账<br />";
                        continue;
                    }
                    InHousePrice = InOrderGoods.Sum(w => w.SupplySalePrice * w.Piece);
                    ...
                    if (entire)
                    {
                        //供应商 :分账金额=进仓价-供应商优惠卷-出仓费
                        SupplierMoney = InHousePrice - InsuranceFee - OutStorageFee- OverDueFee;
                        //仓库 :分账金额=物流费+出仓费+超期费
                        HouseMoney = TransitFee + DeliveryFee + OverDueFee + OutStorageFee;
                    ...
                    {
                        //供应商 :分账金额=-进仓价+出仓费;
                        SupplierMoney = InHousePrice - OutStorageFee;
                        //仓库 :分账金额=-出仓费;
                    ...
                    //{
                        ...
                        //TransitFee = 0;
                        ////查询订单进价
                        //InHousePrice = OrderGoods.Where(w=>w.OrderNo==Convert.ToString(grid["OrderNo"])).Sum(w=>w.SupplySalePrice * w.Piece);
                        ////如果没有进仓价，不分账，人工调整
