            ...
            try
            {
                ...
                foreach (Hashtable grid in GridRows)
                {
                    ...
                    if (Convert.ToString(grid["OrderModel"]) == "1")
                    {
                        ...
                        //判断是不是退整单
                        if (orderData.Piece != Convert.ToInt32(grid["Piece"]))
                        {
                            entire = false;
                        }
                    ...
                    if (Convert.ToString(grid["IsTransitFee"]) == "1")
                    {
                    ...
                    decimal fee = TotalCharge / Convert.ToInt32(grid["Piece"]);
                    OtherFee = Math.Ceiling(fee - fee / Convert.ToDecimal(1.009)) * Convert.ToInt32(grid["Piece"]);
                    ...
                    if (entire)
                    {
                        ...
                        HouseMoney = InsuranceFee + TransitFee + DeliveryFee + OverDueFee + OutStorageFee + HTransitFee;
                    }
                    //否
                    if (!entire)
                    {
                    ...
                    suppBillGoodsList.Add(new CargoSuppClientAccountGoodsEntity
                    {
                        ...
                        OrderNo = Convert.ToString(grid["OrderNo"]),
                        Total = SupplierMoney,
                    ...
                    houseBillGoodsList.Add(new CargoSuppClientAccountGoodsEntity
                    {
                        ...
                        ClientNum = Convert.ToString(grid["ClientNum"]),
                        HouseID = Convert.ToInt32(grid["HouseID"]),
                        HouseName = Convert.ToString(grid["OutHouseName"]),
                        OrderNo = Convert.ToString(grid["OrderNo"]),
                        Total = HouseMoney,
                        InsuranceFee = InsuranceFee,
                        TransitFee = !HTransitFee.Equals(0) ? HTransitFee : TransitFee,
                        DeliveryFee = DeliveryFee,
                        OverDueFee = OverDueFee,
                        OutStorageFee = OutStorageFee,
                        OtherFee = OtherFee,
                ...
                foreach (int it in supp)
                {
                    ...
                    //账单编号
                    bill.AccountNO = Common.GetMaxAccountNoNum();
