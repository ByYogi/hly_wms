<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="qyScanOrderSign.aspx.cs" Inherits="Cargo.QY.qyScanOrderSign" %>
...
<body ontouchstart>
    ...
    <script type="text/javascript">
            ...
            $.ajax({
                ...
                success: function (text) {
                    ...
                    if (it.AwbStatus == "0" || it.AwbStatus == "1" || it.AwbStatus == "2" || it.AwbStatus == "3" || it.AwbStatus == "4") {
                        str += "<a href = \"javascript:PickOrder(" + it.OrderID + ");\" id = \"btnOk\"  class=\"weui-btn weui-btn_primary bg-gray f-black\" style = \"width: 45%; float: left; margin-left: 20px;\">接&nbsp;&nbsp;单</a>";
                    }
                    if (it.AwbStatus == "8") {
        ...
        function PickOrder(no) {
            $.confirm("确认接单?", "确认", function () {
                $.ajax({
                    url: 'qyServices.aspx?method=SetOrderStatus&AwbStatus=8',
                    type: 'post', dataType: 'json', data: { data: no },
                    success: function (text) {
                        if (text.Result == true) {
                            $.toast("接单成功!");
                            window.location.reload();
                            //$.alert("接单成功", "操作提示", function () {
