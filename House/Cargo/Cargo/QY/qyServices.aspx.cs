...
namespace Cargo.QY
{
    public partial class qyServices : QYBasePage
    {
        ...
        public void UpdateOrderStatus()
        {
            ...
            if (order.AwbStatus.Equals("0")&& Convert.ToString(Request["status"])=="5") //订单未出库不允许直接签收
            {
                msg.Message = "订单未出库不允许直接签收！";
                msg.Result = false;
            }
            try
            {
        ...
        public void AddOrderSign()
        {
            ...
            if (order.AwbStatus.Equals("0")) //订单未出库不允许直接签收
            {
                msg.Message = "订单未出库不允许直接签收！";
                msg.Result = false;
            }
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (IsEnd)
                    {
                        ...
                        else if (ApproveEntity.ApplyType.Equals("20"))
                        {
                            //预收款申请审批结束，增加客户的预收款金额数据
                            //weiXinBus.AddCoupon(new List<WXCouponEntity> { new WXCouponEntity { ClientID = ApproveEntity.ClientID, Money = ApproveEntity.LimitMoney, UseStatus = "0", GainDate = DateTime.Now, StartDate = DateTime.Now, EndDate = DateTime.Now.AddDays(10) } }, log);
                        }
                    }
                    if (ApproveEntity.ApplyType.Equals("4"))
                    ...
                    {
                        send.content = "<div></div><div>申请类型：预收款审批</div><div>申请标题：" + ApproveEntity.Title + "</div><div>申请内容：" + ApproveEntity.Memo + "</div><div>审批人：" + QyUserInfo.UserName + "</div><div>审批意见：" + result.DenyReason + "</div><div></div><div class=\"highlight\">请点击本通知进行审批！</div>";
                    }
                    foreach (var it in qyUser)
                    {
                        try
        ...
        public void ScanMoveOrderInCargo()
        {
            ...
            {
                msg.Result = false;
                msg.Message = "移库单是开单状态，不允许入库";
                goto ERR;
            }
            if (mOrderh.Equals("3"))
            {
                msg.Result = false;
                msg.Message = "移库单未全部出库，不允许入库";
                goto ERR;
            }
            #endregion
            //查询原仓库的产品信息
