<%@ Page Title="公告管理" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Notice.aspx.cs" Inherits="Cargo.systempage.Notice" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    ...
    <script type="text/javascript">
        ...
        function dosearch() {
            ...
            gridOpts.url = 'sysService.aspx?method=QueryCargoNotice';
            $('#dg').datagrid('load', {
                Title: $('#ATitle').val(),
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <script type="text/javascript">
        ...
        function DelItem() {
            ...
            $.messager.confirm('<%= Cargo.Common.GetSystemNameAndVersion()%>', '确定删除？', function (r) {
                if (r) {
                    ...
                    $.ajax({
                        url: 'sysService.aspx?method=DelCargoNotice',
        ...
        function saveItem() {
            var idN = $('#ID').val();
            var titleN = $('#Title').val();
            $('#fm').form('submit', {
                url: 'sysService.aspx?method=SaveCargoNotice',
        ...
        //新增已读信息---update=1
        function UpdateNoticeView(ID, Title) {
            $.ajax({
                async: false, cache: false, dataType: "json",
                url: "sysService.aspx?method=SaveCargoNoticeUserView&ID=" + ID + "&update=" + 1 + "&ReadStatus=" + 1 + "&MessageType=" + 0 + "&Title=" + Title,
                success: function (text) {
                    if (text.Result == true) {
                        $('#dgNoticeView').datagrid('reload');
                        QueryIsNoticeView();
