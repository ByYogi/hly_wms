...
namespace Cargo.Report
{
    public partial class reportApi : BasePage
    {
        ...
        public void QueryInDailyReport()
        {
            ...
            if (Convert.ToString(Request["TyreModel"]) != "-1")
            {
                queryEntity.TyreModel = Convert.ToString(Request["TyreModel"]);
            }
        ...
        public void QuerySaleManDailyReportForExport()
        {
            ...
            int pageSize = Convert.ToInt32(Request["rows"]) == 0 ? 5000000 : Convert.ToInt32(Request["rows"]);
            CargoReportBus bus = new CargoReportBus();
            Hashtable list = bus.QuerySaleManDailyReport(pageIndex, pageSize, queryEntity);
            string err = "OK";
        ...
        /// 查询时效配置数据
        /// </summary>
        public void QueryReportCityAgeingSet()
        {
            CargoReportCityAgeingSetEntity queryEntity = new CargoReportCityAgeingSetEntity();

            queryEntity.DepProvince = Convert.ToString(Request["DepProvince"]);
            queryEntity.DepCity = Convert.ToString(Request["DepCity"]);
            queryEntity.DestProvince = Convert.ToString(Request["DestProvince"]);
            queryEntity.DestCity = Convert.ToString(Request["DestCity"]);
            if (Convert.ToString(Request["DelFlag"]) != "-1")
            {
            ...
            int pageSize = Convert.ToInt32(Request["rows"]);
            CargoReportBus bus = new CargoReportBus();
            Hashtable list = bus.QueryReportCityAgeingSet(pageIndex, pageSize, queryEntity);
            //JSON
            String json = JSON.Encode(list);
            Response.Clear();
            Response.Write(json);
        ...
        /// 删除
        /// </summary>
        public void DelReportCityAgeingSet()
        {
            String json = Request["data"];
            if (String.IsNullOrEmpty(json)) return;
            List<CargoReportCityAgeingSetEntity> list = new List<CargoReportCityAgeingSetEntity>();
            ...
            try
            {
                ...
                {
                    list.Add(new CargoReportCityAgeingSetEntity
                    {
                        MID = Convert.ToInt64(row["MID"]),
                        DepProvince = Convert.ToString(row["DepProvince"]),
                        DepCity = Convert.ToString(row["DepCity"]),
                        DestProvince = Convert.ToString(row["DestProvince"]),
                        DestCity = Convert.ToString(row["DestCity"]),
                        DelFlag = Convert.ToString(row["DelFlag"])
                    });
                }
                bus.DelReportCityAgeingSet(list, log);
                msg.Result = true;
                msg.Message = "成功";
            }
        ...
        /// 保存时效配置
        /// </summary>
        public void SaveReportCityAgeingSet()
        {
            CargoReportCityAgeingSetEntity ent = new CargoReportCityAgeingSetEntity();
            CargoReportBus bus = new CargoReportBus();
            ErrMessage msg = new ErrMessage(); msg.Message = "";
            msg.Result = false;
            LogEntity log = new LogEntity();
            ...
            try
            {
                ent.DepProvince = Convert.ToString(Request["DepProvince"]);
                ent.DepCity = Convert.ToString(Request["DepCity"]);
                ent.DestProvince = Convert.ToString(Request["DestProvince"]);
                ent.DestCity = Convert.ToString(Request["DestCity"]);
                ent.DelFlag = Convert.ToString(Request["DelFlag"]);
                ent.ArriveAgeHour = Convert.ToDecimal(Request["ArriveAgeHour"]);
                ent.SignAgeHour = Convert.ToDecimal(Request["SignAgeHour"]);
                ent.OPID = UserInfor.LoginName;
                if (id == "")
                {
                    log.Operate = "A";
                    bus.AddReportCityAgeingSet(ent, log);
                    msg.Result = true;
                    msg.Message = "成功";
                ...
                {
                    log.Operate = "U";
                    ent.MID = Convert.ToInt32(id);
                    bus.UpdateReportCityAgeingSet(ent, log);
                    msg.Result = true;
                    msg.Message = "成功";
