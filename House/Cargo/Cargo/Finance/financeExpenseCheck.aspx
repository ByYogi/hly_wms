<%@ Page Title="财务报销审批" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="financeExpenseCheck.aspx.cs" Inherits="Cargo.Finance.financeExpenseCheck" %>
        ...
        $(document).ready(function () {
            $('#dg').datagrid({
                ...
                columns: [[
                  ...
                  { title: '报销金额', field: 'ExCharge', width: '60px', align: 'right' },
                  {
                  ...
                  },
                  {
                      ...
                      formatter: function (val, row, index) { if (val == "0") { return "待审"; } else if (val == "1") { return "审批中"; } else if (val == "2") { return "拒审"; } else if (val == "3") { return "结束"; } else if (val == "4") { return "审批中"; } else if (val == "5") { return "审批中"; } else { return ""; } }
                  },
                  //{ title: '下一审批人', field: 'NextCheckName', width: '80px' },
                  {
                  ...
                  {
                      title: '拒审原因', field: 'DenyReason', width: '150px', formatter: function (value) {
                          if (value != null && value != "") {
                              return "<span title='" + value + "'>" + value + "</span>";
                          }
                      }
                  ...
                  {
                      field: 'opt', title: '操作', width: '150px', align: 'left',
                      formatter: function (value, rec, index) {
                          var btn = '<a class="editcls" onclick="check(\'' + index + '\')" href="javascript:void(0)">通过审批</a><a class="delcls" onclick="Deny(\'' + index + '\')" href="javascript:void(0)">拒审</a>';
                          return btn;
                      }
                  }
                ]],
                onLoadSuccess: function (data) {
                    $('.editcls').linkbutton({ text: '通过审批', plain: true, iconCls: 'icon-ok' });
                    $('.delcls').linkbutton({ text: '拒审', plain: true, iconCls: 'icon-no' })
                },
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <div id="toolbar">
        <a href="#" class="easyui-linkbutton" iconcls="icon-search" plain="false" onclick="dosearch()">&nbsp;查&nbsp;询&nbsp;</a>&nbsp;&nbsp;<a href="#" class="easyui-linkbutton" iconcls="icon-lock_add"
            plain="false" onclick="plcheck()">&nbsp;批量审批&nbsp;</a>&nbsp;&nbsp;<a href="#" class="easyui-linkbutton"
                iconcls="icon-lock_open" plain="false" onclick="plDeny()">&nbsp;批量拒审&nbsp;</a>&nbsp;&nbsp;<a
                    href="#" class="easyui-linkbutton" iconcls="icon-rss_valid" plain="false" onclick="chuna()">&nbsp;出&nbsp;纳&nbsp;</a>&nbsp;&nbsp;<a
    ...
    <%-- <div id="dlg" class="easyui-dialog" style="width: 300px; height: 150px; padding: 6px 6px"
        closed="true" buttons="#dlg-buttons">
        <input id="BNID" type="hidden" />
        <input id="BExID" type="hidden" />
        <input type="hidden" name="UserName" id="BUserName" />
        <table>
            <tr>
                <td style="text-align: right;">审批人:
                </td>
                <td>
                    <input name="UserID" id="BUserID" class="easyui-combobox" style="width: 150px;" />
                </td>
            </tr>
        </table>
    </div>
    <div id="dlg-buttons">
        <a href="#" class="easyui-linkbutton" iconcls="icon-ok" onclick="appOK()">&nbsp;保&nbsp;存&nbsp;</a>&nbsp;&nbsp;
        <a href="#" class="easyui-linkbutton" iconcls="icon-cancel" onclick="javascript:$('#dlg').dialog('close')">&nbsp;关&nbsp;闭&nbsp;</a>
    </div>--%>
    <div id="dlgDeny" class="easyui-dialog" style="width: 400px; height: 300px; padding: 6px 6px"
        closed="true" buttons="#dlgDeny-buttons">
        <input id="CNID" type="hidden" />
        <input id="CExID" type="hidden" />
        <table>
            <tr>
                <td style="text-align: right;">拒审原因:
                </td>
                <td>
                    <textarea id="DenyReason" rows="6" name="DenyReason" style="width: 250px;"></textarea>
                </td>
            </tr>
        </table>
    </div>
    <div id="dlgDeny-buttons">
        <a href="#" class="easyui-linkbutton" iconcls="icon-ok" onclick="updateRow()">&nbsp;保&nbsp;存&nbsp;</a>&nbsp;&nbsp;
    ...
    <script type="text/javascript">
        ...
        function chuna() {
            ...
            if (rows == null || rows == "") { $.messager.alert('<%= Cargo.Common.GetSystemNameAndVersion()%>', '请选择要到出纳的数据！', 'warning'); return; }
            $.messager.confirm('<%= Cargo.Common.GetSystemNameAndVersion()%>', '确定到出纳？', function (r) {
                if (r) {
                    var json = JSON.stringify(rows)
                    $.ajax({
                        url: "../Finance/financeApi.aspx?method=updateExpenseCashier",
                        type: 'post', dataType: 'json', data: { data: json }, async: false,
                        success: function (text) {
                            if (text.Result == true) {
                                $.messager.alert('<%= Cargo.Common.GetSystemNameAndVersion()%>', '到出纳成功!', 'info');
                                $('#dg').datagrid('reload');
                            }
        ...
        function updateRow() {
            ...
            if (eid == "") {//批量拒审
                ...
                row = $("#dg").datagrid('getData').rows[nid];
                if (row == null || row == "") { $.messager.alert('<%= Cargo.Common.GetSystemNameAndVersion()%>', '请选择要拒审的数据！', 'warning'); return; }
                json = JSON.stringify([row])
            }
            $.ajax({
                url: "../Finance/financeApi.aspx?method=updateExpenseStatus&ty=2&reason=" + escape(DenyReason),
        ...
        function plDeny() {
            ...
            if (row == null || row == "") { $.messager.alert('<%= Cargo.Common.GetSystemNameAndVersion()%>', '请选择要拒审的数据！', 'warning'); return; }
            if (row) {
                $('#dlgDeny').dialog('open').dialog('setTitle', '拒审报销');
                $('#CExID').val('');
                $('#CNID').val('');
                $('#DenyReason').val('');
            }
        ...
        function Deny(Did) {
            ...
            if (row == null || row == "") { $.messager.alert('<%= Cargo.Common.GetSystemNameAndVersion()%>', '请选择要拒审的数据！', 'warning'); return; }
            if (row) {
                $('#dlgDeny').dialog('open').dialog('setTitle', '拒审报销单号：' + row.ExID);
                $('#CExID').val(row.ExID);
                $('#CNID').val(Did);
                $('#DenyReason').val('');
            }
        ...
        function appOK() {
            var eid = $('#BExID').val();
            var nid = $('#BNID').val();
            var DenyReason = $('#BUserID').combobox('getValue');
            var uname = $('#BUserName').val();
            var row;
            var json;
            if (eid == "") {//批量审批
                row = $('#dg').datagrid('getSelections');
            ...
            $.ajax({
                url: "../Finance/financeApi.aspx?method=updateExpenseStatus&ty=1&reason=" + escape(DenyReason) + "&uname=" + escape(uname),
                type: 'post', dataType: 'json', data: { data: json },
                success: function (text) {
                    if (text.Result == true) {
                        $.messager.alert('<%= Cargo.Common.GetSystemNameAndVersion()%>', '审批成功!', 'info');
                        $('#dlg').dialog('close'); 	// close the dialog
                        reload();
                    }
                    else { $.messager.alert('<%= Cargo.Common.GetSystemNameAndVersion()%>', text.Message, 'warning'); }
        ...
        function check(Did) {
            ...
            if (row.CheckStatus == "1") {
                $.messager.alert('<%= Cargo.Common.GetSystemNameAndVersion()%>', '报销单已结清，不允许审批！', 'warning'); return;
            }
            if (row.CheckStatus == "2") {
                $.messager.alert('<%= Cargo.Common.GetSystemNameAndVersion()%>', '报销单结算中，不允许审批！', 'warning'); return;
            }
            if (row) {
                //$('#dlg').dialog('open').dialog('setTitle', '审批报销单号：' + row.ExID);
                //$('#BNID').val(Did);
                //$('#BExID').val(row.ExID);
                //$('#BUserID').combobox({
                //    url: '../systempage/sysService.aspx?method=AutoCompleteUser',
                //    valueField: 'UserID',
                //    textField: 'UserName',
                //    onSelect: function (rec) {
                //        $('#BUserName').val(rec.UserName);
                //    }
                //});
                var json = JSON.stringify([row])
                $.ajax({
                    url: "../Finance/financeApi.aspx?method=updateExpenseStatus&ty=1",
                    type: 'post', dataType: 'json', data: { data: json },
                    success: function (text) {
                        if (text.Result == true) {
                            $.messager.alert('<%= Cargo.Common.GetSystemNameAndVersion()%>', '审批成功!', 'info');
        ...
        function plcheck() {
            ...
            if (row == null || row == "") { $.messager.alert('<%= Cargo.Common.GetSystemNameAndVersion()%>', '请选择要审批的数据！', 'warning'); return; }
            if (row) {
                //$('#dlg').dialog('open').dialog('setTitle', '审批报销单');
                //$('#BExID').val('');
                //$('#BNID').val('');
                //$('#BUserID').combobox({
                //    url: '../systempage/sysService.aspx?method=AutoCompleteUser',
