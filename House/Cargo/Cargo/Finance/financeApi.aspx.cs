    ...
    public partial class financeApi : BasePage
    {
        ...
        public void refund()
        {
            ...
            if (UserInfor == null)
            {
                ...
                entity.CreateAwb = UserName;
                entity.CreateAwbID = UserName;
            }
            else
            {
        ...
        public void saveExpense()
        {
            ...
            if (msg.Result)
            {
                ...
                if (msg.Result)
                {
                    ...
                    ent.ChargeUnit = Convert.ToString(Request["ChargeUnit"]);
                        ...
                        if (row.Count > 0)
                        {
                            ...
                            if (Summarys.Length > 0)
                            {
                                foreach (string Summary in Summarys)
                                {
                                    ...
                                    {
                                        System.Data.DataTable dt = bus.CheckSummaryExist(Summary, id);
                                        if (dt.Rows.Count > 0)
                                        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    foreach (Hashtable row in rows)
                    {
                        ...
                        {
                            //审批结束，处理应用逻辑
                            bool IsPreCollect = false; bool IsAllPreCollect = true;
                            decimal ExpTotal = 0.00M;
                            CargoExpenseEntity expenseEntity = bus.GetExpenseById(result.ExID);
                            if (expenseEntity != null)
                            {
                                if (expenseEntity.exDetail.Count > 0)
                                {
                                    foreach (var exp in expenseEntity.exDetail)
                                    {
                                        if (exp.ZID.Equals(2) && exp.FID.Equals(19) && exp.SID.Equals(151))
                                        {
                                            //支出 返利费用 预收款返利
                                            IsPreCollect = true;
                                            ExpTotal += exp.DetailCharge;
                                        }
                                        else
                                        {
                                            IsAllPreCollect = false;
                                            break;
                                        }
                                    }
                                }
                            }
                            if (IsAllPreCollect && !IsPreCollect)
                            {
                                msg.Message = "报销科目必须全是预收款返利";
                                msg.Result = false;
                                goto ERROR;
                            }
                            if (IsAllPreCollect && IsPreCollect)
                            {
                                //全部是预收款返利
                                if (!expenseEntity.ExCharge.Equals(ExpTotal))
                                {
                                    msg.Message = "明细汇总金额与报销总金额不一致";
                                    msg.Result = false;
                                    goto ERROR;
                                }
                            }
        ...
        public void UpdateRefundCheckStatus()
        {
            ...
            try
            {
                ...
                entity.ClientNum = Convert.ToInt32(Request["ClientNum"]);//结算方式
                entity.CheckOutType = Convert.ToInt32(Request["CheckOutType"]);//结算方式
                ...
                entity.ThrowGood = string.IsNullOrEmpty(Convert.ToString(Request["ThrowGood"])) ? "" : Convert.ToString(Request["ThrowGood"]);//类型
                ...
                entity.OpenOrderNo = string.IsNullOrEmpty(Convert.ToString(Request["OpenOrderNo"])) ? "" : Convert.ToString(Request["OpenOrderNo"]);//关联订单
                entity.AwbStatus = string.IsNullOrEmpty(Convert.ToString(Request["AwbStatus"])) ? "" : Convert.ToString(Request["AwbStatus"]);//订单状态
                entity.OpenOrderAwbStatus = string.IsNullOrEmpty(Convert.ToString(Request["OpenOrderAwbStatus"])) ? "" : Convert.ToString(Request["OpenOrderAwbStatus"]);//关联订单状态
                ...
                entity.isReserve = Convert.ToBoolean(Request["isReserve"]);//isReserve
                ...
                if (msg.Result)
                {
                    ...
                    int IsAuto = 0;
                    bus.UpdateRefundCheckStatus(entity, log, ref IsAuto);

                    //审核通过 && 订单结算方式=预付款
                    if (IsAuto > 0 && entity.CheckOutType == 10 && entity.ClientNum != 0)
                    {
                        CargoOrderEntity order = null;
                        if (IsAuto == 1)
                            order = busOrder.QueryReturnOrderInfoByOrder(new CargoOrderEntity { OrderNo = entity.CargoOrderNo });
                        else if (IsAuto == 2)
                            order = busOrder.QueryOrderTempInfoByOrderNo(entity.CargoOrderNo);

                        var clientData = clientBus.QueryCargoClientEntity(new CargoClientEntity { ClientNum = entity.ClientNum });
                        var data = new CargoClientPreRecordEntity { ClientID = clientData.ClientID, ExID = entity.CargoOrderNo, Money = order.TotalCharge, RecordType = "0", OP_ID = entity.RefundCheckID, OperaType = "0", RebateType = "0", TryeClientCode = null, TypeID = 0, Remark = "预付款收入" };
                        clientBus.SaveRebateData(new List<CargoClientPreRecordEntity> { data }, log);
                    }
        ...
        public void AddReturnAndFactoryOrder(string orderNo, long WXID)
        {
            CargoFinanceBus bus = new CargoFinanceBus();
            ...
            try
            {
                var order = busOrder.QueryOrderInfo(new CargoOrderEntity { OrderNo = orderNo });
                var orderGoods = busOrder.QueryOrderGoodsInfo(new CargoOrderGoodsEntity { OrderNo = orderNo });
                //退货单
                int OrderNum = 0;
                CargoHouseEntity house = cargoHouse.QueryCargoHouseByID(order.HouseID);//查询仓库代码
                order.OrderNo = Common.GetMaxOrderNumByCurrentDate(order.HouseID, house.HouseCode, out OrderNum);//生成退货单号
                order.FacOrderNo = order.OrderNo;// Convert.ToString(Request["FacOrderNo"]);
                ...
                if (order.ClientNum == 165462 || order.ClientNum == 863602 || order.ClientNum == 613469 || order.ClientNum == 516958)
                {
                    ...
                    order.FinanceSecondCheckName = "2029";
                ...
                foreach (var item in orderGoods)
                {
                    ...
                    cargoFactoryOrder.FacOrderNo = order.OrderNo;
                    ...
                    cargoFactoryOrder.OrderNum = order.Piece;
                    cargoFactoryOrder.ReplyNumber = order.Piece;
                    ...
                    goods.Add(new CargoOrderGoodsEntity
                    {
                        ...
                        Piece = item.Piece,//退货数量
                        RelateOrderNo = order.OrderNo,//关联订单-原订单号
                ...
                order.AwbStatus = "0";
                order.OrderType = "0";
                order.TransitFee = order.TransitFee;//配送费
                order.Piece = order.Piece;//  Convert.ToInt32(Request["Piece"]);//总件数
                order.TransportFee = order.TransportFee;//  Convert.ToInt32(Request["Piece"]);//总件数
                order.OverDueFee = order.OverDueFee;
                //1 退 0  不退
                if (order.IsTransitFee == 0)
                    order.TotalCharge = order.TransportFee;
                else
                    order.TotalCharge = order.TransportFee + order.TransitFee;

                order.OP_ID = UserInfor.LoginName.Trim();
                order.CreateAwb = UserInfor.UserName;//开单员
                order.CreateAwbID = UserInfor.LoginName.Trim();//开单员ID
                order.OrderModel = "1";//订单类型-退货单1
                order.BusinessID = order.BusinessID;
                order.MarketType = order.MarketType;
                bus.AddReturnCargoOrder(order);//2.新增仓储订单退货单
                orderManager.AddOrderGoodsInfo(goods);//新增仓储订单退货明细

                //3.更新WxOrder退货来货入库单号
                cargoWeiXinBus.UpdateWxOrderFacOrderNo(new WXOrderEntity { FacOrderNo = order.FacOrderNo, ID = WXID });
            }
            catch (Exception ex)
            {
        ...
        public void AddReturnOrder()
        {
            ...
            {
                CargoFactoryOrderManager cargoFactoryOrderManager = new CargoFactoryOrderManager();
                var rows = JsonConvert.DeserializeObject<List<WxReturnOrderEntity>>(Request["list"]);
                var IsAllReturn = JsonConvert.DeserializeObject<bool>(Request["IsAllReturn"]);
                List<string> Ids = rows.Select(a => a.CaoroOrderID).ToList();

                //CargoOrderEntity order = bus.QueryOrder(new CargoOrderEntity { WXPayOrderNo = entity.WXPayOrderNo });
                List<CargoOrderGoodsEntity> goods = new List<CargoOrderGoodsEntity>();
                var orderList = rows.GroupBy(a => new { a.WXID, a.WXPayOrderNo, a.OrderNo, a.CargoOrderNo }).Select(a => new WxReturnOrderEntity
                {
                    WXID = a.Key.WXID,
                    WXPayOrderNo = a.Key.WXPayOrderNo,
                    OrderNo = a.Key.OrderNo,
                    CargoOrderNo = a.Key.CargoOrderNo,
                    ReturnPiece = a.Sum(aa => aa.ReturnPiece)
                }).ToList();
                //var Orders = bus.GetOrderInfos(Ids);
                CargoOrderEntity CenterOrder = null;
                int CenterHouse = 0;
                int CenterArea = 0;
                long CenterPID = 0;
                ...
                {
                    CargoOrderEntity order = bus.QueryOrder(new CargoOrderEntity { OrderNo = item.CargoOrderNo });


                    var AreaData = bus.GetAreaData(rows.FirstOrDefault().OutHouseID);

                    #region 赋值及判断
                    order.FacOrderNo = rows.FirstOrDefault().FacOrderNo;
                    var RefundCheckStatus = rows.FirstOrDefault().RefundCheckStatus;
                    if (string.IsNullOrEmpty(Convert.ToString(RefundCheckStatus)) || Convert.ToString(RefundCheckStatus) != "0")
                    {
                        msg.Message = "请仔细检查审核状态！";
                        msg.Result = false;
                        goto ERROR;
                    ...
                    // 退货来货入库单号不为空时查询退货单
                    if (!string.IsNullOrEmpty(order.FacOrderNo))
                    {
                        CargoFactoryOrderBus orderBus = new CargoFactoryOrderBus();
                        CargoFactoryOrderEntity factoryOrderEntity = orderBus.QueryFactoryOrderData(new CargoFactoryOrderEntity()
                        {
                            FacName = order.FacOrderNo
                        });
                    ...
                    if (!order.HouseID.Equals(Convert.ToInt32(rows.FirstOrDefault().OutHouseID)))
                    {
                        //获取中心仓数据
                        CenterOrder = bus.QueryCenterOrder(new CargoOrderEntity { OrderNo = item.CargoOrderNo });
                        if (CenterOrder == null)
                        {
                            msg.Message = "未获取到中心仓订单数据！";
                            msg.Result = false;
                            goto ERROR;
                        }
                    }
                    ...
                    //order.HouseID = rows.FirstOrDefault().HouseID;
                    order.HouseIDStr = rows.FirstOrDefault().HouseID.ToString();
                    order.HouseID = rows.FirstOrDefault().OutHouseID;
                    int OrderNum = 0;
                    CargoHouseBus cargoHouse = new CargoHouseBus();
                    CargoHouseEntity house = cargoHouse.QueryCargoHouseByID(order.HouseID);//查询仓库代码
                    order.OrderNo = Common.GetMaxOrderNumByCurrentDate(order.HouseID, house.HouseCode, out OrderNum);//生成退货单号
                    order.WXOrderNo = item.OrderNo;// Convert.ToString(Request["FacOrderNo"]);
                    order.FacOrderNo = order.OrderNo;// Convert.ToString(Request["FacOrderNo"]);
                    order.ThrowGood = "5";//是否抛货（5:退货订单）
                    order.OrderNum = OrderNum;//订单顺序号
                    //entity.ReturnPiece = item.ReturnPiece;//  Convert.ToInt32(Request["ReturnPiece"]);//退货件数

                    order.OutHouseName = rows.FirstOrDefault().OutHouseName;

                    //是否退运费  1  是  0否
                    order.IsTransitFee = Convert.ToInt32(rows.FirstOrDefault().IsTransitFee);

                    //查询产品明细
                    var orderGoods = rows.Where(a => a.OrderNo == item.OrderNo).ToList();
                    ...
                    foreach (var item2 in orderGoods)
                    {
                        var data = order.goodsList.Where(a => a.OrderNo == item2.CargoOrderNo && a.ProductID == item2.ProductID && a.ContainerCode == item2.ContainerCode).FirstOrDefault();
                        if (data == null)
                        {
                            msg.Message = "未获取到明细数据！";
                            msg.Result = false;
                            goto ERROR;
                        }

                        var total = Math.Round(data.ActSalePrice * Convert.ToDecimal(item2.ReturnPiece), 2);//行明细总计
                        var OverDueFee = 0M;//超期费
                        if (item2.OverDueFee > 0) OverDueFee = Math.Round(item2.OverDueFee / Convert.ToDecimal(item2.Piece) * Convert.ToDecimal(item2.ReturnPiece), 2);

                        PieceSum += item2.ReturnPiece;
                        ...
                        if (!order.HouseID.Equals(Convert.ToInt32(order.HouseIDStr)))
                        {
                            var itemData = CenterOrder.goodsList.FirstOrDefault(a => a.ProductCode == data.ProductCode && a.Specs == data.Specs && a.TypeID == data.TypeID
                            && a.GoodsCode == data.GoodsCode);
                            if (itemData == null)
                            {
                                msg.Message = "未获取到中心仓订单明细数据！";
                                msg.Result = false;
                                goto ERROR;
                            }
                            CenterHouse = itemData.HouseID;
                            CenterArea = itemData.AreaID;
                            ContainerCode = itemData.ContainerCode;
                            CenterPID = itemData.ProductID;
                        }
                        ...
                        {
                            OrderNo = order.OrderNo,//退货单号
                            Piece = item2.ReturnPiece,//退货数量
                            RelateOrderNo = item2.CargoOrderNo,//关联订单-原订单号
                            ProductID = CenterOrder != null ? CenterPID : item2.ProductID,
                            ...
                            SuitClientNum = data.SuppClientNum.ToString(),
                            SupplySalePrice = data.SupplySalePrice,//进仓价
                            OP_ID = log.UserID,
                            OutCargoID = outID,
                    ...
                    order.TransitFee = order.TransitFee;//配送费
                    var OrderPiece = order.Piece;
                    order.Piece = PieceSum;//  Convert.ToInt32(Request["Piece"]);//总件数
                    order.TransportFee = TransportFeeSum;//  Convert.ToInt32(Request["Piece"]);//总件数
                    order.OverDueFee = OverDueFeeSum;

                    order.TotalCharge = TransportFeeSum;
                    //默认已经扣了配送费，退的话要加上配送费
                    //1 退 0  不退
                    if (order.IsTransitFee == 1)
                    {
                        order.TotalCharge = TransportFeeSum + order.TransitFee;
                    }

                    //order.TotalCharge = order.TotalCharge - order.TransitFee;
                    ...
                    order.FinanceSecondCheckDate = DateTime.Now;
                    order.CheckDate = DateTime.Now;
                    bus.AddReturnCargoOrderV2(order);//2.新增仓储订单退货单
                    orderManager.AddOrderGoodsInfo(goods);//新增仓储订单退货明细
                    entity.FacOrderNo = order.OrderNo;//WxOrder退货来货入库单号
                    entity.ID = item.WXID;//WxOrderID
                    //3.更新WxOrder退货来货入库单号
                    cargoWeiXinBus.UpdateWxOrderFacOrderNo(entity);
                    //去重
                    var uniqueByDistinctBy = orderGoods.GroupBy(p => new { p.SuppClientNum, p.Batch, p.ProductID, p.ProductCode }).Select(g => g.First()).ToList();
                    foreach (var item2 in uniqueByDistinctBy)
                    {
                        var data = order.goodsList.Where(a => a.ProductID == item2.ProductID).FirstOrDefault();
                        if (!order.HouseID.Equals(Convert.ToInt32(order.HouseIDStr)))
                        {
                            var itemData = CenterOrder.goodsList.FirstOrDefault(a => a.ProductCode == data.ProductCode && a.Specs == data.Specs && a.TypeID == data.TypeID
                            && a.GoodsCode == data.GoodsCode);
                        ...
                        CargoFactoryOrderEntity cargoFactoryOrder = new CargoFactoryOrderEntity();
                        cargoFactoryOrder.FacOrderNo = entity.FacOrderNo;
                        cargoFactoryOrder.OrderType = 2;
                        cargoFactoryOrder.HouseID = order.HouseID;
                        //cargoFactoryOrder.ProductID = Convert.ToString(data.ProductID);
                        cargoFactoryOrder.ProductID = CenterOrder != null ? Convert.ToString(CenterPID) : Convert.ToString(data.ProductID);
                        cargoFactoryOrder.Source = 24;//string.IsNullOrEmpty(data.Source)?0 : Convert.ToInt32(data.Source);
                        cargoFactoryOrder.SourceName = data.SourceName;
                        cargoFactoryOrder.OP_Name = UserInfor.UserName;
                        cargoFactoryOrder.ProductName = house.CargoDepart;//item.ProductName;//"迪乐泰揭阳仓业务部";
                        cargoFactoryOrder.BelongMonth = data.BelongMonth;//当前年月;
                        cargoFactoryOrder.Born = data.Born;
                        cargoFactoryOrder.Assort = data.Assort;
                        cargoFactoryOrder.TypeID = data.TypeID;
                        cargoFactoryOrder.Model = data.Model;
                        cargoFactoryOrder.Specs = data.Specs;
                        cargoFactoryOrder.LoadIndex = data.LoadIndex;
                        cargoFactoryOrder.SpeedLevel = data.SpeedLevel;
                        cargoFactoryOrder.Figure = data.Figure;
                        cargoFactoryOrder.GoodsCode = data.GoodsCode;
                        cargoFactoryOrder.Batch = data.Batch;
                        cargoFactoryOrder.OrderNum = PieceSum;
                        cargoFactoryOrder.ReplyNumber = PieceSum;
                        cargoFactoryOrder.InCargoStatus = 0;
                        cargoFactoryOrder.UnitPrice = (double)data.UnitPrice;
                        cargoFactoryOrder.TradePrice = (double)data.TradePrice;
                        cargoFactoryOrder.SalePrice = (double)data.SalePrice;
                        cargoFactoryOrder.InHousePrice = (double)data.InHousePrice;
                        cargoFactoryOrder.WhetherTax = 0;
                        cargoFactoryOrder.CostPrice = (double)data.CostPrice;
                        cargoFactoryOrder.TaxCostPrice = (double)data.TaxCostPrice;
                        cargoFactoryOrder.NoTaxCostPrice = (double)data.NoTaxCostPrice;
                        cargoFactoryOrder.SaleMoney = (double)(order.TotalCharge - order.TransitFee);
                        cargoFactoryOrder.ReceiveName = "";
                        cargoFactoryOrder.ReceiveCity = "";
                        cargoFactoryOrder.ReceiveMobile = "";
                        cargoFactoryOrder.OP_DATE = DateTime.Now;
                        cargoFactoryOrder.Supplier = data.Supplier;//供应商
                        cargoFactoryOrder.SuppClientNum = data.SuppClientNum.ToString();//供应商编号
                        cargoFactoryOrder.BelongDepart = data.BelongDepart;//供应商编号
                        cargoFactoryOrder.SupplierAddress = data.SupplierAddress;//供应商地址
                        cargoFactoryOrder.SpecsType = order.ThrowGood == "22" ? "4" : order.ThrowGood == "23" ? "5" : "4";
                        //cargoFactoryOrder.SpecsType = data.SpecsType;
                        cargoFactoryOrder.ProductCode = item2.ProductCode;
                        cargoFactoryOrder.TypeName = data.TypeName;//【产品类型名称】推送至好来运的pName、brand
                        //1.新增工厂来货订单退货单
                        cargoFactoryOrderManager.InsertData(cargoFactoryOrder);
                        break;
                    }
                    ...
                    //if (msg.Result)
                    //{
                    //    bus.AddReturnOrderV2(entity, log);
                    //}
                }
        ...
        public void saveRepealItem()
        {
            ...
            try
            {
                ...
                entity.FacOrderNo = string.IsNullOrEmpty(Convert.ToString(submitData.FacOrderNo)) ? "" : Convert.ToString(submitData.FacOrderNo);//退货单号
                ...
                if (!string.IsNullOrEmpty(Convert.ToString(entity.FacOrderNo)))
                {
                    msg.Message = "退货单已生成，不可撤销！";
                    msg.Result = false;
                }
        ...
        public void IsChairmanApproval()
        {
            ErrMessage msg = new ErrMessage()
            {
                Message = "",
            ...
            var idsStr = Convert.ToString(Request["ids"]);
            var idList = idsStr.Split(',').ToList();
            string res = string.Empty;
            if (idList.Count == 0)
            {
                msg.Message = "未获取到单号";
            ...
            CargoFinanceBus bus = new CargoFinanceBus();
            idList = idList.Where(a => !string.IsNullOrWhiteSpace(a)).ToList();
            var list = bus.GetExpenseDetailList(idList);
            var totalNum = list.Sum(a => a.DetailCharge);
            var isApproval = false;
            if (totalNum >= 200000)
            {
                isApproval = true;
            }
            totalNum = list.Where(a => a.FID == 8 && a.SID == 67).Sum(a => a.DetailCharge);
            if (totalNum >= 50000)
            {
                isApproval = true;
            }
            msg.Message = "ok";
            msg.Result = isApproval;
            res = JSON.Encode(msg);
            Response.Write(res);
        }
        ...
        public void QueryReturnOrderDetail()
        {
            ...
            {
                String json1 = JSON.Encode(new List<CargoOrderEntity>()); //JSON
                Response.Clear();
            ...
            #endregion
            CargoFinanceBus bus = new CargoFinanceBus();
            Hashtable list = bus.QueryReturnOrderDetail(queryEntity);
            String json = JSON.Encode(list); //JSON
            Response.Clear();
            Response.Write(json);
        }
